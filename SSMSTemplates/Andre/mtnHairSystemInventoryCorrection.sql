...
***********************************************************************/
CREATE PROCEDURE [dbo].[mtnHairSystemInventoryCorrection]( @CenterID INT = NULL, @HairSystemInventorySnapshotID INT, @User NVARCHAR(25) = 'sa-Inv' )
AS
...
FROM [dbo].[lkpHairSystemInventoryBatchStatus]
...
UPDATE
    [hso]
SET
    [hso].[HairSystemOrderStatusID] = @PriorityStatusID
...
(SELECT
   ...
   , [hso].[ClientMembershipGUID]
   , [hso].[HairSystemOrderStatusID]
   , CASE WHEN [cm].[EndDate] >= GETDATE() THEN 4 ELSE @PriorityStatusID END
   , NULL
   , NULL
   , NULL
   , [hso].[CostContract]
 ...
 INNER JOIN [dbo].[datHairSystemInventoryBatch] AS [batch] ON [hstran].[HairSystemInventoryBatchID] = [batch].[HairSystemInventoryBatchID]
 INNER JOIN [dbo].[datClientMembership] AS [cm] ON [hso].[ClientMembershipGUID] = [cm].[ClientMembershipGUID]
 WHERE [batch].[HairSystemInventorySnapshotID] = @HairSystemInventorySnapshotID AND [batch].[HairSystemInventoryBatchStatusID] = @HairSystemInventoryCompletedBatchStatusID AND [batch].[IsAdjustmentCompleted] <> 1 AND [st].[HairSystemOrderStatusDescriptionShort] IN ('Conversion', 'APPLIED', 'INVNS')
   ...
   --AND dbo.fn_GetLocalDateTime(hso.LastUpdate, hso.CenterID) < @ScanDate
   AND [hso].[CenterID] = [hstran].[ScannedCenterID] AND ( [hso].[CenterID] = @CenterID OR @CenterID IS NULL )) ;
...
UPDATE [hso]
SET
    [hso].[HairSystemOrderStatusID] = CASE WHEN [cm].[EndDate] >= GETDATE() THEN @CentStatusID ELSE @PriorityStatusID END
...
UPDATE
    [hso]
SET
    [hso].[HairSystemOrderStatusID] = @InventoryNotScannedStatusID
  , [hso].[LastUpdate] = GETUTCDATE()
  , [hso].[LastUpdateUser] = @User
FROM [dbo].[datHairSystemOrder] AS [hso]
INNER JOIN [dbo].[datHairSystemInventoryTransaction] AS [hstran] ON [hso].[HairSystemOrderNumber] = [hstran].[HairSystemOrderNumber]
...
SET
  ...
  , [batch].[LastUpdateUser] = @User
FROM [dbo].[datHairSystemInventoryBatch] AS [batch]
WHERE [batch].[HairSystemInventorySnapshotID] = @HairSystemInventorySnapshotID AND [batch].[HairSystemInventoryBatchStatusID] = @HairSystemInventoryCompletedBatchStatusID AND ( [batch].[CenterID] = @CenterID OR @CenterID IS NULL ) ;

IF NOT EXISTS ( SELECT * FROM [dbo].[datHairSystemInventoryBatch] WHERE [HairSystemInventorySnapshotID] = @HairSystemInventorySnapshotID AND [IsAdjustmentCompleted] = 0 )
    BEGIN
        UPDATE [snp]
        SET
            [snp].[IsAdjustmentCompleted] = 1
          , [snp].[LastUpdate] = GETUTCDATE()
          , [snp].[LastUpdateUser] = @User
