...
SELECT
  ...
  , CASE WHEN ( ISNULL([t].[Accommodation], 'In Person Consult') LIKE 'virtual%' OR ISNULL([t].[Accommodation], 'In Person Consult') LIKE 'Video%' )
          AND ISNULL([t].[Result__c], '') IN ('Completed', 'Show Sale')
          AND ISNULL([t].[Action__c], '') IN ('Appointment', 'In House', 'Recovery')
          AND
  -- ISNULL(t.Result__c, '') IN ('Show No Sale', 'Show Sale') AND
...
/********************************** Get sales data *************************************/
SELECT
    MAX([CLT].[ClientSSID]) AS [ClientGUID]
  , [c].[MainGroup]
  , [ctr].[CenterNumber]
...
SELECT
  ...
  , [cd].[CenterNumber]
  , [cd].[FullDate]
  , [cd].[EmployeeFullName2] AS [EmployeeFullName]
...
FROM( SELECT
          *
        , CASE WHEN [CD].[EmployeeFullName] = ',' OR [CD].[EmployeeFullName] IS NULL THEN 'Unknown, Unknown' ELSE [CD].[EmployeeFullName] END AS [EmployeeFullName2]
      FROM [#CombinedData] AS [CD] ) AS [cd]
GROUP BY [cd].[MainGroup]
       , [cd].[CenterNumber]
       , [cd].[EmployeeFullName2]
       , [cd].[Accommodation]
       , [cd].[FullDate]
OPTION( RECOMPILE ) ;
...
EXEC [dbo].[spRpt_ClosingByConsultant_V2] @CenterType = 1, @StartDate = '20220201', @EndDate = '20220223' ;
--931
