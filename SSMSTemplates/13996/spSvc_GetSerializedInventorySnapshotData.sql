...
FROM [HairClubCMS].[dbo].[datSerializedInventoryAuditSnapshot]
...
FROM [HairClubCMS].[dbo].[datSerializedInventoryAuditSnapshot] AS [s]
LEFT JOIN [HairClubCMS].[dbo].[datSerializedInventoryAuditBatch] AS [b] ON [s].[SerializedInventoryAuditSnapshotID] = [b].[SerializedInventoryAuditSnapshotID]
LEFT JOIN [HairClubCMS].[dbo].[cfgCenter] AS [c] ON [b].[CenterID] = [c].[CenterID]
LEFT JOIN [HairClubCMS].[dbo].[cfgCenterManagementArea] AS [cma] ON [c].[CenterManagementAreaID] = [cma].[CenterManagementAreaID]
LEFT JOIN [HairClubCMS].[dbo].[lkpInventoryAuditBatchStatus] AS [bs] ON [b].[InventoryAuditBatchStatusID] = [bs].[InventoryAuditBatchStatusID]
LEFT JOIN [HairClubCMS].[dbo].[datEmployee] AS [e] ON [b].[CompletedByEmployeeGUID] = [e].[EmployeeGUID]
LEFT JOIN [HairClubCMS].[dbo].[datEmployee] AS [e_rv] ON [b].[ReviewCompletedByEmployeeGUID] = [e_rv].[EmployeeGUID]
LEFT JOIN [HairClubCMS].[dbo].[datSerializedInventoryAuditTransaction] AS [t] ON [b].[SerializedInventoryAuditBatchID] = [t].[SerializedInventoryAuditBatchID]
LEFT JOIN [HairClubCMS].[dbo].[cfgSalesCode] AS [sc] ON [t].[SalesCodeID] = [sc].[SalesCodeID]
LEFT JOIN [HairClubCMS].[dbo].[cfgSalesCodeCenter] AS [scc] ON( [sc].[SalesCodeID] = [scc].[SalesCodeID] AND [scc].[CenterID] = [b].[CenterID] )
LEFT JOIN [HairClubCMS].[dbo].[datSalesCodeCenterInventory] AS [scci] ON [scc].[SalesCodeCenterID] = [scci].[SalesCodeCenterID]
LEFT JOIN [HairClubCMS].[dbo].[datSerializedInventoryAuditTransactionSerialized] AS [ts] ON [t].[SerializedInventoryAuditTransactionID] = [ts].[SerializedInventoryAuditTransactionID]
LEFT JOIN [HairClubCMS].[dbo].[lkpSerializedInventoryStatus] AS [sis] ON [ts].[SerializedInventoryStatusID] = [sis].[SerializedInventoryStatusID]
LEFT JOIN [HairClubCMS].[dbo].[lkpInventoryNotScannedReason] AS [insr] ON [ts].[InventoryNotScannedReasonID] = [insr].[InventoryNotScannedReasonID]
LEFT JOIN [HairClubCMS].[dbo].[datEmployee] AS [tse] ON [ts].[ScannedEmployeeGUID] = [tse].[EmployeeGUID]
LEFT JOIN [HairClubCMS].[dbo].[cfgCenter] AS [tsc] ON [ts].[ScannedCenterID] = [tsc].[CenterID]
LEFT JOIN [HairClubCMS].[dbo].[datSalesCodeCenterInventorySerialized] AS [sccis] ON [ts].[SerialNumber] = [sccis].[SerialNumber]
LEFT JOIN [HairClubCMS].[dbo].[lkpSerializedInventoryStatus] AS [currsis] ON [sccis].[SerializedInventoryStatusID] = [currsis].[SerializedInventoryStatusID]
OUTER APPLY( SELECT TOP 1
                    [ia].[InventoryAdjustmentDate]
                  , [ctrfrom].[CenterNumber] AS [FromCenter]
                  , [ctrto].[CenterNumber] AS [ToCenter]
             FROM [HairClubCMS].[dbo].[datInventoryAdjustmentDetailSerialized] AS [iads]
             INNER JOIN [HairClubCMS].[dbo].[datInventoryAdjustmentDetail] AS [iad] ON [iads].[InventoryAdjustmentDetailID] = [iad].[InventoryAdjustmentDetailID]
             INNER JOIN [HairClubCMS].[dbo].[datInventoryAdjustment] AS [ia] ON [iad].[InventoryAdjustmentID] = [ia].[InventoryAdjustmentID]
             LEFT JOIN [HairClubCMS].[dbo].[cfgCenter] AS [ctrfrom] ON [ia].[CenterID] = [ctrfrom].[CenterID]
             LEFT JOIN [HairClubCMS].[dbo].[cfgCenter] AS [ctrto] ON [ia].[TransferToCenterID] = [ctrto].[CenterID]
