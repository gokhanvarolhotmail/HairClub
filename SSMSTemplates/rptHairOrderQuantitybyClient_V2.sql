...
SELECT
  ...
  , MIN(CASE WHEN [hsos].[HairSystemOrderStatusDescriptionShort] = 'ORDER' THEN NULLIF([clt].[CreateDate], '19000101')END) AS [OldestOrderPlacedDate]
  , MIN(CASE WHEN [hsos].[HairSystemOrderStatusDescriptionShort] = 'ORDER' THEN DATEADD(MONTH, 8, NULLIF([clt].[CreateDate], '19000101'))END) AS [OldestOrderPlacedDueDate]
INTO [#Calc01]
FROM [dbo].[datClient] AS [clt]
