/* CreateDate: 05/05/2020 17:42:39.000 , ModifyDate: 05/05/2020 17:42:39.000 */
GO
create procedure [sp_MSupd_dbolkpCenterOwnership]
		@c1 int = NULL,
		@c2 int = NULL,
		@c3 nvarchar(100) = NULL,
		@c4 nvarchar(10) = NULL,
		@c5 nvarchar(50) = NULL,
		@c6 nvarchar(50) = NULL,
		@c7 nvarchar(50) = NULL,
		@c8 nvarchar(50) = NULL,
		@c9 nvarchar(50) = NULL,
		@c10 nvarchar(50) = NULL,
		@c11 int = NULL,
		@c12 nchar(10) = NULL,
		@c13 int = NULL,
		@c14 bit = NULL,
		@c15 datetime = NULL,
		@c16 nvarchar(25) = NULL,
		@c17 datetime = NULL,
		@c18 nvarchar(25) = NULL,
		@c19 bit = NULL,
		@c20 int = NULL,
		@pkc1 int = NULL,
		@bitmap binary(3)
as
begin
	declare @primarykey_text nvarchar(100) = ''
if (substring(@bitmap,1,1) & 1 = 1)
begin
update [dbo].[lkpCenterOwnership] set
		[CenterOwnershipID] = case substring(@bitmap,1,1) & 1 when 1 then @c1 else [CenterOwnershipID] end,
		[CenterOwnershipSortOrder] = case substring(@bitmap,1,1) & 2 when 2 then @c2 else [CenterOwnershipSortOrder] end,
		[CenterOwnershipDescription] = case substring(@bitmap,1,1) & 4 when 4 then @c3 else [CenterOwnershipDescription] end,
		[CenterOwnershipDescriptionShort] = case substring(@bitmap,1,1) & 8 when 8 then @c4 else [CenterOwnershipDescriptionShort] end,
		[OwnerLastName] = case substring(@bitmap,1,1) & 16 when 16 then @c5 else [OwnerLastName] end,
		[OwnerFirstName] = case substring(@bitmap,1,1) & 32 when 32 then @c6 else [OwnerFirstName] end,
		[CorporateName] = case substring(@bitmap,1,1) & 64 when 64 then @c7 else [CorporateName] end,
		[Address1] = case substring(@bitmap,1,1) & 128 when 128 then @c8 else [Address1] end,
		[Address2] = case substring(@bitmap,2,1) & 1 when 1 then @c9 else [Address2] end,
		[City] = case substring(@bitmap,2,1) & 2 when 2 then @c10 else [City] end,
		[StateID] = case substring(@bitmap,2,1) & 4 when 4 then @c11 else [StateID] end,
		[PostalCode] = case substring(@bitmap,2,1) & 8 when 8 then @c12 else [PostalCode] end,
		[CountryID] = case substring(@bitmap,2,1) & 16 when 16 then @c13 else [CountryID] end,
		[IsActiveFlag] = case substring(@bitmap,2,1) & 32 when 32 then @c14 else [IsActiveFlag] end,
		[CreateDate] = case substring(@bitmap,2,1) & 64 when 64 then @c15 else [CreateDate] end,
		[CreateUser] = case substring(@bitmap,2,1) & 128 when 128 then @c16 else [CreateUser] end,
		[LastUpdate] = case substring(@bitmap,3,1) & 1 when 1 then @c17 else [LastUpdate] end,
		[LastUpdateUser] = case substring(@bitmap,3,1) & 2 when 2 then @c18 else [LastUpdateUser] end,
		[IsClientExperienceSurveyEnabled] = case substring(@bitmap,3,1) & 4 when 4 then @c19 else [IsClientExperienceSurveyEnabled] end,
		[ClientExperienceSurveyDelayDays] = case substring(@bitmap,3,1) & 8 when 8 then @c20 else [ClientExperienceSurveyDelayDays] end
	where [CenterOwnershipID] = @pkc1
if @@rowcount = 0
    if @@microsoftversion>0x07320000
		Begin
			if exists (Select * from sys.all_parameters where object_id = OBJECT_ID('sp_MSreplraiserror') and [name] = '@param3')
			Begin

				set @primarykey_text = @primarykey_text + '[CenterOwnershipID] = ' + convert(nvarchar(100),@pkc1,1)
				exec sp_MSreplraiserror @errorid=20598, @param1=N'[dbo].[lkpCenterOwnership]', @param2=@primarykey_text, @param3=13233
			End
			Else
				exec sp_MSreplraiserror @errorid=20598
		End
end
else
begin
update [dbo].[lkpCenterOwnership] set
		[CenterOwnershipSortOrder] = case substring(@bitmap,1,1) & 2 when 2 then @c2 else [CenterOwnershipSortOrder] end,
		[CenterOwnershipDescription] = case substring(@bitmap,1,1) & 4 when 4 then @c3 else [CenterOwnershipDescription] end,
		[CenterOwnershipDescriptionShort] = case substring(@bitmap,1,1) & 8 when 8 then @c4 else [CenterOwnershipDescriptionShort] end,
		[OwnerLastName] = case substring(@bitmap,1,1) & 16 when 16 then @c5 else [OwnerLastName] end,
		[OwnerFirstName] = case substring(@bitmap,1,1) & 32 when 32 then @c6 else [OwnerFirstName] end,
		[CorporateName] = case substring(@bitmap,1,1) & 64 when 64 then @c7 else [CorporateName] end,
		[Address1] = case substring(@bitmap,1,1) & 128 when 128 then @c8 else [Address1] end,
		[Address2] = case substring(@bitmap,2,1) & 1 when 1 then @c9 else [Address2] end,
		[City] = case substring(@bitmap,2,1) & 2 when 2 then @c10 else [City] end,
		[StateID] = case substring(@bitmap,2,1) & 4 when 4 then @c11 else [StateID] end,
		[PostalCode] = case substring(@bitmap,2,1) & 8 when 8 then @c12 else [PostalCode] end,
		[CountryID] = case substring(@bitmap,2,1) & 16 when 16 then @c13 else [CountryID] end,
		[IsActiveFlag] = case substring(@bitmap,2,1) & 32 when 32 then @c14 else [IsActiveFlag] end,
		[CreateDate] = case substring(@bitmap,2,1) & 64 when 64 then @c15 else [CreateDate] end,
		[CreateUser] = case substring(@bitmap,2,1) & 128 when 128 then @c16 else [CreateUser] end,
		[LastUpdate] = case substring(@bitmap,3,1) & 1 when 1 then @c17 else [LastUpdate] end,
		[LastUpdateUser] = case substring(@bitmap,3,1) & 2 when 2 then @c18 else [LastUpdateUser] end,
		[IsClientExperienceSurveyEnabled] = case substring(@bitmap,3,1) & 4 when 4 then @c19 else [IsClientExperienceSurveyEnabled] end,
		[ClientExperienceSurveyDelayDays] = case substring(@bitmap,3,1) & 8 when 8 then @c20 else [ClientExperienceSurveyDelayDays] end
	where [CenterOwnershipID] = @pkc1
if @@rowcount = 0
    if @@microsoftversion>0x07320000
		Begin
			if exists (Select * from sys.all_parameters where object_id = OBJECT_ID('sp_MSreplraiserror') and [name] = '@param3')
			Begin

				set @primarykey_text = @primarykey_text + '[CenterOwnershipID] = ' + convert(nvarchar(100),@pkc1,1)
				exec sp_MSreplraiserror @errorid=20598, @param1=N'[dbo].[lkpCenterOwnership]', @param2=@primarykey_text, @param3=13233
			End
			Else
				exec sp_MSreplraiserror @errorid=20598
		End
end
end
GO
