/* CreateDate: 05/05/2020 17:42:53.900 , ModifyDate: 05/05/2020 17:42:53.900 */
GO
create procedure [sp_MSupd_dbolkpHairDensityLevels]
		@c1 int = NULL,
		@c2 int = NULL,
...
update [dbo].[lkpHairDensityLevels] set
		[HairDensityLevelsSortOrder] = case substring(@bitmap,1,1) & 2 when 2 then @c2 else [HairDensityLevelsSortOrder] end,
		[HairDensityLevelsDescription] = case substring(@bitmap,1,1) & 4 when 4 then @c3 else [HairDensityLevelsDescription] end,
		[HairDensityLevelsDescriptionShort] = case substring(@bitmap,1,1) & 8 when 8 then @c4 else [HairDensityLevelsDescriptionShort] end,
		[HairDensityLevelsDescriptionLong] = case substring(@bitmap,1,1) & 16 when 16 then @c5 else [HairDensityLevelsDescriptionLong] end,
	...
	where [HairDensityLevelsID] = @pkc1
if @@rowcount = 0
    if @@microsoftversion>0x07320000
		Begin
			...
			Begin
				...
				set @primarykey_text = @primarykey_text + '[HairDensityLevelsID] = ' + convert(nvarchar(100),@pkc1,1)
				exec sp_MSreplraiserror @errorid=20598, @param1=N'[dbo].[lkpHairDensityLevels]', @param2=@primarykey_text, @param3=13233
			End
			Else
