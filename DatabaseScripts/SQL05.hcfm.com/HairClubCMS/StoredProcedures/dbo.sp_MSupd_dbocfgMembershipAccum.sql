/* CreateDate: 05/05/2020 17:42:44.440 , ModifyDate: 05/05/2020 17:42:44.440 */
GO
create procedure [sp_MSupd_dbocfgMembershipAccum]
		@c1 int = NULL,
		@c2 int = NULL,
		@c3 int = NULL,
		@c4 int = NULL,
...
update [dbo].[cfgMembershipAccum] set
		[MembershipAccumulatorSortOrder] = case substring(@bitmap,1,1) & 2 when 2 then @c2 else [MembershipAccumulatorSortOrder] end,
		[MembershipID] = case substring(@bitmap,1,1) & 4 when 4 then @c3 else [MembershipID] end,
		[AccumulatorID] = case substring(@bitmap,1,1) & 8 when 8 then @c4 else [AccumulatorID] end,
		[InitialQuantity] = case substring(@bitmap,1,1) & 16 when 16 then @c5 else [InitialQuantity] end,
    ...
    if @@microsoftversion>0x07320000
		Begin
			...
			Begin
				...
				exec sp_MSreplraiserror @errorid=20598, @param1=N'[dbo].[cfgMembershipAccum]', @param2=@primarykey_text, @param3=13233
			End
			Else
