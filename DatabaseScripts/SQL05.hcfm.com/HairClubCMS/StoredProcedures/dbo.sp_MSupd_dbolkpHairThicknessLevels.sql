/* CreateDate: 05/05/2020 17:42:54.290 , ModifyDate: 05/05/2020 17:42:54.290 */
GO
create procedure [sp_MSupd_dbolkpHairThicknessLevels]
		@c1 int = NULL,
		@c2 int = NULL,
...
update [dbo].[lkpHairThicknessLevels] set
		[HairThicknessLevelsSortOrder] = case substring(@bitmap,1,1) & 2 when 2 then @c2 else [HairThicknessLevelsSortOrder] end,
		[HairThicknessLevelsDescription] = case substring(@bitmap,1,1) & 4 when 4 then @c3 else [HairThicknessLevelsDescription] end,
		[HairThicknessLevelsDescriptionShort] = case substring(@bitmap,1,1) & 8 when 8 then @c4 else [HairThicknessLevelsDescriptionShort] end,
		[HairThicknessLevelsDescriptionLong] = case substring(@bitmap,1,1) & 16 when 16 then @c5 else [HairThicknessLevelsDescriptionLong] end,
	...
	where [HairThicknessLevelsID] = @pkc1
if @@rowcount = 0
    if @@microsoftversion>0x07320000
		Begin
			...
			Begin
				...
				set @primarykey_text = @primarykey_text + '[HairThicknessLevelsID] = ' + convert(nvarchar(100),@pkc1,1)
				exec sp_MSreplraiserror @errorid=20598, @param1=N'[dbo].[lkpHairThicknessLevels]', @param2=@primarykey_text, @param3=13233
			End
			Else
