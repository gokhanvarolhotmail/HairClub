/* CreateDate: 05/03/2010 12:09:38.180 , ModifyDate: 05/03/2010 12:09:38.180 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DDS_DimDoctorRegion_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimDoctorRegion_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_ent_stage].[spHC_DDS_DimDoctorRegion_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimDoctorRegion]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_ent_stage].[synHC_DDS_DimDoctorRegion] (
					  [DoctorRegionSSID]
			...
			SELECT
				...
				, STG.[DoctorRegionDescription]
				, STG.[DoctorRegionDescriptionShort]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[DoctorRegionDescription] = STG.[DoctorRegionDescription]
				, DW.[DoctorRegionDescriptionShort] = STG.[DoctorRegionDescriptionShort]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_ent_stage].[DimDoctorRegion] STG
		JOIN [bi_ent_stage].[synHC_DDS_DimDoctorRegion] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimDoctorRegion]
