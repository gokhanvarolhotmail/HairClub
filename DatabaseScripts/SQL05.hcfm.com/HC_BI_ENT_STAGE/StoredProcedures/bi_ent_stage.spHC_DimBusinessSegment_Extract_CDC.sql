/* CreateDate: 05/03/2010 12:09:40.410 , ModifyDate: 03/26/2018 15:46:48.313 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DimBusinessSegment_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimBusinessSegment_Extract_CDC] is used to retrieve a
-- list Business Unit
--
--   exec [bi_ent_stage].[spHC_DimBusinessSegment_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpBusinessSegment'
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						SELECT @DataPkgKey
								...
								, chg.[BusinessSegmentID] AS [BusinessSegmentSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[BusinessSegmentDescription])),'') AS nvarchar(50)) AS [BusinessSegmentDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[BusinessSegmentDescriptionShort])),'') AS nvarchar(10)) AS [BusinessSegmentDescriptionShort]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[BusinessSegmentID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpBusinessSegment](@From_LSN, @To_LSN, @row_filter_option) chg
