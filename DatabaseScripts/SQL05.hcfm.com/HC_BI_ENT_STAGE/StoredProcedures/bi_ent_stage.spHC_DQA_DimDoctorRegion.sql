/* CreateDate: 05/03/2010 12:10:02.543 , ModifyDate: 05/03/2010 12:10:02.543 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DQA_DimDoctorRegion]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimDoctorRegion] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_ent_stage].[spHC_DQA_DimDoctorRegion] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_ent_stage].[spHC_DQA_DimDoctorRegion_0000] @DataPkgKey
			END
