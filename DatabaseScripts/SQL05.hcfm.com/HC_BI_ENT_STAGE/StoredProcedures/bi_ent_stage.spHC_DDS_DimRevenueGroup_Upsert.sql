/* CreateDate: 05/03/2010 12:09:38.257 , ModifyDate: 05/03/2010 12:09:38.257 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DDS_DimRevenueGroup_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimRevenueGroup_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_ent_stage].[spHC_DDS_DimRevenueGroup_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimRevenueGroup]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_ent_stage].[synHC_DDS_DimRevenueGroup] (
					  [RevenueGroupSSID]
		...
		FROM [bi_ent_stage].[DimRevenueGroup] STG
		JOIN [bi_ent_stage].[synHC_DDS_DimRevenueGroup] DW ON
		...
		FROM [bi_ent_stage].[DimRevenueGroup] STG
		JOIN [bi_ent_stage].[synHC_DDS_DimRevenueGroup] DW ON
				DW.[RevenueGroupKey] = STG.[RevenueGroupKey]
			AND DW.[RowIsCurrent] = 1
		WHERE	STG.[IsType2] = 1
			AND	STG.[IsException] = 0
			AND STG.[DataPkgKey] = @DataPkgKey
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimRevenueGroup]
