/* CreateDate: 05/03/2010 12:10:02.147 , ModifyDate: 05/03/2010 12:10:02.147 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DQA_DimBusinessUnitBrand]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimBusinessUnitBrand] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_ent_stage].[spHC_DQA_DimBusinessUnitBrand] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_ent_stage].[spHC_DQA_DimBusinessUnitBrand_0000] @DataPkgKey
			END
