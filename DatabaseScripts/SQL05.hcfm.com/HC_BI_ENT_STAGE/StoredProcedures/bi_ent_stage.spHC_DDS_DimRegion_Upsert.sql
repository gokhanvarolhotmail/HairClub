/* CreateDate: 05/03/2010 12:09:38.233 , ModifyDate: 07/11/2011 12:44:36.403 */
GO
CREATE procedure [bi_ent_stage].[spHC_DDS_DimRegion_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimRegion_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_ent_stage].[spHC_DDS_DimRegion_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimRegion]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_ent_stage].[synHC_DDS_DimRegion] (
					  [RegionSSID]
					, [RegionDescription]
		...
		UPDATE DW SET
				  DW.[RegionDescription] = STG.[RegionDescription]
				, DW.[RegionDescriptionShort] = STG.[RegionDescriptionShort]
				, DW.[RegionSortOrder] = STG.[RegionSortOrder]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_ent_stage].[DimRegion] STG
		JOIN [bi_ent_stage].[synHC_DDS_DimRegion] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimRegion]
