/* CreateDate: 05/03/2010 12:09:37.920 , ModifyDate: 05/03/2010 12:09:37.920 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DDS_DimBusinessUnitBrand_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimBusinessUnitBrand_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_ent_stage].[spHC_DDS_DimBusinessUnitBrand_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimBusinessUnitBrand]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_ent_stage].[synHC_DDS_DimBusinessUnitBrand] (
			...
			SELECT
				...
				, STG.[BusinessUnitBrandDescription]
				, STG.[BusinessUnitBrandDescriptionShort]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[BusinessUnitBrandDescription] = STG.[BusinessUnitBrandDescription]
				, DW.[BusinessUnitBrandDescriptionShort] = STG.[BusinessUnitBrandDescriptionShort]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_ent_stage].[DimBusinessUnitBrand] STG
		JOIN [bi_ent_stage].[synHC_DDS_DimBusinessUnitBrand] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimBusinessUnitBrand]
