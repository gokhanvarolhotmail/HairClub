/* CreateDate: 05/03/2010 12:09:37.947 , ModifyDate: 05/03/2010 12:09:37.947 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DDS_DimBusinessUnit_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimBusinessUnit_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_ent_stage].[spHC_DDS_DimBusinessUnit_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimBusinessUnit]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_ent_stage].[synHC_DDS_DimBusinessUnit] (
					  [BusinessUnitSSID]
			...
			SELECT
				...
				, STG.[BusinessUnitDescription]
				, STG.[BusinessUnitDescriptionShort]
				, STG.[BusinessUnitBrandKey]
				, STG.[BusinessUnitBrandSSID]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[BusinessUnitDescription] = STG.[BusinessUnitDescription]
				, DW.[BusinessUnitDescriptionShort] = STG.[BusinessUnitDescriptionShort]
				, DW.[BusinessUnitBrandKey] = STG.[BusinessUnitBrandKey]
				, DW.[BusinessUnitBrandSSID] = STG.[BusinessUnitBrandSSID]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_ent_stage].[DimBusinessUnit] STG
		JOIN [bi_ent_stage].[synHC_DDS_DimBusinessUnit] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimBusinessUnit]
