/* CreateDate: 05/03/2010 12:10:02.227 , ModifyDate: 03/26/2018 15:54:24.670 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DQA_DimBusinessUnit_0010]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimBusinessUnit_0010] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_ent_stage].[spHC_DQA_DimBusinessUnit_0010]
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	SET @RuleKey = 10	--Log for Information
	SET @TableName = N'[bi_ent_dqa].[DimBusinessUnit]'
/*
	...
	BEGIN TRY
		...
		-- Put other clauses here
			AND ([BusinessUnitBrandKey] IS NULL)
		...
		-----------------------
		EXEC	@return_value = [bi_ent_stage].[spHC_DQA_DimBusinessUnit_Upsert] @DataPkgKey, @RuleKey, @RuleActionKey,
