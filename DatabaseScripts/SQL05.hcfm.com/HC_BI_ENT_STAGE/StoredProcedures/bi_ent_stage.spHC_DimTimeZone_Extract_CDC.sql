/* CreateDate: 05/03/2010 12:10:01.913 , ModifyDate: 05/03/2010 12:10:01.913 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DimTimeZone_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimTimeZone_Extract_CDC] is used to retrieve a
-- list TimeZone
--
--   exec [bi_ent_stage].[spHC_DimTimeZone_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpTimeZone'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						INSERT INTO [bi_ent_stage].[DimTimeZone]
								   ( [DataPkgKey]
								   , [TimeZoneKey]
								   , [TimeZoneSSID]
								   , [TimeZoneDescription]
						...
						SELECT @DataPkgKey
								, NULL AS [TimeZoneKey]
								, chg.[TimeZoneID] AS [TimeZoneSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[TimeZoneDescription])),'') AS nvarchar(50)) AS [TimeZoneDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[TimeZoneDescriptionShort])),'') AS nvarchar(10)) AS [TimeZoneDescriptionShort]
								, chg.[UTCOffset] AS [UTCOffset]
								, chg.[UsesDayLightSavingsFlag] AS [UsesDayLightSavingsFlag]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[TimeZoneID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpTimeZone](@From_LSN, @To_LSN, @row_filter_option) chg
