/* CreateDate: 05/03/2010 12:09:57.557 , ModifyDate: 07/11/2011 12:44:36.373 */
GO
CREATE procedure [bi_ent_stage].[spHC_DimRegion_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimRegion_Extract_CDC] is used to retrieve a
-- list Business Unit
--
--   exec [bi_ent_stage].[spHC_DimRegion_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpRegion'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						SELECT @DataPkgKey
								, NULL AS [RegionKey]
								, chg.[RegionID] AS [RegionSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[RegionDescription])),'') AS nvarchar(50)) AS [RegionDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[RegionDescriptionShort])),'') AS nvarchar(10)) AS [RegionDescriptionShort]
								, [RegionSortOrder]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[RegionID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpRegion](@From_LSN, @To_LSN, @row_filter_option) chg
