/* CreateDate: 05/03/2010 12:09:38.207 , ModifyDate: 05/03/2010 12:09:38.207 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DDS_DimGeography_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimGeography_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_ent_stage].[spHC_DDS_DimGeography_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimGeography]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_ent_stage].[synHC_DDS_DimGeography] (
					 [PostalCode]
		...
		FROM [bi_ent_stage].[DimGeography] STG
		JOIN [bi_ent_stage].[synHC_DDS_DimGeography] DW ON
		...
		FROM [bi_ent_stage].[DimGeography] STG
		JOIN [bi_ent_stage].[synHC_DDS_DimGeography] DW ON
				DW.[GeographyKey] = STG.[GeographyKey]
			AND DW.[RowIsCurrent] = 1
		WHERE	STG.[IsType2] = 1
			AND	STG.[IsException] = 0
			AND STG.[DataPkgKey] = @DataPkgKey
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimGeography]
