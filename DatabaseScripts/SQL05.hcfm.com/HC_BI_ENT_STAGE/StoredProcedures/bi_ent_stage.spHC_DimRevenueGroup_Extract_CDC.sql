/* CreateDate: 05/03/2010 12:09:59.777 , ModifyDate: 05/03/2010 12:09:59.777 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DimRevenueGroup_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimRevenueGroup_Extract_CDC] is used to retrieve a
-- list Business Unit
--
--   exec [bi_ent_stage].[spHC_DimRevenueGroup_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpRevenueGroup'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						INSERT INTO [bi_ent_stage].[DimRevenueGroup]
								   ( [DataPkgKey]
								   , [RevenueGroupKey]
								   , [RevenueGroupSSID]
						...
						SELECT @DataPkgKey
								, NULL AS [RevenueGroupKey]
								, chg.[RevenueGroupID] AS [RevenueGroupSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[RevenueGroupDescription])),'') AS nvarchar(50)) AS [RevenueGroupDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[RevenueGroupDescriptionShort])),'') AS nvarchar(10)) AS [RevenueGroupDescriptionShort]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[RevenueGroupID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpRevenueGroup](@From_LSN, @To_LSN, @row_filter_option) chg
