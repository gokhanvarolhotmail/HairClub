/* CreateDate: 05/03/2010 12:09:38.297 , ModifyDate: 05/03/2010 12:09:38.297 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DDS_DimTimeZone_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimTimeZone_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_ent_stage].[spHC_DDS_DimTimeZone_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimTimeZone]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_ent_stage].[synHC_DDS_DimTimeZone] (
					  [TimeZoneSSID]
					, [TimeZoneDescription]
			...
			SELECT
				...
				, STG.[TimeZoneDescriptionShort]
				, STG.[UTCOffset]
				, STG.[UsesDayLightSavingsFlag]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				...
				, DW.[TimeZoneDescriptionShort] = STG.[TimeZoneDescriptionShort]
				, DW.[UTCOffset] = STG.[UTCOffset]
				, DW.[UsesDayLightSavingsFlag] = STG.[UsesDayLightSavingsFlag]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_ent_stage].[DimTimeZone] STG
		JOIN [bi_ent_stage].[synHC_DDS_DimTimeZone] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimTimeZone]
