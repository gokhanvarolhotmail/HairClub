/* CreateDate: 05/03/2010 12:09:38.033 , ModifyDate: 05/03/2010 12:09:38.033 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DDS_DimCenterType_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimCenterType_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_ent_stage].[spHC_DDS_DimCenterType_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimCenterType]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_ent_stage].[synHC_DDS_DimCenterType] (
					  [CenterTypeSSID]
			...
			SELECT
				  STG.[CenterTypeSSID]
				, STG.[CenterTypeDescription]
				, STG.[CenterTypeDescriptionShort]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[CenterTypeDescription] = STG.[CenterTypeDescription]
				, DW.[CenterTypeDescriptionShort] = STG.[CenterTypeDescriptionShort]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_ent_stage].[DimCenterType] STG
		JOIN [bi_ent_stage].[synHC_DDS_DimCenterType] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_ent_stage].[synHC_DDS_DimCenterType]
