/* CreateDate: 05/03/2010 12:10:02.020 , ModifyDate: 05/03/2010 12:10:02.020 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DQA_DimBusinessSegment]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimBusinessSegment] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_ent_stage].[spHC_DQA_DimBusinessSegment] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_ent_stage].[spHC_DQA_DimBusinessSegment_0000] @DataPkgKey
			END

		-----------------------
		-- Validate Rule 6 - BusinessUnit cannot be NULL
		-----------------------
		EXEC	@return_value = [bi_ent_stage].[spHC_DQA_DimBusinessSegment_0012] @DataPkgKey
