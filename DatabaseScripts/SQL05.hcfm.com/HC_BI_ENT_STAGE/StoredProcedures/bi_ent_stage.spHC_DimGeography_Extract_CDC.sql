/* CreateDate: 05/03/2010 12:09:55.490 , ModifyDate: 05/03/2010 12:09:55.490 */
GO
CREATE PROCEDURE [bi_ent_stage].[spHC_DimGeography_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimGeography_Extract_CDC] is used to retrieve a
-- list Contact Emails
--
--   exec [bi_ent_stage].[spHC_DimGeography_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_onca_zip'
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HCM].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						SELECT @DataPkgKey
								...
								, CAST(ISNULL(LTRIM(RTRIM(chg.[zip_code])),'') AS nvarchar(15)) AS [PostalCode]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[country_code])),'') AS nvarchar(50)) AS [CountryRegionDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[country_code])),'') AS nvarchar(10)) AS [CountryRegionDescriptionShort]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[state_code])),'') AS nvarchar(50)) AS [StateProvinceDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[state_code])),'') AS nvarchar(10)) AS [StateProvinceDescriptionShort]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[zip_code])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HCM].[cdc].[fn_cdc_get_net_changes_dbo_onca_zip](@From_LSN, @To_LSN, @row_filter_option) chg
