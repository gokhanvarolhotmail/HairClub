/* CreateDate: 05/02/2010 09:45:05.940 , ModifyDate: 05/02/2010 09:45:05.940 */
GO
CREATE PROCEDURE [bief_config].[_DBErrorLog_LogError]
(
...
BEGIN
	...
	SET @IsErrorLoggingEnabled = (SELECT [bief_config].[fn_IsErrorLoggingEnabled]())
    ...
    BEGIN TRY
		...
		IF (@IsErrorLoggingEnabled = 1)
			BEGIN
				INSERT [bief_config].[_DBErrorLog]
					(
					  [UserName]
					, [ErrorNumber]
					, [ErrorSeverity]
    ...
    BEGIN CATCH
        ...
        EXECUTE [bief_config].[_DBErrorLog_PrintError];
        RETURN -1;
    END CATCH
END;
GO
