/* CreateDate: 10/11/2017 16:22:46.880 , ModifyDate: 10/30/2017 12:08:22.253 */
GO
-- =============================================
-- Author:		Daniel Polania
-- Create date: 09/21/2017
-- Description:	Gathers all activites for existing leads already in salesforce
-- EXEC dbo.oncd_contact_activities_salesforce_ExistingLead_updated_diff
-- =============================================
CREATE PROCEDURE [dbo].[oncd_contact_activities_salesforce_ExistingLead_updated_diff]
AS
BEGIN
    ...
    WHERE OAC.updated_date >= @creation_date
    ...
    WHERE OA.result_code IN ( 'CANCEL', 'RESCHEDULE', 'APPOINT', 'RECOVERY' )
          ...
          AND OAC.updated_date >= @creation_date
          AND ISNULL(OA.cst_do_not_export, 'N') = 'N'
    ORDER BY c.contact_id,
             OA.completion_date,
             OAU.user_code;
    ...
    SELECT T.activity_id,
           ...
           END AS CreatedByL,
           CASE
               WHEN T.action_code = 'Appointment' THEN
    ...
    FROM #temp AS T
        ...
        LEFT JOIN dbo.SF_OnContact_ActiveUsers AS SOACM
            ON SOACM.TM_Num = T.completed_by_user_code
    ...
    SELECT a.activity_id,
           ...
           END AS CreatedByL,
           CASE
               WHEN a.action_code = 'APPOINT' THEN
