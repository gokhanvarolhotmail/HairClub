/* CreateDate: 09/25/2017 17:38:29.577 , ModifyDate: 10/30/2017 12:02:58.153 */
GO
-- =============================================
-- Author:		Daniel Polania
-- Create date: 09/21/2017
-- Description:	Gathers all activites associated with cleints since 2013 to be ingested by Salesforce

-- EXEC [dbo].[oncd_client_activities_salesforce]
-- =============================================
CREATE PROCEDURE [dbo].[oncd_client_activities_salesforce]
AS
BEGIN
    ...
    FROM HCM.dbo.oncd_contact AS OC WITH (NOLOCK)
        ...
        INNER JOIN dbo.HCM_SFDC_Client AS HSC
            ON OC.contact_id = HSC.contact_id
    ...
    SELECT a.activity_id,
           ...
           ISNULL(
                     CAST(YEAR(c.updated_date) AS NVARCHAR(4)) + '-' + CAST(MONTH(c.updated_date) AS NVARCHAR(2)) + '-'
                     + CAST(DAY(c.updated_date) AS NVARCHAR(2)) + 'T'
                     + CAST(DATEPART(HOUR, c.updated_date) AS NVARCHAR(2)) + ':'
                     + CAST(DATEPART(MINUTE, c.updated_date) AS NVARCHAR(2)) + ':'
                     + CAST(DATEPART(SECOND, c.updated_date) AS NVARCHAR(2)),
                     ''
                 ) AS updated_date,
           CASE
    ...
    FROM #Calls AS c
        RIGHT JOIN #APPT AS a
            ON a.contact_id = c.contact_id
               ...
               AND c.rn = a.rn
        INNER JOIN dbo.HCM_SFDC_Client AS HSC WITH (NOLOCK)
            ON c.contact_id = HSC.contact_id
        LEFT JOIN HCM.dbo.oncd_contact_company AS OCC WITH (NOLOCK)
            ON OCC.contact_id = c.contact_id
               AND OCC.primary_flag = 'Y'
    ORDER BY OCC.company_id;
