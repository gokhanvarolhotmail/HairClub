/* CreateDate: 11/13/2017 14:40:55.500 , ModifyDate: 11/13/2017 14:40:55.500 */
GO
-- =============================================
-- Author:		Daniel Polania
-- Create date: 09/21/2017
-- Description:	Gathers all activites for existing leads
-- EXEC [dbo].[HCM_SFDC_LeadTask_Batch]
-- =============================================
CREATE PROCEDURE [dbo].[HCM_SFDC_LeadTask_Batch]
AS
BEGIN
    ...
    --Grab all contact ids and related activity ids with action code of Appoint and a activity type of null for new or existing leads
    ...
    FROM HCM.dbo.oncd_activity AS OA WITH (NOLOCK)
        ...
        INNER JOIN HCM.dbo.oncd_contact AS OC
    ...
    WHERE OAC.creation_date >= @creation_date
          ...
          AND OC.cst_sfdc_lead_id IS NOT NULL
    UNION
    SELECT OAC.contact_id,
           HSSLL.cst_sfdc_lead_id,
           OA.activity_id
    FROM dbo.HCM_SFDC_SuccessLog_Lead AS HSSLL
        INNER JOIN HCM.dbo.oncd_activity_contact AS OAC WITH (NOLOCK)
            ON HSSLL.contact_id = OAC.contact_id
        INNER JOIN HCM.dbo.oncd_activity AS OA WITH (NOLOCK)
            ON OAC.activity_id = OA.activity_id
    WHERE HSSLL.processed_date = @batch_date
          AND OA.cst_activity_type_code IS NULL
          AND OA.action_code = 'APPOINT'
          AND ISNULL(OA.cst_do_not_export, 'N') = 'N'
    ...
    FROM dbo.HCM_SFDC_SuccessLog_Lead AS HSSLL
        INNER JOIN HCM.dbo.oncd_activity_contact AS OAC WITH (NOLOCK)
            ON OAC.contact_id = HSSLL.contact_id
        INNER JOIN HCM.dbo.oncd_activity AS OA WITH (NOLOCK)
            ON OA.activity_id = OAC.activity_id
        INNER JOIN HCM.dbo.oncd_contact AS OC
            ON OC.contact_id = HSSLL.contact_id
    WHERE OA.activity_id NOT IN (
                                    SELECT IA.activity_id FROM #IgnoreActivities AS IA
                                )
          AND ISNULL(OA.cst_do_not_export, 'N') = 'N'
          AND OA.cst_sfdc_task_id IS NULL
          AND OAC.creation_date >= @creation_date
          AND HSSLL.processed_date = @batch_date;
    ...
    INSERT INTO dbo.HCM_SFDC_Batch_LeadTask
    SELECT T.activity_id,
           T.cst_sfdc_lead_id,
           CASE
