/* CreateDate: 05/03/2010 12:19:47.737 , ModifyDate: 05/03/2010 12:19:47.737 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimSalesCode_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimSalesCode_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimSalesCode_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimSalesCode]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimSalesCode] (
					  [SalesCodeSSID]
			...
			SELECT
				...
				, STG.[SalesCodeTypeSSID]
				, STG.[SalesCodeTypeDescription]
				, STG.[SalesCodeTypeDescriptionShort]
				, STG.[ProductVendorSSID]
				, STG.[ProductVendorDescription]
				, STG.[ProductVendorDescriptionShort]
				, STG.[SalesCodeDepartmentKey]
				, STG.[SalesCodeDepartmentSSID]
				, STG.[BarCode]
				, STG.[PriceDefault]
				, STG.[GLNumber]
				, STG.[ServiceDuration]
		...
		UPDATE DW SET
				...
				, DW.[SalesCodeTypeSSID] = STG.[SalesCodeTypeSSID]
				, DW.[SalesCodeTypeDescription] = STG.[SalesCodeTypeDescription]
				, DW.[SalesCodeTypeDescriptionShort] = STG.[SalesCodeTypeDescriptionShort]
				, DW.[ProductVendorSSID] = STG.[ProductVendorSSID]
				, DW.[ProductVendorDescription] = STG.[ProductVendorDescription]
				, DW.[ProductVendorDescriptionShort] = STG.[ProductVendorDescriptionShort]
				, DW.[SalesCodeDepartmentKey] = STG.[SalesCodeDepartmentKey]
				, DW.[SalesCodeDepartmentSSID] = STG.[SalesCodeDepartmentSSID]
				, DW.[BarCode] = STG.[BarCode]
				, DW.[PriceDefault] = STG.[PriceDefault]
				, DW.[GLNumber] = STG.[GLNumber]
				, DW.[ServiceDuration] = STG.[ServiceDuration]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimSalesCode] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimSalesCode] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimSalesCode]
