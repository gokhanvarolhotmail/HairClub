/* CreateDate: 05/03/2010 12:20:19.973 , ModifyDate: 10/26/2011 11:44:56.780 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DQA_FactSalesTransactionTender_0124]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_FactSalesTransactionTender_0124] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_FactSalesTransactionTender_0124]
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	SET @RuleKey = 124	--SalesOrderTenderKey Foreign Key not defined
	SET @TableName = N'[bi_cms_dqa].[FactSalesTransactionTender]'
	...
	BEGIN TRY
		...
		-- Put other clauses here
			AND ([SalesOrderTenderKey] IS NULL
			OR [SalesOrderTenderKey] = 0)
		...
		-----------------------
		EXEC	@return_value = [bi_cms_stage].[spHC_DQA_FactSalesTransactionTender_Upsert] @DataPkgKey, @RuleKey, @RuleActionKey,
