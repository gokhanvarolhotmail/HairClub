/* CreateDate: 05/03/2010 12:19:47.897 , ModifyDate: 10/30/2013 21:09:02.560 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimSalesOrderTender_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimSalesOrderTender_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimSalesOrderTender_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
--          03/21/2013  EKnapp       Delete extra rows even if not a CDC run.
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimSalesOrderTender]
		...
		DELETE FROM [bi_cms_stage].[synHC_DDS_DimSalesOrderTender]
			WHERE SalesOrderTenderKey IN
		(SELECT SalesOrderTenderKey FROM bi_cms_stage.DimSalesOrderTender WHERE IsDelete=1 AND DataPkgKey=@DataPkgKey)

		SET @DeletedRowCnt = @@ROWCOUNT

		-- if not a CDC run just delete extra rows
		DELETE  FROM [bi_cms_stage].[synHC_DDS_DimSalesOrderTender]
		WHERE [SalesOrderTenderKey] > -1
		AND [SalesOrderTenderSSID] NOT
		IN (
				SELECT SRC.SalesOrderTenderGUID
				FROM [bi_cms_stage].[synHC_SRC_TBL_CMS_datSalesOrderTender] SRC
				)
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimSalesOrderTender] (
			...
			SELECT
				...
				, STG.[Amount]
				, STG.[CheckNumber]
				, STG.[CreditCardLast4Digits]
				, STG.[ApprovalCode]
				, STG.[CreditCardTypeSSID]
				, STG.[CreditCardTypeDescription]
				, STG.[CreditCardTypeDescriptionShort]
				, STG.[FinanceCompanySSID]
				, STG.[FinanceCompanyDescription]
				, STG.[FinanceCompanyDescriptionShort]
				, STG.[InterCompanyReasonSSID]
				, STG.[InterCompanyReasonDescription]
				, STG.[InterCompanyReasonDescriptionShort]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				...
				, DW.[Amount] = STG.[Amount]
				, DW.[CheckNumber] = STG.[CheckNumber]
				, DW.[CreditCardLast4Digits] = STG.[CreditCardLast4Digits]
				, DW.[ApprovalCode] = STG.[ApprovalCode]
				, DW.[CreditCardTypeSSID] = STG.[CreditCardTypeSSID]
				, DW.[CreditCardTypeDescription] = STG.[CreditCardTypeDescription]
				, DW.[CreditCardTypeDescriptionShort] = STG.[CreditCardTypeDescriptionShort]
				, DW.[FinanceCompanySSID] = STG.[FinanceCompanySSID]
				, DW.[FinanceCompanyDescription] = STG.[FinanceCompanyDescription]
				, DW.[FinanceCompanyDescriptionShort] = STG.[FinanceCompanyDescriptionShort]
				, DW.[InterCompanyReasonSSID] = STG.[InterCompanyReasonSSID]
				, DW.[InterCompanyReasonDescription] = STG.[InterCompanyReasonDescription]
				, DW.[InterCompanyReasonDescriptionShort] = STG.[InterCompanyReasonDescriptionShort]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimSalesOrderTender] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimSalesOrderTender] DW ON
		...
		FROM [bi_cms_stage].[DimSalesOrderTender] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimSalesOrderTender] DW ON
				DW.[SalesOrderTenderKey] = STG.[SalesOrderTenderKey]
			AND DW.[RowIsCurrent] = 1
		WHERE	STG.[IsType2] = 1
			AND	STG.[IsException] = 0
			AND STG.[DataPkgKey] = @DataPkgKey
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimSalesOrderTender]
