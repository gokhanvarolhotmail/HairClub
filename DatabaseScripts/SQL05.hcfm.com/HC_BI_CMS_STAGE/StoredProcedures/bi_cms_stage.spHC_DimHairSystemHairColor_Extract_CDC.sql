/* CreateDate: 06/27/2011 17:23:01.243 , ModifyDate: 06/27/2011 17:23:01.243 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimHairSystemHairColor_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimHairSystemHairColor_Extract_CDC] is used to retrieve a
-- list HairSystemHairColor
--
--   exec [bi_cms_stage].[spHC_DimHairSystemHairColor_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
-- -------  ----------  -----------  ------------------------------------
--  v1.0    06/16/2011  EKnapp       Initial Creation
-------------------------------------------------------------------------
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpHairSystemHairColor'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN)) -- there is a logical range
					BEGIN
						...
						INSERT INTO [bi_cms_stage].[DimHairSystemHairColor]
								   ( [DataPkgKey]
									, [HairSystemHairColorKey]
						...
						SELECT @DataPkgKey
								, NULL AS [HairSystemHairColorKey]
								, chg.HairSystemHairColorID AS [HairSystemHairColorSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemHairColorDescription)),'') AS nvarchar(50)) AS [HairSystemHairColorDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemHairColorDescriptionShort)),'') AS nvarchar(10)) AS [HairSystemHairColorDescriptionShort]
								, chg.HairSystemHairColorSortOrder
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[HairSystemHairColorID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpHairSystemHairColor](@From_LSN, @To_LSN, @row_filter_option) chg
