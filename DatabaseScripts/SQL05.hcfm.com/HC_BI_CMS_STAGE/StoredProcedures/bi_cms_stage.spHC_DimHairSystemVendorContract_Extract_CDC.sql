/* CreateDate: 06/27/2011 17:23:31.793 , ModifyDate: 06/27/2011 17:23:31.793 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimHairSystemVendorContract_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimHairSystemVendorContract_Extract_CDC] is used to retrieve a
-- list HairSystemVendorContract
--
--   exec [bi_cms_stage].[spHC_DimHairSystemVendorContract_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_cfgHairSystemVendorContract'
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN)) -- there is a logical range
					BEGIN
						...
						SELECT @DataPkgKey
								...
								, RTRIM(LTRIM(ISNULL(chg.ContractName,''))) AS [HairSystemContractName]
								...
								, chg.ContractBeginDate as 'HairSystemVendorContractBeginDate'
								, chg.ContractEndDate as 'HairSystemVendorContracEndDate'
								, case when isnull(chg.IsRepair,0)=1 then 'Y' else 'N' end  as 'IsRepair'
								, case when isnull(chg.[IsActiveContract],0)=1 then 'Y' else 'N' end as 'IsActiveContract'
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[HairSystemVendorContractID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_cfgHairSystemVendorContract](@From_LSN, @To_LSN, @row_filter_option) chg
						inner join [bi_cms_stage].[synHC_SRC_TBL_CMS_cfgvendor] VN
								ON chg.VendorID = VN.VendorID
