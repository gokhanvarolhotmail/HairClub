/* CreateDate: 06/27/2011 17:23:25.600 , ModifyDate: 06/27/2011 17:23:25.600 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimHairSystemType_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimHairSystemType_Extract_CDC] is used to retrieve a
-- list HairSystemType
--
--   exec [bi_cms_stage].[spHC_DimHairSystemType_Extract_CDC]  '2011-01-01 01:00:00'
--                                       , '2011-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
-- -------  ----------  -----------  ------------------------------------
--  v1.0    06/16/2011  KMurdoch       Initial Creation
-------------------------------------------------------------------------
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpHairSystemType'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN)) -- there is a logical range
					BEGIN
						...
						INSERT INTO [bi_cms_stage].[DimHairSystemType]
								   ( [DataPkgKey]
									, [HairSystemTypeKey]
									, [HairSystemTypeSSID]
						...
						SELECT @DataPkgKey
								, NULL AS [HairSystemTypeKey]
								, chg.HairSystemID AS [HairSystemTypeSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemDescription)),'') AS nvarchar(50)) AS [HairSystemTypeDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemDescriptionShort)),'') AS nvarchar(10)) AS [HairSystemTypeDescriptionShort]
								, chg.HairSystemSortOrder
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[HairSystemID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_cfgHairSystem](@From_LSN, @To_LSN, @row_filter_option) chg
