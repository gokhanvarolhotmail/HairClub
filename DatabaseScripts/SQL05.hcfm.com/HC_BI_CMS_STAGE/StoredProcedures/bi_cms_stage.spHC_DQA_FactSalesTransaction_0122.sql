/* CreateDate: 05/03/2010 12:20:20.307 , ModifyDate: 10/26/2011 11:41:21.083 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DQA_FactSalesTransaction_0122]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_FactSalesTransaction_0122] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_FactSalesTransaction_0122]
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	SET @RuleKey = 122	--SalesOrderDetailKey Foreign Key not defined
	...
	BEGIN TRY
		...
		-- Put other clauses here
			AND ([SalesOrderDetailKey] IS NULL
			OR [SalesOrderDetailKey] = 0)
