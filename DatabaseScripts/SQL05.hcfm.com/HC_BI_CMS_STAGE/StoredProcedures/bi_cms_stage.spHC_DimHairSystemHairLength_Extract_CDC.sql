/* CreateDate: 06/27/2011 17:22:57.553 , ModifyDate: 06/27/2011 17:22:57.553 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimHairSystemHairLength_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimHairSystemHairLength_Extract_CDC] is used to retrieve a
-- list HairSystemHairLength
--
--   exec [bi_cms_stage].[spHC_DimHairSystemHairLength_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
-- -------  ----------  -----------  ------------------------------------
--  v1.0    06/17/2011  EKnapp       Initial Creation
-------------------------------------------------------------------------
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpHairSystemHairLength'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN)) -- there is a logical range
					BEGIN
						...
						INSERT INTO [bi_cms_stage].[DimHairSystemHairLength]
								   ( [DataPkgKey]
						...
						SELECT @DataPkgKey
								, NULL AS [HairSystemHairLengthKey]
								, chg.HairSystemHairLengthID AS [HairSystemHairLengthSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemHairLengthDescription)),'') AS nvarchar(50)) AS [HairSystemHairLengthDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemHairLengthDescriptionShort)),'') AS nvarchar(10)) AS [HairSystemHairLengthDescriptionShort]
								, chg.HairSystemHairLengthValue
								, chg.HairSystemHairLengthSortOrder
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[HairSystemHairLengthID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpHairSystemHairLength](@From_LSN, @To_LSN, @row_filter_option) chg
