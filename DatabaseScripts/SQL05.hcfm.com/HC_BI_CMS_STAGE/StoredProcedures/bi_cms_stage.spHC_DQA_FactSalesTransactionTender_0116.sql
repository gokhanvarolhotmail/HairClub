/* CreateDate: 05/03/2010 12:20:19.873 , ModifyDate: 10/26/2011 11:43:38.910 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DQA_FactSalesTransactionTender_0116]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_FactSalesTransactionTender_0116] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_FactSalesTransactionTender_0116]
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	SET @RuleKey = 116	--SalesOrderTypeKey Foreign Key not defined
	...
	BEGIN TRY
		...
		-- Put other clauses here
			AND ([SalesOrderTypeKey] IS NULL
			OR [SalesOrderTypeKey] = 0)
