/* CreateDate: 05/03/2010 12:20:28.297 , ModifyDate: 10/26/2011 11:32:42.337 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_FactSales_Transform_Lkup_SalesOrderKey]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [sspHC_FactSales_Transform_Lkup_SalesOrderKey] is used to determine
-- the SalesOrderKey foreign key values in the FactSales table using DimSalesOrder.
--
--
--   exec [bi_cms_stage].[spHC_FactSales_Transform_Lkup_SalesOrderKey] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------
		EXEC	@return_value = [bi_cms_stage].[spHC_FactSales_LoadInferred_DimSalesOrder] @DataPkgKey
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage = 'SalesOrderSSID is null - FSales_Trans_Lkup'
		FROM [bi_cms_stage].[FactSales] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND STG.[SalesOrderSSID] IS NULL
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage = 'SalesOrderKey is null - FSales_Trans_Lkup'
		FROM [bi_cms_stage].[FactSales] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND (STG.[SalesOrderKey] IS NULL
