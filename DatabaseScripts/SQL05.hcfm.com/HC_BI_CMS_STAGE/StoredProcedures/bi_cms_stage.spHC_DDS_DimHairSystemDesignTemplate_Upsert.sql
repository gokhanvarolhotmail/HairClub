/* CreateDate: 06/27/2011 17:23:32.253 , ModifyDate: 06/27/2011 17:23:32.253 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimHairSystemDesignTemplate_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimHairSystemDesignTemplate_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimHairSystemDesignTemplate_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemDesignTemplate]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemDesignTemplate] (
			...
			SELECT
				  STG.[HairSystemDesignTemplateSSID]
				, STG.[HairSystemDesignTemplateDescription]
				, STG.[HairSystemDesignTemplateDescriptionShort]
				, STG.[HairSystemDesignTemplateSortOrder]
				, STG.[Active]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[HairSystemDesignTemplateDescription] = STG.[HairSystemDesignTemplateDescription]
				, DW.[HairSystemDesignTemplateDescriptionShort] = STG.[HairSystemDesignTemplateDescriptionShort]
				, DW.[HairSystemDesignTemplateSortOrder] = STG.[HairSystemDesignTemplateSortOrder]
				, DW.[Active] = STG.[Active]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimHairSystemDesignTemplate] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemDesignTemplate] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemDesignTemplate]
