/* CreateDate: 10/05/2010 14:04:37.133 , ModifyDate: 10/05/2010 14:17:47.547 */
GO
CREATE PROCEDURE [bi_cms_stage].[spSTG_UnitTest_DimAccumulatorAdjustment]


AS
-------------------------------------------------------------------------
-- [spSTG_UnitTest_DimAccumulatorAdjustment] is used to Unit Test
-- DimAccumulatorAdjustment
--
--
--   exec [bi_cms_stage].[spSTG_UnitTest_DimAccumulatorAdjustment]
--
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	BEGIN TRY
		...
		EXEC	@return_value = [bi_cms_stage].[spHC_DimAccumulatorAdjustment_Extract] @DataPkgKey, @LSET, @CET

		-- Transform the data
		EXEC	@return_value = [bi_cms_stage].[spHC_DimAccumulatorAdjustment_Transform] @DataPkgKey

		-- Validate the data
		EXEC	@return_value = [bi_cms_stage].[spHC_DQA_DimAccumulatorAdjustment] @DataPkgKey

		-- Load the data into DDS
		EXEC	@return_value = [bi_cms_stage].[spHC_DDS_DimAccumulatorAdjustment_Upsert] @DataPkgKey
							 , @IgnoreRowCnt				 output
							 , @InsertRowCnt				 output
							 , @UpdateRowCnt				 output
		...
		EXEC	@return_value = [bi_cms_stage].[spHC_DimAccumulatorAdjustment_Cleanup] @DataPkgKey
