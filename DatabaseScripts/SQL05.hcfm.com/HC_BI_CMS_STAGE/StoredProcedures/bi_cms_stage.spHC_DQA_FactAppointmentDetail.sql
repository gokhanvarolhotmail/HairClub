/* CreateDate: 06/27/2011 17:23:47.963 , ModifyDate: 06/27/2011 17:23:47.963 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DQA_FactAppointmentDetail]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_FactAppointmentDetail] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_FactAppointmentDetail] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_cms_stage].[spHC_DQA_FactAppointmentDetail_0000] @DataPkgKey
			END

		-----------------------
		-- Validate Rule 109 - SalesCode Foreign Key not defined
		-----------------------
		EXEC	@return_value = [bi_cms_stage].[spHC_DQA_FactAppointmentDetail_0109] @DataPkgKey
