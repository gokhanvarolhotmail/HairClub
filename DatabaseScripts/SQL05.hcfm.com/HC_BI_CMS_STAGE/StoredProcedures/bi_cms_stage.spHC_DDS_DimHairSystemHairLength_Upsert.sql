/* CreateDate: 06/27/2011 17:23:32.140 , ModifyDate: 06/27/2011 17:23:32.140 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimHairSystemHairLength_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimHairSystemHairLength_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimHairSystemHairLength_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemHairLength]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemHairLength] (
			...
			SELECT
				  STG.[HairSystemHairLengthSSID]
				, STG.[HairSystemHairLengthDescription]
				, STG.[HairSystemHairLengthDescriptionShort]
				, STG.[HairSystemHairLengthValue]
				, STG.[HairSystemHairLengthSortOrder]
				, STG.[Active]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[HairSystemHairLengthDescription] = STG.[HairSystemHairLengthDescription]
				, DW.[HairSystemHairLengthDescriptionShort] = STG.[HairSystemHairLengthDescriptionShort]
				, DW.[HairSystemHairLengthValue] = STG.[HairSystemHairLengthValue]
				, DW.[HairSystemHairLengthSortOrder] = STG.[HairSystemHairLengthSortOrder]
				, DW.[Active] = STG.[Active]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimHairSystemHairLength] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemHairLength] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemHairLength]
