/* CreateDate: 06/27/2011 17:23:47.600 , ModifyDate: 06/27/2011 17:23:47.600 */
GO
create PROCEDURE [bi_cms_stage].[spHC_DQA_DimHairSystemRecession]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimHairSystemRecession] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_DimHairSystemRecession] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_cms_stage].[spHC_DQA_DimHairSystemRecession_0000] @DataPkgKey
			END
