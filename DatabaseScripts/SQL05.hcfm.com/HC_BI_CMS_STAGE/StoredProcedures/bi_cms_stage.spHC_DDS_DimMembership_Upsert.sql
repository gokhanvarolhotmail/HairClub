/* CreateDate: 05/03/2010 12:19:47.650 , ModifyDate: 12/04/2012 09:40:21.830 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimMembership_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimMembership_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimMembership_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
--			12/05/2012  KMurdoch	 Added BusinessSegmentDescription
-------------------------------------------------------------------------
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimMembership]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimMembership] (
					  [MembershipSSID]
			...
			SELECT
				  STG.[MembershipSSID]
				, STG.[MembershipDescription]
				, STG.[MembershipDescriptionShort]
				, STG.[BusinessSegmentKey]
				, STG.[BusinessSegmentSSID]
				, STG.[BusinessSegmentDescription]
				, STG.[BusinessSegmentDescriptionShort]
				, STG.[RevenueGroupSSID]
				, STG.[RevenueGroupDescription]
				, STG.[RevenueGroupDescriptionShort]
				, STG.[GenderSSID]
				, STG.[GenderDescription]
				, STG.[GenderDescriptionShort]
				, STG.[MembershipDurationMonths]
				, STG.[MembershipContractPrice]
				, STG.[MembershipMonthlyFee]
				, STG.[MembershipSortOrder]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[MembershipDescription] = STG.[MembershipDescription]
				, DW.[MembershipDescriptionShort] = STG.[MembershipDescriptionShort]
				, DW.[BusinessSegmentKey] = STG.[BusinessSegmentKey]
				, DW.[BusinessSegmentSSID] = STG.[BusinessSegmentSSID]
				, DW.[BusinessSegmentDescription] = STG.[BusinessSegmentDescription]
				, DW.[BusinessSegmentDescriptionShort] = STG.[BusinessSegmentDescriptionShort]
				, DW.[RevenueGroupSSID] = STG.[RevenueGroupSSID]
				, DW.[RevenueGroupDescription] = STG.[RevenueGroupDescription]
				, DW.[RevenueGroupDescriptionShort] = STG.[RevenueGroupDescriptionShort]
				, DW.[GenderSSID] = STG.[GenderSSID]
				, DW.[GenderDescription] = STG.[GenderDescription]
				, DW.[GenderDescriptionShort] = STG.[GenderDescriptionShort]
				, DW.[MembershipDurationMonths] = STG.[MembershipDurationMonths]
				, DW.[MembershipContractPrice] = STG.[MembershipContractPrice]
				, DW.[MembershipMonthlyFee] = STG.[MembershipMonthlyFee]
				, DW.[MembershipSortOrder] = STG.[MembershipSortOrder]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimMembership] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimMembership] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimMembership]
