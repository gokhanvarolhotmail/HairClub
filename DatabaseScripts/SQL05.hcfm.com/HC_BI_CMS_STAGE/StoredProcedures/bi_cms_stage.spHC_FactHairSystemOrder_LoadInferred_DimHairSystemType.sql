/* CreateDate: 06/27/2011 17:23:05.033 , ModifyDate: 06/27/2011 17:23:05.033 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_FactHairSystemOrder_LoadInferred_DimHairSystemType]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [spHC_FactHairSystemOrder_LoadInferred_DimHairSystemType] is used to load inferred
-- members to the DimHairSystemType table.
--
--
--   exec [bi_cms_stage].[spHC_FactHairSystemOrder_LoadInferred_DimHairSystemType] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	DECLARE		@DataPkgDetailKey	int
 	...
 	SET @TableName = N'[bi_ent_dds].[DimHairSystemType]'
	...
	BEGIN TRY
		...
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemType] (
			...
			SELECT
				  DISTINCT(STG.[HairSystemTypeSSID])
				, '' -- [HairSystemTypeDescription]
				, '' -- [HairSystemTypeDescriptionShort]
				, -1 -- [HairSystemTypeSortOrder]
				, 'N' -- [Active]
				, 1 -- [RowIsCurrent]
