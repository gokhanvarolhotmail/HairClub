/* CreateDate: 06/27/2011 17:23:05.203 , ModifyDate: 06/27/2011 17:23:05.203 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_FactHairSystemOrder_LoadInferred_DimHairSystemFrontalDensity]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [spHC_FactHairSystemOrder_LoadInferred_DimHairSystemFrontalDensity] is used to load inferred
-- members to the DimHairSystemFrontalDensity table.
--
--
--   exec [bi_cms_stage].[spHC_FactHairSystemOrder_LoadInferred_DimHairSystemFrontalDensity] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	DECLARE		@DataPkgDetailKey	int
 	...
 	SET @TableName = N'[bi_ent_dds].[DimHairSystemFrontalDensity]'
	...
	BEGIN TRY
		...
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemFrontalDensity] (
			...
			SELECT
				  DISTINCT(STG.[HairSystemFrontalDensitySSID])
				, '' -- [HairSystemFrontalDensityDescription]
				, '' -- [HairSystemFrontalDensityDescriptionShort]
				, -1 -- [HairSystemFrontalDensitySortOrder]
				, 'N' -- [Active]
				, 1 -- [RowIsCurrent]
