/* CreateDate: 06/27/2011 17:23:28.527 , ModifyDate: 06/27/2011 17:23:28.527 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimHairSystemOrderStatus_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimHairSystemOrderStatus_Extract_CDC] is used to retrieve a
-- list HairSystemOrderStatus
--
--   exec [bi_cms_stage].[spHC_DimHairSystemOrderStatus_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
-- -------  ----------  -----------  ------------------------------------
--  v1.0    06/21/2011  MBurrell       Initial Creation
-------------------------------------------------------------------------
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpHairSystemOrderStatus'
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN)) -- there is a logical range
					BEGIN
						...
						SELECT @DataPkgKey
								...
								, chg.HairSystemOrderStatusID AS [HairSystemOrderStatusSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemOrderStatusDescription)),'') AS nvarchar(50)) AS [HairSystemOrderStatusDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemOrderStatusDescriptionShort)),'') AS nvarchar(10)) AS [HairSystemOrderStatusDescriptionShort]
								, chg.HairSystemOrderStatusSortOrder
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[HairSystemOrderStatusID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpHairSystemOrderStatus](@From_LSN, @To_LSN, @row_filter_option) chg
