/* CreateDate: 10/05/2010 14:04:36.583 , ModifyDate: 10/05/2010 14:04:36.583 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimClientMembershipAccum_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimClientMembershipAccum_Extract_CDC] is used to retrieve a
-- list ClientMembership
--
--   exec [bi_cms_stage].[spHC_DimClientMembershipAccum_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_datClientMembershipAccum'
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						SELECT @DataPkgKey
								...
								, chg.[ClientMembershipAccumGUID] AS [ClientMembershipAccumSSID]
								...
								, COALESCE(chg.[UsedAccumQuantity],0) AS [UsedAccumQuantity]
								, COALESCE(chg.[AccumMoney],0.0) AS [AccumMoney]
								, COALESCE(chg.[AccumDate],'1/1/1753') AS [AccumDate]
								, COALESCE(chg.[TotalAccumQuantity],0) AS [TotalAccumQuantity]
								, COALESCE(chg.[AccumQuantityRemainingCalc],0.0) AS [AccumQuantityRemaining]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[ClientMembershipAccumGUID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_datClientMembershipAccum](@From_LSN, @To_LSN, @row_filter_option) chg
