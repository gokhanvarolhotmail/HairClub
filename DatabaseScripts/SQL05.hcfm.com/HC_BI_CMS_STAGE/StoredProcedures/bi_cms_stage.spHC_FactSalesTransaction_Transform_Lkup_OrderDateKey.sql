/* CreateDate: 05/03/2010 12:20:25.743 , ModifyDate: 10/26/2011 11:40:17.727 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_FactSalesTransaction_Transform_Lkup_OrderDateKey]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [spHC_FactSalesTransaction_Transform_Lkup_eadCreationDateKey] is used to determine
-- which records have late arriving dimensions and adds them
--
--
--   exec [bi_cms_stage].[spHC_FactSalesTransaction_Transform_Lkup_OrderDateKey] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage = 'OrderDate is null - FSalesTrx_Trans_Lkup'
		FROM [bi_cms_stage].[FactSalesTransaction] STG
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage = 'OrderDateKey is null - FSalesTrx_Trans_Lkup'
		FROM [bi_cms_stage].[FactSalesTransaction] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
