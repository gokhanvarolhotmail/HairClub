/* CreateDate: 06/27/2011 17:23:48.550 , ModifyDate: 11/06/2012 10:54:18.330 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DQA_FactHairSystemOrder_0022]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_FactHairSystemOrder_0022] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_FactHairSystemOrder_0022]
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------
		-- Check for General Exceptions for CenterKey & ClientHomeCenterKey
		-----------------------
		UPDATE STG SET
			RuleKey = @RuleKey
		...
		WHERE 1=1   --1=1 Rule is active  1=0 Rule is inactive
			AND DataPkgKey = @DataPkgKey
		-- Put other clauses here
			AND ([ClientHomeCenterKey] IS NULL
			OR [ClientHomeCenterKey] = 0)
