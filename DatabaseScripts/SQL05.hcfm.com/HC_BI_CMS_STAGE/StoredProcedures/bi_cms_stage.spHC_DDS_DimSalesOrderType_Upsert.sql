/* CreateDate: 05/03/2010 12:19:47.947 , ModifyDate: 05/03/2010 12:19:47.947 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimSalesOrderType_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimSalesOrderType_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimSalesOrderType_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimSalesOrderType]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimSalesOrderType] (
					  [SalesOrderTypeSSID]
			...
			SELECT
				...
				, STG.[SalesOrderTypeDescription]
				, STG.[SalesOrderTypeDescriptionShort]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[SalesOrderTypeDescription] = STG.[SalesOrderTypeDescription]
				, DW.[SalesOrderTypeDescriptionShort] = STG.[SalesOrderTypeDescriptionShort]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimSalesOrderType] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimSalesOrderType] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimSalesOrderType]
