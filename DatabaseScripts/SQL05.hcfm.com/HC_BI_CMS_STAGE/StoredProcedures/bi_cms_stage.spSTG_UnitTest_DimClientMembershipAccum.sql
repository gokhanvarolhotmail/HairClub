/* CreateDate: 10/05/2010 14:04:37.170 , ModifyDate: 10/05/2010 14:16:41.180 */
GO
CREATE PROCEDURE [bi_cms_stage].[spSTG_UnitTest_DimClientMembershipAccum]


AS
-------------------------------------------------------------------------
-- [spSTG_UnitTest_DimClientMembershipAccum] is used to Unit Test
-- DimClientMembershipAccum
--
--
--   exec [bi_cms_stage].[spSTG_UnitTest_DimClientMembershipAccum]
--
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
				...
				, @CETTimeIncrementUnit	varchar(10) = 'YEAR'
	...
	BEGIN TRY
		...
		EXEC	@return_value = [bi_cms_stage].[spHC_DimClientMembershipAccum_Extract] @DataPkgKey, @LSET, @CET

		-- Transform the data
		EXEC	@return_value = [bi_cms_stage].[spHC_DimClientMembershipAccum_Transform] @DataPkgKey

		-- Validate the data
		EXEC	@return_value = [bi_cms_stage].[spHC_DQA_DimClientMembershipAccum] @DataPkgKey

		-- Load the data into DDS
		EXEC	@return_value = [bi_cms_stage].[spHC_DDS_DimClientMembershipAccum_Upsert] @DataPkgKey
					 , @IgnoreRowCnt				 output
					 , @InsertRowCnt				 output
					 , @UpdateRowCnt				 output
		...
		EXEC	@return_value = [bi_cms_stage].[spHC_DimClientMembershipAccum_Cleanup] @DataPkgKey
