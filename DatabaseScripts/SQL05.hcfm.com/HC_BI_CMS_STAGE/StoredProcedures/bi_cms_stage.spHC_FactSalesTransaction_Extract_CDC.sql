/* CreateDate: 05/03/2010 12:20:25.237 , ModifyDate: 05/17/2021 08:19:20.010 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_FactSalesTransaction_Extract_CDC]
		@DataPkgKey				int
...
--			11/27/2012  KMurdoch	 Modified join to Client Membership to be from dat tables rather than Dim tables
--          01/03/2013  EKnapp       Check both the Sales Order and SO Detail CDC for changes since detail is only extracted when Sales Order is closed.
...
--									 Added 5037 where the SalesCodeDepartmentSSID = 5035
...
--			04/26/2016  KMurdoch	 Added 'XTDMEM1000','XTDMSO1000'
...
--			08/07/2020  KMurdoch     Added EXTFLEX & EFTFlexSol
...
--			05/17/2021  KMurdoch     Added EXT18 to query
-------------------------------------------------------------------------
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
				, @CDCTableNameSO			varchar(150)	-- Name of CDC table
				, @CDCTableNameSOD			varchar(150)	-- Name of CDC table

				, @ExtractRowCnt		int

 	SET @TableName = N'[bi_cms_dds].[FactSalesTransaction]'

 	SET @CDCTableNameSO = N'dbo_datSalesOrder'
 	SET @CDCTableNameSOD = N'dbo_datSalesOrderDetail'
	   ...
	   -- Put parameters in name value list to aid in reporting
		...
		IF (@CET IS NOT NULL) AND (@LSET IS NOT NULL)
			BEGIN
				...
				IF (@Start_Time is null) -- this would not be normal.
				  BEGIN
					SELECT @From_LSN = [HairClubCMS].[sys].[fn_cdc_get_min_lsn](@CDCTableNameSO)
				  END
				ELSE
				BEGIN
					IF ([HairClubCMS].[sys].[fn_cdc_map_lsn_to_time]([HairClubCMS].[sys].[fn_cdc_get_min_lsn](@CDCTableNameSO)) > @Start_Time) or
					   ([HairClubCMS].[sys].[fn_cdc_map_lsn_to_time]([HairClubCMS].[sys].[fn_cdc_get_min_lsn](@CDCTableNameSOD)) > @Start_Time) or
...
/*----------------------------------------------------------------------------------------------------------------------------------------*/
							...
							DECLARE @DistinctChangedSO as table(id int identity(1,1),SalesOrderGUID uniqueidentifier)
							...
							SELECT @DataPkgKey
										...
										, CASE WHEN chg.CENTERID LIKE '[1278]%'
											THEN
												...
												THEN chg.[ClientMembershipGUID]
												ELSE  ISNULL(sod.[PreviousClientMembershipGUID], chg.[ClientMembershipGUID]) END
											ELSE chg.[ClientMembershipGUID]
									   ...
									   ,	CASE WHEN dsc.[SalesCodeDepartmentSSID] IN ( 1099 ) -- Cancellations (MMCancel)
										...
										,	CASE	WHEN chg.OrderDate < '05/10/2013' THEN
													...
													WHEN chg.OrderDate >= '05/10/2013' THEN
							...
							,		ISNULL(( CASE WHEN DSC.SalesCodeDepartmentSSID IN ( 1010 )
												AND DM.BusinessSegmentSSID <> 3
												AND DM.RevenueGroupSSID = 1
												AND DM.MembershipDescriptionShort NOT IN ( 'SHOWSALE', 'SHOWNOSALE', 'SNSSURGOFF', 'RETAIL', 'HCFK', 'NONPGM', 'MODEL', 'EMPLOYEE',
																						'EMPLOYEXT', 'MODELEXT','EMPLOYEE6' )
												THEN SOD.Quantity
										...
										,	CASE WHEN chg.IsGuaranteeFlag = 1 then
											(	CASE WHEN dsc.SalesCodeDepartmentSSID IN (1010)
											...
											(	CASE WHEN dsc.SalesCodeDepartmentSSID IN (1010)
												...
												-
  												CASE WHEN dsc.SalesCodeDepartmentSSID IN (1099)
														AND (prevmem.MembershipDescriptionShort IN ('TRADITION','NALACARTE','NFOLLIGRFT','HWBRIO') or
														...
														OR
															(
																...
																AND dm.MembershipDescriptionShort NOT IN ( 'EXT6','EXT12','EXT18','EXTENH6','EXTENH12', 'EXTENH9',
																				'EXTMEM','EXTMEMSOL', 'EXTFLEX', 'EXTFLEXSOL', 'EXTINITIAL','EXTPREMMEN', 'EXTPREMWOM', 'GRAD','GRDSVEZ','GRAD12','GRDSVEZ','GRADSOL12',
																				'GRADSOL6','GRADSOL12','GRDSV12','GRDSVSOL12','GRDSV','GRDSVSOL','ELITENB','ELITENBSOL','NSILVER',
																				'NMINIPREM','NGOLD','NPLATINUM','NDIAMOND','NPREMIERE','NSUPERPREM','NULTRAPREM','NPREM24','NPREM36',
																				'NPREM48','NPREM60','NPREM72','POSTEXT' )
															)
					...
					,	CASE WHEN chg.IsGuaranteeFlag = 1 THEN(	CASE WHEN dsc.SalesCodeDepartmentSSID IN (1010)
																			--AND dm.MembershipDescriptionShort IN ('Xtrand','Xtrand6', 'Xtrand12')
								...
								,	CASE WHEN (
									...
									END AS 'NB_XtrAmt'
										...
										,	CASE WHEN (dsc.SalesCodeDepartmentSSID IN (2020)
														...
														OR
															(dsc.SalesCodeDepartmentSSID IN (2020) and
															 dm.MembershipDescriptionShort IN ('XTRANDMEM','XTDMEMSOL','XTDMEM1000','XTDMSO1000'))
														OR
									...
									END AS 'PCP_XtrAmt'
										...
										,	CASE WHEN ( dsc.SalesCodeDepartmentSSID = 3065 OR dm.MembershipDescriptionShort IN ( 'EMPLOYEE', 'EMPLOYEXT', 'EMPLOYXTR', 'EMPLOYMDP' ) ) THEN 0
										...
										,	CASE WHEN dsc.SalesCodeDepartmentSSID IN (1010, 1075, 1090, 2025)
															and dm.BusinessSegmentSSID = 3
															AND dm.MembershipSSID NOT IN (42,58,315,318)
										...
										,	CASE WHEN dsc.[SalesCodeDepartmentSSID] IN ( 1010, 1075,1090 )  -- Conversions(MMConv) Renewals (MMRenew)
													  AND dm.[MembershipSSID] in (43,44,259,260,261,262,263,264,265,266,267,268,269,270,316,317) THEN 1
												 ELSE 0
											END
											-
											CASE WHEN dsc.[SalesCodeDepartmentSSID] IN ( 1099 )  -- Cancellations (MMCancel)
										...
										, CASE WHEN (dsc.SalesCodeDepartmentSSID IN (3065) AND dm.RevenueGroupSSID = 1 AND dm.MembershipDescriptionShort IN ( 'EMPLOYEE', 'EMPLOYEXT', 'EMPLOYXTR', 'EMPLOYMDP' ) ) then sod.Quantity
										...
										, CASE WHEN (dsc.SalesCodeDepartmentSSID IN (3065) AND dm.RevenueGroupSSID = 1 AND dm.MembershipDescriptionShort IN ( 'EMPLOYEE', 'EMPLOYEXT', 'EMPLOYXTR', 'EMPLOYMDP' ) ) then sod.ExtendedPriceCalc
												 WHEN (dsc.SalesCodeDescription LIKE '%cap%' AND dsc.SalesCodeDepartmentSSID = 2020 AND dm.RevenueGroupSSID = 1 AND dm.MembershipDescriptionShort IN ( 'EMPLOYEE', 'EMPLOYEXT', 'EMPLOYXTR', 'EMPLOYMDP','EMPLOYEE6' ) )
														THEN sod.ExtendedPriceCalc
												 WHEN (dsc.SalesCodeDescription LIKE '%laser%' AND dsc.SalesCodeDepartmentSSID = 2020 AND dm.RevenueGroupSSID = 1 AND dm.MembershipDescriptionShort IN ( 'EMPLOYEE', 'EMPLOYEXT', 'EMPLOYXTR', 'EMPLOYMDP','EMPLOYEE6' ) )
														THEN sod.ExtendedPriceCalc
										...
										, CASE WHEN (dsc.SalesCodeDepartmentSSID IN (3065) AND dm.RevenueGroupSSID IN (2,3,4,5) AND dm.MembershipDescriptionShort IN ( 'EMPLOYEE', 'EMPLOYEXT', 'EMPLOYXTR', 'EMPLOYMDP' ) ) then sod.Quantity
										...
										, CASE WHEN (dsc.SalesCodeDepartmentSSID IN (3065) AND dm.RevenueGroupSSID IN (2,3,4,5) AND dm.MembershipDescriptionShort IN ( 'EMPLOYEE', 'EMPLOYEXT', 'EMPLOYXTR', 'EMPLOYMDP' ) ) then sod.ExtendedPriceCalc
												 WHEN (dsc.SalesCodeDescription LIKE '%cap%' AND dsc.SalesCodeDepartmentSSID = 2020 AND dm.RevenueGroupSSID IN (2,3,4,5) AND dm.MembershipDescriptionShort IN ( 'EMPLOYEE', 'EMPLOYEXT', 'EMPLOYXTR', 'EMPLOYMDP','EMPLOYEE6' ) )
														THEN sod.ExtendedPriceCalc
												 WHEN (dsc.SalesCodeDescription LIKE '%laser%' AND dsc.SalesCodeDepartmentSSID = 2020 AND dm.RevenueGroupSSID IN (2,3,4,5) AND dm.MembershipDescriptionShort IN ( 'EMPLOYEE', 'EMPLOYEXT', 'EMPLOYXTR', 'EMPLOYMDP','EMPLOYEE6' ) )
														THEN sod.ExtendedPriceCalc
							...
							FROM @DistinctChangedSO changedSO
								    INNER JOIN  [bi_cms_stage].[synHC_SRC_TBL_CMS_datSalesOrder] chg
											ON  changedSO.SalesOrderGUID= chg.SalesOrderGUID
									...
									INNER JOIN [bi_cms_stage].[synHC_SRC_TBL_CMS_datClientMembership] dcm
										ON chg.ClientMembershipGUID = dcm.ClientMembershipGUID
									INNER JOIN bi_cms_stage.synHC_DDS_DimMembership dm
										ON dcm.MembershipID = dm.MembershipSSID
									LEFT OUTER JOIN bi_cms_stage.synHC_DDS_DimAccumulatorAdjustment daa1
										ON sod.SalesOrderDetailGUID = daa1.SalesOrderDetailSSID
												and daa1.AccumulatorSSID = 1
									LEFT OUTER JOIN bi_cms_stage.synHC_DDS_DimAccumulatorAdjustment daa12
										ON sod.SalesOrderDetailGUID = daa12.SalesOrderDetailSSID
												and daa12.AccumulatorSSID = 12
									LEFT OUTER JOIN bi_cms_stage.synHC_DDS_DimAccumulatorAdjustment daa30
