/* CreateDate: 08/09/2012 16:38:20.183 , ModifyDate: 08/09/2012 16:38:20.183 */
GO
create PROCEDURE [bi_cms_stage].[spHC_DQA_DimActiveDirectoryGroup]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimActiveDirectoryGroup] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_DimActiveDirectoryGroup] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_cms_stage].[spHC_DQA_DimActiveDirectoryGroup_0000] @DataPkgKey
			END
