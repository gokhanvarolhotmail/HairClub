/* CreateDate: 05/03/2010 12:19:47.363 , ModifyDate: 05/03/2010 12:19:47.363 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimAccumulatorActionType_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimAccumulatorActionType_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimAccumulatorActionType_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimAccumulatorActionType]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimAccumulatorActionType] (
			...
			SELECT
				  STG.[AccumulatorActionTypeSSID]
				, STG.[AccumulatorActionTypeDescription]
				, STG.[AccumulatorActionTypeDescriptionShort]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[AccumulatorActionTypeDescription] = STG.[AccumulatorActionTypeDescription]
				, DW.[AccumulatorActionTypeDescriptionShort] = STG.[AccumulatorActionTypeDescriptionShort]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimAccumulatorActionType] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimAccumulatorActionType] DW ON
		...
		FROM [bi_cms_stage].[DimAccumulatorActionType] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimAccumulatorActionType] DW ON
				DW.[AccumulatorActionTypeKey] = STG.[AccumulatorActionTypeKey]
			AND DW.[RowIsCurrent] = 1
		WHERE	STG.[IsType2] = 1
			AND	STG.[IsException] = 0
			AND STG.[DataPkgKey] = @DataPkgKey
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimAccumulatorActionType]
