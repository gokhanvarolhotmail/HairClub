/* CreateDate: 05/03/2010 12:20:28.200 , ModifyDate: 10/26/2011 14:20:12.143 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_FactSales_Transform_Lkup_ClientMembershipKey]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [sspHC_FactSales_Transform_Lkup_ClientMembershipKey] is used to determine
-- the ClientMembershipKey foreign key values in the FactSales table using DimClientMembership.
--
--
--   exec [bi_cms_stage].[spHC_FactSales_Transform_Lkup_ClientMembershipKey] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------
		EXEC	@return_value = [bi_cms_stage].[spHC_FactSales_LoadInferred_DimClientMembership] @DataPkgKey
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage = 'ClientMembershipSSID is null - FSales_Trans_Lkup'
		FROM [bi_cms_stage].[FactSales] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND STG.[ClientMembershipSSID] IS NULL
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage = 'ClientMembershipKey is null - FSales_Trans_Lkup'
		FROM [bi_cms_stage].[FactSales] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND (STG.[ClientMembershipKey] IS NULL
