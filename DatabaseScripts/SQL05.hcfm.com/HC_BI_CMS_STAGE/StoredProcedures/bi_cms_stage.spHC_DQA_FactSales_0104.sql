/* CreateDate: 05/03/2010 12:20:20.457 , ModifyDate: 10/26/2011 11:30:45.240 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DQA_FactSales_0104]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_FactSales_0104] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_FactSales_0104]
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	SET @TableName = N'[bi_cms_dqa].[FactSales]'
	...
	BEGIN TRY
		...
		-----------------------
		EXEC	@return_value = [bi_cms_stage].[spHC_DQA_FactSales_Upsert] @DataPkgKey, @RuleKey, @RuleActionKey,
