/* CreateDate: 06/27/2011 17:23:48.007 , ModifyDate: 06/27/2011 17:23:48.007 */
GO
create PROCEDURE [bi_cms_stage].[spHC_DQA_FactSurgeryCloseoutEmployee]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_FactSurgeryCloseoutEmployee] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_FactSurgeryCloseoutEmployee] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_cms_stage].[spHC_DQA_FactSurgeryCloseoutEmployee_0000] @DataPkgKey
			END

		-----------------------
		-- Validate Rule 106 - Employee Foreign Key not defined
		-----------------------
		EXEC	@return_value = [bi_cms_stage].[spHC_DQA_FactSurgeryCloseoutEmployee_0106] @DataPkgKey
