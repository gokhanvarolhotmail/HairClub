/* CreateDate: 06/27/2011 17:23:08.490 , ModifyDate: 06/27/2011 17:23:08.490 */
GO
create PROCEDURE [bi_cms_stage].[spHC_FactAppointmentEmployee_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_FactAppointmentEmployee_Extract_CDC] is used to retrieve a
-- list Appointment
--
--   exec [bi_cms_stage].[spHC_FactAppointmentEmployee_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						UPDATE [bief_stage].[_DataFlow]
							...
							INSERT INTO [bi_cms_stage].[FactAppointmentEmployee]
									   ( [DataPkgKey]
							...
							SELECT @DataPkgKey
										...
										, COALESCE(chg.[AppointmentEmployeeGUID], CONVERT(UNIQUEIDENTIFIER,'00000000-0000-0000-0000-000000000002')) AS [AppointmentEmployeeSSID]
										...
										, 0 AS [IsDelete]
										, 0 AS [IsDuplicate]
										, CAST(ISNULL(LTRIM(RTRIM(chg.AppointmentEmployeeGUID)),'') AS nvarchar(50)) AS [SourceSystemKey]
							FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_datAppointmentEmployee](@From_LSN, @To_LSN, @row_filter_option) chg
