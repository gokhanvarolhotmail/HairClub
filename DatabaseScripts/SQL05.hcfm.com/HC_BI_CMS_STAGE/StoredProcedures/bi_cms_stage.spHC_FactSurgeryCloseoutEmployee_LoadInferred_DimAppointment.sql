/* CreateDate: 06/27/2011 17:30:08.920 , ModifyDate: 12/10/2012 14:16:07.330 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_FactSurgeryCloseoutEmployee_LoadInferred_DimAppointment]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [spHC_FactSurgeryCloseoutEmployee_LoadInferred_DimAppointment] is used to load inferred
-- members to the DimAppointment table.
--
--
--   exec [bi_cms_stage].[spHC_FactSurgeryCloseoutEmployee_LoadInferred_DimAppointment] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimAppointment] (
			...
			SELECT DISTINCT
				...
				, -1
				, ''
				, CAST('1753-01-01 00:00:00' AS Time)
				, CAST('1753-01-01 00:00:00' AS Time)
