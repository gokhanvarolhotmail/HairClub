/* CreateDate: 05/03/2010 12:20:05.450 , ModifyDate: 03/11/2011 14:31:36.060 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimSalesCodeDivision_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimSalesCodeDivision_Extract_CDC] is used to retrieve a
-- list SalesCodeDivision
--
--   exec [bi_cms_stage].[spHC_DimSalesCodeDivision_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpSalesCodeDivision'
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						SELECT @DataPkgKey
								...
								, CAST(ISNULL(LTRIM(RTRIM(chg.[SalesCodeDivisionDescription])),'') AS nvarchar(50)) AS [SalesCodeDivisionDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[SalesCodeDivisionDescriptionShort])),'') AS nvarchar(10)) AS [SalesCodeDivisionDescriptionShort]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[SalesCodeDivisionID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpSalesCodeDivision](@From_LSN, @To_LSN, @row_filter_option) chg
