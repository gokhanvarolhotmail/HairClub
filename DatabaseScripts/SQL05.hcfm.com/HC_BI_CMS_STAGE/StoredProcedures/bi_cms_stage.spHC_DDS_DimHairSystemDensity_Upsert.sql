/* CreateDate: 06/27/2011 17:23:32.287 , ModifyDate: 06/27/2011 17:23:32.287 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimHairSystemDensity_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimHairSystemDensity_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimHairSystemDensity_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemDensity] (
			...
			SELECT
				  STG.[HairSystemDensitySSID]
				, STG.[HairSystemDensityDescription]
				, STG.[HairSystemDensityDescriptionShort]
				, STG.[HairSystemDensitySortOrder]
				, STG.[Active]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[HairSystemDensityDescription] = STG.[HairSystemDensityDescription]
				, DW.[HairSystemDensityDescriptionShort] = STG.[HairSystemDensityDescriptionShort]
				, DW.[HairSystemDensitySortOrder] = STG.[HairSystemDensitySortOrder]
				, DW.[Active] = STG.[Active]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimHairSystemDensity] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemDensity] DW ON
