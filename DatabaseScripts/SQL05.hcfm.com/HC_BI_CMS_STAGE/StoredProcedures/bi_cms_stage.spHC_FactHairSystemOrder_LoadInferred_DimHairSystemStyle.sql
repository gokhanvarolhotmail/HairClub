/* CreateDate: 06/27/2011 17:23:05.093 , ModifyDate: 06/27/2011 17:23:05.093 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_FactHairSystemOrder_LoadInferred_DimHairSystemStyle]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [spHC_FactHairSystemOrder_LoadInferred_DimHairSystemStyle] is used to load inferred
-- members to the DimHairSystemStyle table.
--
--
--   exec [bi_cms_stage].[spHC_FactHairSystemOrder_LoadInferred_DimHairSystemStyle] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	DECLARE		@DataPkgDetailKey	int
 	...
 	SET @TableName = N'[bi_ent_dds].[DimHairSystemStyle]'
	...
	BEGIN TRY
		...
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemStyle] (
			...
			SELECT
				  DISTINCT(STG.[HairSystemStyleSSID])
				, '' -- [HairSystemStyleDescription]
				, '' -- [HairSystemStyleDescriptionShort]
				, -1 -- [HairSystemStyleSortOrder]
				, 'N' -- [Active]
				, 1 -- [RowIsCurrent]
