/* CreateDate: 06/27/2011 17:23:05.060 , ModifyDate: 06/27/2011 17:23:05.060 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_FactHairSystemOrder_LoadInferred_DimHairSystemTexture]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [spHC_FactHairSystemOrder_LoadInferred_DimHairSystemTexture] is used to load inferred
-- members to the DimHairSystemTexture table.
--
--
--   exec [bi_cms_stage].[spHC_FactHairSystemOrder_LoadInferred_DimHairSystemTexture] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	DECLARE		@DataPkgDetailKey	int
 	...
 	SET @TableName = N'[bi_ent_dds].[DimHairSystemTexture]'
	...
	BEGIN TRY
		...
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemTexture] (
			...
			SELECT
				  DISTINCT(STG.[HairSystemTextureSSID])
				, '' -- [HairSystemTextureDescription]
				, '' -- [HairSystemTextureDescriptionShort]
				, -1 --[HairSystemTextureSortOrder]
				, 'N' -- [Active]
				, 1 -- [RowIsCurrent]
