/* CreateDate: 06/27/2011 17:23:04.933 , ModifyDate: 06/27/2011 17:23:04.933 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_FactHairSystemOrder_LoadInferred_DimEmployee]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [spHC_FactHairSystemOrder_LoadInferred_DimEmployee] is used to load inferred
-- members to the DimEmployee table.
--
--
--   exec [bi_cms_stage].[spHC_FactHairSystemOrder_LoadInferred_DimEmployee] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	DECLARE		@DataPkgDetailKey	int
 	...
 	SET @TableName = N'[bi_ent_dds].[DimEmployee]'
	...
	BEGIN TRY
		...
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimEmployee] (
			...
			SELECT
				  DISTINCT(STG.[MeasurementsByEmployeeSSID])
			    , '' -- [EmployeeFirstName]
			    , '' -- [EmployeeLastName]
			    , '' -- [EmployeeInitials]
			    , 0 -- [SalutationSSID]
