/* CreateDate: 06/27/2011 17:23:31.960 , ModifyDate: 06/27/2011 17:23:31.960 */
GO
create PROCEDURE [bi_cms_stage].[spHC_DDS_DimHairSystemTexture_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimHairSystemTexture_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimHairSystemTexture_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemTexture]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemTexture] (
			...
			SELECT
				  STG.[HairSystemTextureSSID]
				, STG.[HairSystemTextureDescription]
				, STG.[HairSystemTextureDescriptionShort]
				, STG.[HairSystemTextureSortOrder]
				, STG.[Active]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[HairSystemTextureDescription] = STG.[HairSystemTextureDescription]
				, DW.[HairSystemTextureDescriptionShort] = STG.[HairSystemTextureDescriptionShort]
				, DW.[HairSystemTextureSortOrder] = STG.[HairSystemTextureSortOrder]
				, DW.[Active] = STG.[Active]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimHairSystemTexture] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemTexture] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemTexture]
