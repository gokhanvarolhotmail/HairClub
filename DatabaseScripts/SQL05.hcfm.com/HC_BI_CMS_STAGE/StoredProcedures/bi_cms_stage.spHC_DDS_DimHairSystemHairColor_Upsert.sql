/* CreateDate: 06/27/2011 17:23:32.180 , ModifyDate: 06/27/2011 17:23:32.180 */
GO
create PROCEDURE [bi_cms_stage].[spHC_DDS_DimHairSystemHairColor_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimHairSystemHairColor_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimHairSystemHairColor_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemHairColor]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemHairColor] (
			...
			SELECT
				  STG.[HairSystemHairColorSSID]
				, STG.[HairSystemHairColorDescription]
				, STG.[HairSystemHairColorDescriptionShort]
				, STG.[HairSystemHairColorSortOrder]
				, STG.[Active]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[HairSystemHairColorDescription] = STG.[HairSystemHairColorDescription]
				, DW.[HairSystemHairColorDescriptionShort] = STG.[HairSystemHairColorDescriptionShort]
				, DW.[HairSystemHairColorSortOrder] = STG.[HairSystemHairColorSortOrder]
				, DW.[Active] = STG.[Active]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimHairSystemHairColor] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemHairColor] DW ON
		...
		FROM [bi_cms_stage].[DimHairSystemHairColor] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemHairColor] DW ON
				DW.[HairSystemHairColorKey] = STG.[HairSystemHairColorKey]
			AND DW.[RowIsCurrent] = 1
		WHERE	STG.[IsType2] = 1
			AND	STG.[IsException] = 0
			AND STG.[DataPkgKey] = @DataPkgKey
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemHairColor]
