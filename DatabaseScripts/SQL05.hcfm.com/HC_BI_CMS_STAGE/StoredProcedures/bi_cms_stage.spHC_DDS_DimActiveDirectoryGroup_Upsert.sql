/* CreateDate: 08/09/2012 16:44:11.350 , ModifyDate: 08/09/2012 16:44:11.350 */
GO
create PROCEDURE [bi_cms_stage].[spHC_DDS_DimActiveDirectoryGroup_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimActiveDirectoryGroup_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimActiveDirectoryGroup_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimActiveDirectoryGroup]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimActiveDirectoryGroup] (
		...
		FROM [bi_cms_stage].[DimActiveDirectoryGroup] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimActiveDirectoryGroup] DW ON
		...
		FROM [bi_cms_stage].[DimActiveDirectoryGroup] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimActiveDirectoryGroup] DW ON
				DW.[ActiveDirectoryGroupKey] = STG.[ActiveDirectoryGroupKey]
			AND DW.[RowIsCurrent] = 1
		WHERE	STG.[IsType2] = 1
			AND	STG.[IsException] = 0
			AND STG.[DataPkgKey] = @DataPkgKey
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimActiveDirectoryGroup]
