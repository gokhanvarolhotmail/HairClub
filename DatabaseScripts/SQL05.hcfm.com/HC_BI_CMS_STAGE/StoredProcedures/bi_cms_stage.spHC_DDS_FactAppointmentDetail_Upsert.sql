/* CreateDate: 06/27/2011 17:23:12.330 , ModifyDate: 03/17/2014 19:46:57.280 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_FactAppointmentDetail_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_FactAppointmentDetail_Upsert] is used to determine
-- which records have late arriving dimensions and adds them
--
--
--   exec [bi_cms_stage].[spHC_DDS_FactAppointmentDetail_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
--			07/15/2011  KMurdoch	 Added AppointmentDetailSSID
--			03/22/2013	EKnapp		 Delete Extra Records
-------------------------------------------------------------------------
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_FactAppointmentDetail]
		...
		--DELETE FROM [bi_cms_stage].[synHC_DDS_FactAppointmentDetail]
		--WHERE [AppointmentDetailSSID] NOT IN
		--(SELECT SRC.[AppointmentDetailGUID]
		--		FROM  bi_cms_stage.synHC_SRC_TBL_CMS_datAppointmentDetail SRC WITH (NOLOCK))

		SET @DeletedRowCnt =  @@ROWCOUNT


		--New Rows
		INSERT INTO [bi_cms_stage].[synHC_DDS_FactAppointmentDetail](
					  [AppointmentKey]
			...
			SELECT DISTINCT
					  STG.[AppointmentKey]
					, STG.[AppointmentDetailSSID]
					, STG.[SalesCodeKey]
					, STG.[AppointmentDetailDuration]
					, @DataPkgKey
					, -2 -- 'Not Updated Yet'
			FROM [bi_cms_stage].[FactAppointmentDetail] STG
			WHERE	STG.[IsNew] = 1
		...
		UPDATE DW SET
			...
			, DW.[AppointmentDetailDuration] = STG.[AppointmentDetailDuration]
			, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[FactAppointmentDetail] STG
		JOIN [bi_cms_stage].[synHC_DDS_FactAppointmentDetail] DW WITH (NOLOCK)
			ON DW.[AppointmentDetailKey] = STG.[AppointmentDetailKey]
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_FactAppointmentDetail]
