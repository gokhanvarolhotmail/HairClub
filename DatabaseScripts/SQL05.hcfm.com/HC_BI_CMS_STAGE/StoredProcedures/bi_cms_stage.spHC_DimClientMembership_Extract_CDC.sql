/* CreateDate: 05/03/2010 12:19:54.600 , ModifyDate: 02/12/2019 10:37:20.473 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimClientMembership_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimClientMembership_Extract_CDC] is used to retrieve a
-- list ClientMembership
--
--   exec [bi_cms_stage].[spHC_DimClientMembership_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_datClientMembership'
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						SELECT @DataPkgKey
								...
								, chg.[ClientMembershipGUID] AS [ClientMembershipSSID]
								, chg.[Member1_ID_Temp] AS [Member1_ID_Temp]
								, NULL AS [ClientKey]
								, chg.[ClientGUID] AS [ClientSSID]
								, NULL AS [CenterKey]
								, chg.[CenterID] AS [CenterSSID]
								, NULL AS [MembershipKey]
								, chg.[MembershipID] AS [MembershipSSID]
								, chg.[ClientMembershipStatusID] AS [ClientMembershipStatusSSID]
								...
								, COALESCE(chg.[ContractPrice],0.0) AS [ClientMembershipContractPrice]
								, COALESCE(chg.[ContractPaidAmount],0.0) AS [ClientMembershipContractPaidAmount]
								, COALESCE(chg.[MonthlyFee],0.0) AS [ClientMembershipMonthlyFee]
								, COALESCE(chg.[BeginDate],'1/1/1753') AS [ClientMembershipBeginDate]
								, COALESCE(chg.[EndDate],'12/31/9999') AS [ClientMembershipEndDate]
								, COALESCE(chg.[CancelDate],'12/31/9999') AS [ClientMembershipCancelDate]
								, chg.[ClientMembershipIdentifier]
								, chg.[NationalMonthlyFee]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[ClientMembershipGUID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_datClientMembership](@From_LSN, @To_LSN, @row_filter_option) chg
							Left Outer JOIN [bi_cms_stage].[synHC_SRC_TBL_CMS_lkpClientMembershipStatus] clms
									ON chg.[ClientMembershipStatusID] = clms.[ClientMembershipStatusID]
