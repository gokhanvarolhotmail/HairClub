/* CreateDate: 06/27/2011 17:23:12.370 , ModifyDate: 06/27/2011 17:23:12.370 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimHairSystemVendorContract_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimHairSystemVendorContract_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimHairSystemVendorContract_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
-- -------  ----------  -----------  ------------------------------------
--  v1.0    06/15/2011  EKnapp       Initial Creation
-------------------------------------------------------------------------
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemVendorContract]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemVendorContract] (
			...
			SELECT
				  STG.[HairSystemVendorContractSSID]
				, STG.[HairSystemVendorContractName]
				, STG.[HairSystemVendorDescription]
				, STG.[HairSystemVendorDescriptionShort]
				, STG.[HairSystemVendorContractBeginDate]
				, STG.[HairSystemVendorContractEndDate]
				, STG.[IsRepair]
				, STG.[IsActiveContract]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[HairSystemVendorContractName] = STG.[HairSystemVendorContractName]
				, DW.[HairSystemVendorDescription] = STG.[HairSystemVendorDescription]
				, DW.[HairSystemVendorDescriptionShort] = STG.[HairSystemVendorDescriptionShort]
				, DW.[HairSystemVendorContractBeginDate] = STG.[HairSystemVendorContractBeginDate]
				, DW.[HairSystemVendorContractEndDate] = STG.[HairSystemVendorContractEndDate]
				, DW.[IsRepair] = STG.[IsRepair]
				, DW.[IsActiveContract] = STG.[IsActiveContract]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimHairSystemVendorContract] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemVendorContract] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemVendorContract]
