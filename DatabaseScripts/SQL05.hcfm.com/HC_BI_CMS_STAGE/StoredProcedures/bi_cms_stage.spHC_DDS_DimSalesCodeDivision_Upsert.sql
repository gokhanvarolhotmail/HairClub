/* CreateDate: 05/03/2010 12:19:47.710 , ModifyDate: 05/03/2010 12:19:47.710 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimSalesCodeDivision_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimSalesCodeDivision_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimSalesCodeDivision_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimSalesCodeDivision]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimSalesCodeDivision] (
			...
			SELECT
				...
				, STG.[SalesCodeDivisionDescription]
				, STG.[SalesCodeDivisionDescriptionShort]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[SalesCodeDivisionDescription] = STG.[SalesCodeDivisionDescription]
				, DW.[SalesCodeDivisionDescriptionShort] = STG.[SalesCodeDivisionDescriptionShort]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimSalesCodeDivision] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimSalesCodeDivision] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimSalesCodeDivision]
