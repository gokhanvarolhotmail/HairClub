/* CreateDate: 06/27/2011 17:23:22.653 , ModifyDate: 06/27/2011 17:23:22.653 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimHairSystemDensity_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimHairSystemDensity_Extract_CDC] is used to retrieve a
-- list HairSystemDensity
--
--   exec [bi_cms_stage].[spHC_DimHairSystemDensity_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpHairSystemDensity'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN)) -- there is a logical range
					BEGIN
						...
						INSERT INTO [bi_cms_stage].[DimHairSystemDensity]
								   ( [DataPkgKey]
						...
						SELECT @DataPkgKey
								, NULL AS [HairSystemDensityKey]
								, chg.HairSystemDensityID AS [HairSystemDensitySSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemDensityDescription)),'') AS nvarchar(50)) AS [HairSystemDensityDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemDensityDescriptionShort)),'') AS nvarchar(10)) AS [HairSystemDensityDescriptionShort]
								, chg.HairSystemDensitySortOrder
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[HairSystemDensityID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpHairSystemDensity](@From_LSN, @To_LSN, @row_filter_option) chg
