/* CreateDate: 06/27/2011 17:23:32.070 , ModifyDate: 06/27/2011 17:23:32.070 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimHairSystemOrderStatus_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimHairSystemOrderStatus_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimHairSystemOrderStatus_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemOrderStatus]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemOrderStatus] (
			...
			SELECT
				  STG.[HairSystemOrderStatusSSID]
				, STG.[HairSystemOrderStatusDescription]
				, STG.[HairSystemOrderStatusDescriptionShort]
				, STG.[HairSystemOrderStatusSortOrder]
				, STG.[CanApplyFlag]
				, STG.[CanTransferFlag]
				, STG.[CanEditFlag]
				, STG.[IsActiveFlag]
				, STG.[CanCancelFlag]
				, STG.[IsPreallocationFlag]
				, STG.[CanRedoFlag]
				, STG.[CanRepairFlag]
				, STG.[ShowInHistoryFlag]
				, STG.[CanAddToStockFlag]
				, STG.[IncludeInMembershipCountFlag]
				, STG.[CanRequestCreditFlag]
				, STG.[Active]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[HairSystemOrderStatusDescription] = STG.[HairSystemOrderStatusDescription]
				, DW.[HairSystemOrderStatusDescriptionShort] = STG.[HairSystemOrderStatusDescriptionShort]
				, DW.[HairSystemOrderStatusSortOrder] = STG.[HairSystemOrderStatusSortOrder]
				, DW.[CanApplyFlag] = STG.[CanApplyFlag]
				, DW.[CanTransferFlag] = STG.[CanTransferFlag]
				, DW.[CanEditFlag] = STG.[CanEditFlag]
				, DW.[IsActiveFlag] = STG.[IsActiveFlag]
				, DW.[CanCancelFlag] = STG.[CanCancelFlag]
				, DW.[IsPreallocationFlag] = STG.[IsPreallocationFlag]
				, DW.[CanRedoFlag] = STG.[CanRedoFlag]
				, DW.[CanRepairFlag] = STG.[CanRepairFlag]
				, DW.[ShowInHistoryFlag] = STG.[ShowInHistoryFlag]
				, DW.[CanAddToStockFlag] = STG.[CanAddToStockFlag]
				, DW.[IncludeInMembershipCountFlag] = STG.[IncludeInMembershipCountFlag]
				, DW.[CanRequestCreditFlag] = STG.[CanRequestCreditFlag]
				, DW.[Active] = STG.[Active]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimHairSystemOrderStatus] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemOrderStatus] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemOrderStatus]
