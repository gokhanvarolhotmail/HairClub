/* CreateDate: 08/09/2012 16:48:24.837 , ModifyDate: 08/09/2012 16:48:24.837 */
GO
create PROCEDURE [bi_cms_stage].[spHC_DimSecurityElement_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimSecurityElement_Extract_CDC] is used to retrieve a
-- list SecurityElement
--
--   exec [bi_cms_stage].[spHC_DimSecurityElement_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpSecurityElement'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN)) -- there is a logical range
					BEGIN
						...
						INSERT INTO [bi_cms_stage].[DimSecurityElement]
								   ( [DataPkgKey]
									, [SecurityElementKey]
									, [SecurityElementSSID]
						...
						SELECT @DataPkgKey
								, NULL AS [SecurityElementKey]
								, chg.SecurityElementID AS [SecurityElementSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.SecurityElementDescription)),'') AS nvarchar(50)) AS [SecurityElementDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.SecurityElementDescriptionShort)),'') AS nvarchar(10)) AS [SecurityElementDescriptionShort]
								, chg.SecurityElementSortOrder
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[SecurityElementID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpSecurityElement](@From_LSN, @To_LSN, @row_filter_option) chg
