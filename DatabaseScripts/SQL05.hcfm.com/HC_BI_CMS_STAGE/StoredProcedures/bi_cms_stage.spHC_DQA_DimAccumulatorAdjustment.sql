/* CreateDate: 10/05/2010 14:04:36.830 , ModifyDate: 10/05/2010 14:04:36.830 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DQA_DimAccumulatorAdjustment]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimAccumulatorAdjustment] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_DimAccumulatorAdjustment] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_cms_stage].[spHC_DQA_DimAccumulatorAdjustment_0000] @DataPkgKey
			END
