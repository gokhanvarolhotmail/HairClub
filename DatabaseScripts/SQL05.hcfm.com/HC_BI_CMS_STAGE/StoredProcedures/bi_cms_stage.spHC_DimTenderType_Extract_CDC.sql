/* CreateDate: 05/03/2010 12:20:18.667 , ModifyDate: 05/03/2010 12:20:18.667 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimTenderType_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimTenderType_Extract_CDC] is used to retrieve a
-- list TenderType
--
--   exec [bi_cms_stage].[spHC_DimTenderType_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpTenderType'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						INSERT INTO [bi_cms_stage].[DimTenderType]
								   ( [DataPkgKey]
								   , [TenderTypeKey]
								   , [TenderTypeSSID]
						...
						SELECT @DataPkgKey
								, NULL AS [TenderTypeKey]
								, chg.[TenderTypeID] AS [TenderTypeSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[TenderTypeDescription])),'') AS nvarchar(50)) AS [TenderTypeDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[TenderTypeDescriptionShort])),'') AS nvarchar(10)) AS [TenderTypeDescriptionShort]
								, COALESCE(chg.[MaxOccurrences], 0) AS [MaxOccurrences]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[TenderTypeID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpTenderType](@From_LSN, @To_LSN, @row_filter_option) chg
