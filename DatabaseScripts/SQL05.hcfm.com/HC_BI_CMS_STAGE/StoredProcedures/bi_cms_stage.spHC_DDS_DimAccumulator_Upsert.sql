/* CreateDate: 05/03/2010 12:19:47.410 , ModifyDate: 05/03/2010 12:19:47.410 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimAccumulator_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimAccumulator_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimAccumulator_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimAccumulator]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimAccumulator] (
					  [AccumulatorSSID]
			...
			SELECT
				  STG.[AccumulatorSSID]
				, STG.[AccumulatorDescription]
				, STG.[AccumulatorDescriptionShort]
				, STG.[AccumulatorDataTypeSSID]
				, STG.[AccumulatorDataTypeDescription]
				, STG.[AccumulatorDataTypeDescriptionShort]
				, STG.[SchedulerActionTypeSSID]
				, STG.[SchedulerActionTypeDescription]
				, STG.[SchedulerActionTypeDescriptionShort]
				, STG.[SchedulerAdjustmentTypeSSID]
				, STG.[SchedulerAdjustmentTypeDescription]
				, STG.[SchedulerAdjustmentTypeDescriptionShort]
		...
		UPDATE DW SET
				  DW.[AccumulatorDescription] = STG.[AccumulatorDescription]
				, DW.[AccumulatorDescriptionShort] = STG.[AccumulatorDescriptionShort]
				, DW.[AccumulatorDataTypeSSID] = STG.[AccumulatorDataTypeSSID]
				, DW.[AccumulatorDataTypeDescription] = STG.[AccumulatorDataTypeDescription]
				, DW.[AccumulatorDataTypeDescriptionShort] = STG.[AccumulatorDataTypeDescriptionShort]
				, DW.[SchedulerActionTypeSSID] = STG.[SchedulerActionTypeSSID]
				, DW.[SchedulerActionTypeDescription] = STG.[SchedulerActionTypeDescription]
				, DW.[SchedulerActionTypeDescriptionShort] = STG.[SchedulerActionTypeDescriptionShort]
				, DW.[SchedulerAdjustmentTypeSSID] = STG.[SchedulerAdjustmentTypeSSID]
				, DW.[SchedulerAdjustmentTypeDescription] = STG.[SchedulerAdjustmentTypeDescription]
				, DW.[SchedulerAdjustmentTypeDescriptionShort] = STG.[SchedulerAdjustmentTypeDescriptionShort]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimAccumulator] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimAccumulator] DW ON
		...
		FROM [bi_cms_stage].[DimAccumulator] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimAccumulator] DW ON
				DW.[AccumulatorKey] = STG.[AccumulatorKey]
			AND DW.[RowIsCurrent] = 1
		WHERE	STG.[IsType2] = 1
			AND	STG.[IsException] = 0
			AND STG.[DataPkgKey] = @DataPkgKey
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimAccumulator]
