/* CreateDate: 06/27/2011 17:23:31.993 , ModifyDate: 06/27/2011 17:23:31.993 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimHairSystemStyle_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimHairSystemStyle_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimHairSystemStyle_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemStyle]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemStyle] (
					  [HairSystemStyleSSID]
			...
			SELECT
				  STG.[HairSystemStyleSSID]
				, STG.[HairSystemStyleDescription]
				, STG.[HairSystemStyleDescriptionShort]
				, STG.[HairSystemStyleSortOrder]
				, STG.[Active]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[HairSystemStyleDescription] = STG.[HairSystemStyleDescription]
				, DW.[HairSystemStyleDescriptionShort] = STG.[HairSystemStyleDescriptionShort]
				, DW.[HairSystemStyleSortOrder] = STG.[HairSystemStyleSortOrder]
				, DW.[Active]= STG.[Active]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimHairSystemStyle] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemStyle] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemStyle]
