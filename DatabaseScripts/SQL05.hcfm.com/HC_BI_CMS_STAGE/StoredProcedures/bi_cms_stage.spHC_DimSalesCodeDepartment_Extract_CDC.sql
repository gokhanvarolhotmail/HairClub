/* CreateDate: 05/03/2010 12:20:03.190 , ModifyDate: 05/03/2010 12:20:03.190 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimSalesCodeDepartment_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimSalesCodeDepartment_Extract_CDC] is used to retrieve a
-- list Business Unit
--
--   exec [bi_cms_stage].[spHC_DimSalesCodeDepartment_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpSalesCodeDepartment'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						INSERT INTO [bi_cms_stage].[DimSalesCodeDepartment]
								   ( [DataPkgKey]
						...
						SELECT @DataPkgKey
								...
								, chg.[SalesCodeDepartmentID] AS [SalesCodeDepartmentSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[SalesCodeDepartmentDescription])),'') AS nvarchar(50)) AS [SalesCodeDepartmentDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[SalesCodeDepartmentDescriptionShort])),'') AS nvarchar(10)) AS [SalesCodeDepartmentDescriptionShort]
								, NULL AS [SalesCodeDivisionKey]
								, chg.[SalesCodeDivisionID] AS [SalesCodeDivisionSSID]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[SalesCodeDepartmentID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpSalesCodeDepartment](@From_LSN, @To_LSN, @row_filter_option) chg
