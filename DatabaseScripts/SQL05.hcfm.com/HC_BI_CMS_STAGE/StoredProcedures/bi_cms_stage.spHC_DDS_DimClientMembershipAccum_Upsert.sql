/* CreateDate: 10/05/2010 14:04:30.970 , ModifyDate: 03/21/2013 17:21:34.390 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimClientMembershipAccum_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimClientMembershipAccum_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimClientMembershipAccum_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimClientMembershipAccum]
		...
		DELETE FROM [bi_cms_stage].[synHC_DDS_DimClientMembershipAccum]
		WHERE [ClientMembershipAccumKey] > -1
		AND [ClientMembershipAccumSSID] NOT
		IN (
				SELECT SRC.[ClientMembershipAccumGUID]
				FROM [bi_cms_stage].[synHC_SRC_TBL_CMS_datClientMembershipAccum] SRC WITH (NOLOCK)
				)
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimClientMembershipAccum] (
			...
			SELECT
					...
					, STG.[UsedAccumQuantity]
					, STG.[AccumMoney]
					, STG.[AccumDate]
					, STG.[TotalAccumQuantity]
					, STG.[AccumQuantityRemaining]
		...
		UPDATE DW SET
					...
					, DW.[UsedAccumQuantity] = STG.[UsedAccumQuantity]
					, DW.[AccumMoney] = STG.[AccumMoney]
					, DW.[AccumDate] = STG.[AccumDate]
					, DW.[TotalAccumQuantity] = STG.[TotalAccumQuantity]
					, DW.[AccumQuantityRemaining] = STG.[AccumQuantityRemaining]

				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimClientMembershipAccum] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimClientMembershipAccum] DW ON
				DW.[ClientMembershipSSID] = STG.[ClientMembershipSSID]
		...
		FROM [bi_cms_stage].[DimClientMembershipAccum] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimClientMembershipAccum] DW ON
				DW.[ClientMembershipAccumKey] = STG.[ClientMembershipAccumKey]
			AND DW.[RowIsCurrent] = 1
		WHERE	STG.[IsType2] = 1
			AND	STG.[IsException] = 0
			AND STG.[DataPkgKey] = @DataPkgKey
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimClientMembershipAccum]
