/* CreateDate: 06/27/2011 17:23:32.103 , ModifyDate: 06/27/2011 17:23:32.103 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimHairSystemMatrixColor_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimHairSystemMatrixColor_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimHairSystemMatrixColor_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemMatrixColor]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemMatrixColor] (
			...
			SELECT
				  STG.[HairSystemMatrixColorSSID]
				, STG.[HairSystemMatrixColorDescription]
				, STG.[HairSystemMatrixColorDescriptionShort]
				, STG.[HairSystemMatrixColorSortOrder]
				, STG.[Active]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[HairSystemMatrixColorDescription] = STG.[HairSystemMatrixColorDescription]
				, DW.[HairSystemMatrixColorDescriptionShort] = STG.[HairSystemMatrixColorDescriptionShort]
				, DW.[HairSystemMatrixColorSortOrder] = STG.[HairSystemMatrixColorSortOrder]
				, DW.[Active] = STG.[Active]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimHairSystemMatrixColor] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemMatrixColor] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemMatrixColor]
