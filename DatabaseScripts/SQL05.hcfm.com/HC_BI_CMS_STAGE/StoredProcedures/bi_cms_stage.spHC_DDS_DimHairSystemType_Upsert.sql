/* CreateDate: 06/27/2011 17:23:31.920 , ModifyDate: 06/27/2011 17:23:31.920 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimHairSystemType_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimHairSystemType_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimHairSystemType_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemType]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemType] (
					  [HairSystemTypeSSID]
			...
			SELECT
				  STG.[HairSystemTypeSSID]
				, STG.[HairSystemTypeDescription]
				, STG.[HairSystemTypeDescriptionShort]
				, STG.[HairSystemTypeSortOrder]
				, STG.[Active]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[HairSystemTypeDescription] = STG.[HairSystemTypeDescription]
				, DW.[HairSystemTypeDescriptionShort] = STG.[HairSystemTypeDescriptionShort]
				, DW.[HairSystemTypeSortOrder] = STG.[HairSystemTypeSortOrder]
				, DW.[Active] = STG.[Active]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimHairSystemType] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemType] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemType]
