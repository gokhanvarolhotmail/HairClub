/* CreateDate: 05/03/2010 12:20:27.953 , ModifyDate: 05/03/2010 12:20:27.953 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_FactSales_LoadInferred_DimCenter_ClientHomeCenter]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [spHC_FactSales_LoadInferred_DimCenter] is used to load inferred
-- members to the DimCenter table.
--
--
--   exec [bi_cms_stage].[spHC_FactSales_LoadInferred_DimCenter] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimCenter] (
			...
			SELECT
				  DISTINCT(STG.[ClientHomeCenterSSID])
				, -1
				, -2
				, -1
				, -2
