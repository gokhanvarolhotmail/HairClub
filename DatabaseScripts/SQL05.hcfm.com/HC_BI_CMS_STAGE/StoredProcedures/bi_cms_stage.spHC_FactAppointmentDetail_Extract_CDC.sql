/* CreateDate: 06/27/2011 17:23:12.120 , ModifyDate: 06/27/2011 17:23:12.120 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_FactAppointmentDetail_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_FactAppointmentDetail_Extract_CDC] is used to retrieve a
-- list Appointment
--
--   exec [bi_cms_stage].[spHC_FactAppointmentDetail_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						UPDATE [bief_stage].[_DataFlow]
							...
							INSERT INTO [bi_cms_stage].[FactAppointmentDetail]
									   ( [DataPkgKey]
							...
							SELECT @DataPkgKey
										...
										, COALESCE(chg.[AppointmentDetailGUID], CONVERT(UNIQUEIDENTIFIER,'00000000-0000-0000-0000-000000000002')) AS [AppointmentDetailSSID]
										...
										, chg.AppointmentDetailDuration
	    								, 0 AS [IsException]
										, 0 AS [IsDelete]
										, 0 AS [IsDuplicate]
										, CAST(ISNULL(LTRIM(RTRIM(chg.AppointmentDetailGUID)),'') AS nvarchar(50)) AS [SourceSystemKey]
							FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_datAppointmentDetail](@From_LSN, @To_LSN, @row_filter_option) chg
