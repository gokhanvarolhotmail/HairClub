/* CreateDate: 10/05/2010 14:04:36.980 , ModifyDate: 10/05/2010 14:04:36.980 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DQA_DimClientMembershipAccum]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimClientMembershipAccum] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_DimClientMembershipAccum] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_cms_stage].[spHC_DQA_DimClientMembershipAccum_0000] @DataPkgKey
			END
