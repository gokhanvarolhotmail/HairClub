/* CreateDate: 08/02/2012 14:06:45.500 , ModifyDate: 03/13/2019 11:26:20.900 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimEmployeePosition_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimEmployeePosition_Extract_CDC] is used to retrieve a
-- list of Employee Positions
--
--   exec [bi_cms_stage].[spHC_DimEmployeePosition_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
-- -------  ----------  -----------  ------------------------------------
--  v1.0    08/02/2011  KMurdoch       Initial Creation
--
-------------------------------------------------------------------------
BEGIN
	   ...
	   -- Put parameters in name value list to aid in reporting
		...
		IF (@CET IS NOT NULL) AND (@LSET IS NOT NULL)
			BEGIN
				...
				--		SELECT @DataPkgKey
				--			, NULL AS [EmployeePositionKey]
				--			, chg.[EmployeePositionID] AS [EmployeePositionSSID]
				--			, CAST(ISNULL(LTRIM(RTRIM(chg.[EmployeePositionDescription])),'') AS nvarchar(100)) AS [EmployeePositionDescription]
				--			, CAST(ISNULL(LTRIM(RTRIM(chg.[EmployeePositionDescriptionShort])),'') AS nvarchar(10)) AS [EmployeePositionDescriptionShort]
				--			, chg.[EmployeePositionSortOrder]
				--			, CAST(ISNULL(LTRIM(RTRIM(chg.[ActiveDirectoryGroup])),'') AS nvarchar(50)) AS [ActiveDirectoryGroup]
				--			, chg.[IsAdministratorFlag]
				--			, chg.[IsEmployeeOneFlag]
				--			, chg.[IsEmployeeTwoFlag]
				--			, chg.[IsEmployeeThreeFlag]
				--			, chg.[IsEmployeeFourFlag]
				--			, chg.[CanScheduleFlag]
				--			, chg.[ApplicationTimeoutMinutes]
				--			, chg.[UseDefaultCenterFlag]
				--			, chg.[IsSurgeryCenterEmployeeFlag]
				--			, chg.[IsNonSurgeryCenterEmployeeFlag]
				--			, CASE WHEN ISNULL(chg.[IsActiveFlag],0) = 1 then 'Y' else 'N' end as Active
				--			, chg.[LastUpdate] AS [ModifiedDate]
				--			, 0 AS [IsNew]
				--			, 0 AS [IsType1]
				--			, 0 AS [IsType2]
				--			, 0 AS [IsException]
				...
				--		FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpEmployeePosition](@From_LSN, @To_LSN, @row_filter_option) chg
