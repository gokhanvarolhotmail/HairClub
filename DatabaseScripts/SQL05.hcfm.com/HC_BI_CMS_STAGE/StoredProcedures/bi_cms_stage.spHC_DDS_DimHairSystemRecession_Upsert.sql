/* CreateDate: 06/27/2011 17:23:32.027 , ModifyDate: 06/27/2011 17:23:32.027 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimHairSystemRecession_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimHairSystemRecession_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimHairSystemRecession_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemRecession]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemRecession] (
			...
			SELECT
				  STG.[HairSystemRecessionSSID]
				, STG.[HairSystemRecessionDescription]
				, STG.[HairSystemRecessionDescriptionShort]
				, STG.[HairSystemRecessionSortOrder]
				, STG.[Active]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[HairSystemRecessionDescription] = STG.[HairSystemRecessionDescription]
				, DW.[HairSystemRecessionDescriptionShort] = STG.[HairSystemRecessionDescriptionShort]
				, DW.[HairSystemRecessionSortOrder] = STG.[HairSystemRecessionSortOrder]
				, DW.[Active]= STG.[Active]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimHairSystemRecession] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemRecession] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemRecession]
