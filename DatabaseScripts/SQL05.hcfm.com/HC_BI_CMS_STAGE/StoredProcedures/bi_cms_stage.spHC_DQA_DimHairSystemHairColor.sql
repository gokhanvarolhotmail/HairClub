/* CreateDate: 06/27/2011 17:23:47.487 , ModifyDate: 06/27/2011 17:23:47.487 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DQA_DimHairSystemHairColor]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimHairSystemHairColor] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_DimHairSystemHairColor] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_cms_stage].[spHC_DQA_DimHairSystemHairColor_0000] @DataPkgKey
			END
