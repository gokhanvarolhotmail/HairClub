/* CreateDate: 06/27/2011 17:23:32.220 , ModifyDate: 06/27/2011 17:23:32.220 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DDS_DimHairSystemFrontalDensity_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimHairSystemFrontalDensity_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_cms_stage].[spHC_DDS_DimHairSystemFrontalDensity_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemFrontalDensity]
		...
		-- New Records
		------------------------
		INSERT INTO [bi_cms_stage].[synHC_DDS_DimHairSystemFrontalDensity] (
			...
			SELECT
				  STG.[HairSystemFrontalDensitySSID]
				, STG.[HairSystemFrontalDensityDescription]
				, STG.[HairSystemFrontalDensityDescriptionShort]
				, STG.[HairSystemFrontalDensitySortOrder]
				, STG.[Active]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				  DW.[HairSystemFrontalDensityDescription] = STG.[HairSystemFrontalDensityDescription]
				, DW.[HairSystemFrontalDensityDescriptionShort] = STG.[HairSystemFrontalDensityDescriptionShort]
				, DW.[HairSystemFrontalDensitySortOrder] = STG.[HairSystemFrontalDensitySortOrder]
				, DW.[Active] = STG.[Active]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_cms_stage].[DimHairSystemFrontalDensity] STG
		JOIN [bi_cms_stage].[synHC_DDS_DimHairSystemFrontalDensity] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_cms_stage].[synHC_DDS_DimHairSystemFrontalDensity]
