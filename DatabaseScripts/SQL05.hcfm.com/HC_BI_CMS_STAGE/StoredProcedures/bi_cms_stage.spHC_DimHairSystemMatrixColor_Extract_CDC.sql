/* CreateDate: 06/27/2011 17:23:15.570 , ModifyDate: 06/27/2011 17:23:15.570 */
GO
create PROCEDURE [bi_cms_stage].[spHC_DimHairSystemMatrixColor_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimHairSystemMatrixColor_Extract_CDC] is used to retrieve a
-- list HairSystemMatrixColor
--
--   exec [bi_cms_stage].[spHC_DimHairSystemMatrixColor_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpHairSystemMatrixColor'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN)) -- there is a logical range
					BEGIN
						...
						INSERT INTO [bi_cms_stage].[DimHairSystemMatrixColor]
								   ( [DataPkgKey]
						...
						SELECT @DataPkgKey
								, NULL AS [HairSystemMatrixColorKey]
								, chg.HairSystemMatrixColorID AS [HairSystemMatrixColorSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemMatrixColorDescription)),'') AS nvarchar(50)) AS [HairSystemMatrixColorDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemMatrixColorDescriptionShort)),'') AS nvarchar(10)) AS [HairSystemMatrixColorDescriptionShort]
								, chg.HairSystemMatrixColorSortOrder
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[HairSystemMatrixColorID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpHairSystemMatrixColor](@From_LSN, @To_LSN, @row_filter_option) chg
