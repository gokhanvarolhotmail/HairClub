/* CreateDate: 06/27/2011 17:23:44.593 , ModifyDate: 06/27/2011 17:23:44.593 */
GO
create PROCEDURE [bi_cms_stage].[spHC_DimHairSystemStyle_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimHairSystemStyle_Extract_CDC] is used to retrieve a
-- list HairSystemStyle
--
--   exec [bi_cms_stage].[spHC_DimHairSystemStyle_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpHairSystemStyle'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN)) -- there is a logical range
					BEGIN
						...
						INSERT INTO [bi_cms_stage].[DimHairSystemStyle]
								   ( [DataPkgKey]
									, [HairSystemStyleKey]
									, [HairSystemStyleSSID]
						...
						SELECT @DataPkgKey
								, NULL AS [HairSystemStyleKey]
								, chg.HairSystemStyleID AS [HairSystemStyleSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemStyleDescription)),'') AS nvarchar(50)) AS [HairSystemStyleDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.HairSystemStyleDescriptionShort)),'') AS nvarchar(10)) AS [HairSystemStyleDescriptionShort]
								, chg.HairSystemStyleSortOrder
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[HairSystemStyleID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpHairSystemStyle](@From_LSN, @To_LSN, @row_filter_option) chg
