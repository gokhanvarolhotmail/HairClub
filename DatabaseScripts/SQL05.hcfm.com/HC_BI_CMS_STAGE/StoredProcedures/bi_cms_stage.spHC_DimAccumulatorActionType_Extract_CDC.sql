/* CreateDate: 05/03/2010 12:19:50.310 , ModifyDate: 05/03/2010 12:19:50.310 */
GO
CREATE PROCEDURE [bi_cms_stage].[spHC_DimAccumulatorActionType_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimAccumulatorActionType_Extract_CDC] is used to retrieve a
-- list Business Unit
--
--   exec [bi_cms_stage].[spHC_DimAccumulatorActionType_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_lkpAccumulatorActionType'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HairClubCMS].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						INSERT INTO [bi_cms_stage].[DimAccumulatorActionType]
								   ( [DataPkgKey]
						...
						SELECT @DataPkgKey
								, NULL AS [AccumulatorActionTypeKey]
								, chg.[AccumulatorActionTypeID] AS [AccumulatorActionTypeSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[AccumulatorActionTypeDescription])),'') AS nvarchar(50)) AS [AccumulatorActionTypeDescription]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[AccumulatorActionTypeDescriptionShort])),'') AS nvarchar(10)) AS [AccumulatorActionTypeDescriptionShort]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[AccumulatorActionTypeID])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HairClubCMS].[cdc].[fn_cdc_get_net_changes_dbo_lkpAccumulatorActionType](@From_LSN, @To_LSN, @row_filter_option) chg
