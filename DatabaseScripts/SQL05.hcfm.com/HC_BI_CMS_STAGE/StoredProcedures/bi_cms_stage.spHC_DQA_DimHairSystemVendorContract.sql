/* CreateDate: 06/27/2011 17:23:47.897 , ModifyDate: 06/27/2011 17:23:47.897 */
GO
create PROCEDURE [bi_cms_stage].[spHC_DQA_DimHairSystemVendorContract]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimHairSystemVendorContract] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_cms_stage].[spHC_DQA_DimHairSystemVendorContract] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_cms_stage].[spHC_DQA_DimHairSystemVendorContract_0000] @DataPkgKey
			END
