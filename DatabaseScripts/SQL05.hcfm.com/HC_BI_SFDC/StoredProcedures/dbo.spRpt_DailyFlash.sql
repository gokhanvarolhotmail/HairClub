/* CreateDate: 03/21/2022 07:50:26.190 , ModifyDate: 03/21/2022 07:50:26.190 */
GO
/***********************************************************************
    ...
    SELECT t.FullDate
         , SUM(CASE
                   WHEN t.Action__c IN ('Appointment', 'In House', 'Recovery') AND
                        ISNULL(t.Result__c, '') NOT IN ('Void', 'Cancel', 'Reschedule', 'Center Exception') THEN 1
                   ELSE 0 END)                                                                       AS 'Appointments'
         , SUM(CASE
                   WHEN ISNULL(t.Result__c, '') IN ('Show No Sale', 'Show Sale') AND
                        ISNULL(t.ExcludeFromConsults, 0) = 0
                       THEN 1
                   ELSE 0 END)                                                                       AS 'Consultations'
         , SUM(CASE
                   WHEN ISNULL(t.Accomodation, 'In Person Consult') = 'In Person Consult' AND
