/* CreateDate: 07/20/2014 22:04:21.977 , ModifyDate: 08/04/2014 18:31:45.270 */
GO
CREATE PROC [dbo].[spGetSQLPerfStats]
AS
SET NOCOUNT ON

CREATE TABLE #tFileList
(
	DatabaseName sysname,
	LogSize decimal(18,5),
	LogUsed decimal(18,5),
	Status INT
)

INSERT INTO #tFileList
       EXEC spSQLPerf

INSERT INTO LogSpaceStats (DatabaseName, LogSize, LogUsed)
SELECT DatabaseName, LogSize, LogUsed
FROM #tFileList
WHERE DatabaseName IN ('HC_Accounting', 'HC_BI_CMS_DDS', 'HC_BI_ENT_DDS', 'HC_BI_MKTG_DDS', 'HCM', 'HairClubCMS')

DROP TABLE #tFileList
GO
