/* CreateDate: 10/19/2016 15:23:22.973 , ModifyDate: 06/16/2020 13:23:55.550 */
GO
/***********************************************************************
PROCEDURE:				spSvc_GetClientTextMessageReminders
...
EXEC spSvc_GetClientTextMessageReminders ''
***********************************************************************/
CREATE PROCEDURE [dbo].[spSvc_GetClientTextMessageReminders]
(
...
,	Client_Email [NVARCHAR](100) NULL
...
,	LanguageID [INT] NULL
,	IsCenterClosedForAppointmentFlag [BIT] NULL
);
...
;WITH    CenterEmployees
          AS ( SELECT   ec.CenterID
			...
			(	SELECT   ROW_NUMBER() OVER ( PARTITION BY dcp.ClientGUID ORDER BY dcp.ClientPhoneSortOrder ASC ) AS 'RowID'
				...
				WHERE	ISNULL(dcp.CanConfirmAppointmentByText, 0) = 1
			)
     INSERT INTO @tmpRecords
            ...
            ,       c.LanguageID
			,		NULL AS 'IsCenterClosedForAppointmentFlag'
            FROM    HairClubCMS.dbo.datAppointment a
					INNER JOIN HairClubCMS.dbo.datClient c
            ...
            WHERE   1 = 1
					AND ( p.PhoneNumber IS NOT NULL AND LEN(RTRIM(LTRIM(p.PhoneNumber))) > 0 )
...
INSERT  INTO datClientMessageLog (
        ...
        ,       FORMATMESSAGE(TM.TextMessage, LEFT(DATENAME(dw, TR.Appointment_Date), 3) + ' ' + CAST(MONTH(TR.Appointment_Date) AS VARCHAR) + '/' + CAST(DAY(TR.Appointment_Date) AS VARCHAR), CONVERT(VARCHAR(15), TR.Appointment_Time, 100), CTR.CenterDescription, TR.Center_Phone) AS 'TextMessage'
        ...
        FROM    @tmpRecords TR
                INNER JOIN HC_Marketing.dbo.lkpTextMessageProcess TMP
                    ON TMP.TextMessageProcessDescriptionShort = '72HRCLTEXT'
