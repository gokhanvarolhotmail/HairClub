/* CreateDate: 08/21/2018 14:08:14.710 , ModifyDate: 10/26/2018 12:33:08.390 */
GO
/***********************************************************************
PROCEDURE:				spSvc_NobleSourceTollFreeNumberExport
DESTINATION SERVER:		SQL05
DESTINATION DATABASE:	HC_Marketing
AUTHOR:					Dominic Leiba
IMPLEMENTOR:			Dominic Leiba
DATE IMPLEMENTED:		8/15/2018
DESCRIPTION:			8/15/2018
------------------------------------------------------------------------
NOTES:
...
EXEC spSvc_NobleSourceTollFreeNumberExport
***********************************************************************/
CREATE PROCEDURE [dbo].[spSvc_NobleSourceTollFreeNumberExport]
AS
BEGIN
...
SELECT  sc.SourceCode
,		REPLACE(REPLACE(REPLACE(REPLACE(sc.Number, '(',''), ')',''), '-',''), ' ','') AS 'TollFreeNumber'
,		sc.DNIS AS 'HairclubDNIS'
FROM    #SourceCode sc
WHERE	ISNULL(sc.Number, '') <> ''
		AND (
				( sc.Status = 'In Progress' AND GETDATE() BETWEEN sc.StartDate AND sc.EndDate ) -- Current Campaigns
					OR ( sc.Status = 'In Progress' AND sc.StartDate >= GETDATE() ) -- Future Campaigns
			)
