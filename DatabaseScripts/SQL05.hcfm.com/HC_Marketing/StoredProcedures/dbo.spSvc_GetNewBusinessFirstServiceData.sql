/* CreateDate: 10/24/2017 18:21:10.450 , ModifyDate: 10/22/2018 10:34:48.760 */
GO
/***********************************************************************
PROCEDURE:				spSvc_GetNewBusinessFirstServiceData
...
AND FirstServiceDate is exactly 2 days ago
...
EXEC spSvc_GetNewBusinessFirstServiceData NULL, NULL
***********************************************************************/
CREATE PROCEDURE [dbo].[spSvc_GetNewBusinessFirstServiceData]
(
...
IF ( @StartDate IS NULL OR @EndDate IS NULL )
   BEGIN
		SET @StartDate = DATEADD(dd, -2, CAST(CONVERT(VARCHAR, @CurrentDate, 10) AS DATETIME))
		SET @EndDate = DATEADD(dd, -2, CAST(CONVERT(VARCHAR, @CurrentDate, 10) AS DATETIME))
...
INSERT  INTO #Center
		...
		WHERE   CONVERT(VARCHAR, ctr.CenterNumber) LIKE '[278]%'
...
INSERT	INTO #ClientMembership
		...
		WHERE	rg.RevenueGroupDescription = 'New Business'
				AND cm.MembershipDescription NOT IN ( 'New Client (ShowNoSale)', 'New Client (Surgery Offered)', 'Post Extreme', 'Bosley Medical Services', 'Additional  Surgery' )
				AND cms.ClientMembershipStatusDescription = 'Active'


/********************************** Get First Service Data ***********************************************/
INSERT  INTO #Service
		SELECT	ROW_NUMBER() OVER ( PARTITION BY so.ClientGUID ORDER BY so.OrderDate ) AS 'RowID'
		,		so.ClientGUID
		...
		FROM    HairClubCMS.dbo.datSalesOrder so WITH ( NOLOCK )
				...
				INNER JOIN HairClubCMS.dbo.datSalesOrderDetail sod WITH ( NOLOCK )
				...
				LEFT OUTER JOIN HairClubCMS.dbo.datEmployee sty WITH ( NOLOCK )
					ON sty.EmployeeGUID = sod.Employee2GUID
		WHERE   so.OrderDate >= cm.BeginDate
				AND ( sc.SalesCodeDescriptionShort = 'NB1A' -- Initial New Style
					OR sc.SalesCodeDepartmentID IN ( 5035, 5037 ) ) -- Services
				AND sc.SalesCodeDescriptionShort NOT IN ( 'XTRPRECHK', 'XTRREDO', 'EXTSVCXTD', 'XTRTRAINSVC', 'XTRPROMOSVC', 'TAPESVC', 'HALOSVC' )
...
INSERT	INTO #Client
		...
		WHERE   ( s.RowID = 1 AND s.ServiceDate BETWEEN @StartDate AND @EndDate ) -- First service was 2 days ago
