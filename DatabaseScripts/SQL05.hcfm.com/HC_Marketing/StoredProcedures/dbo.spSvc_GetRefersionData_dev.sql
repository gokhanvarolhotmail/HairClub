/* CreateDate: 07/16/2021 14:07:26.463 , ModifyDate: 07/19/2021 11:42:08.830 */
GO
/***********************************************************************
...
EXEC spSvc_GetRefersionData_dev ''
***********************************************************************/
CREATE PROCEDURE [dbo].[spSvc_GetRefersionData_dev](
...
-- Get Leads Created within time period
    INSERT INTO datRefersionLog_dev
    ...
    FROM SQL06.HC_BI_SFDC.dbo.Lead l WITH (NOLOCK)
        ...
        SELECT TOP 1 rl.RefersionLogID
                   , rl.SFDC_LeadID
                   , rl.IsReprocessFlag
        FROM HC_Marketing.dbo.datRefersionLog_dev rl
...
-- Update Batch ID
    ...
    WHILE @rowCount > 0
        BEGIN
            ...
            SET BatchID = @batchID
            FROM NextBatch
                     JOIN datRefersionLog_dev
                          ON NextBatch.RefersionLogID = datRefersionLog_dev.RefersionLogID;
...
-- Reset records marked for reprocessing
    ...
    FROM datRefersionLog_dev rl
             INNER JOIN datRefersionLog_dev rl2
