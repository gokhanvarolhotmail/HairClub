/* CreateDate: 09/02/2021 19:50:05.160 , ModifyDate: 09/02/2021 19:50:05.160 */
GO
/***********************************************************************
...
EXEC spSvc_GetRefersionData_adhoc 'SSID', 'LeadID', 'Step', 'Date based on Step'
EXEC spSvc_GetRefersionData_adhoc '04B6F67D-B5A0-466F-BD72-299545BA4593', '00Q5e000007NXTPEA4', '2', '2021-07-10'
***********************************************************************/
CREATE PROCEDURE [dbo].[spSvc_GetRefersionData_AdHoc](
...
-- Get Shows that occurred within time period
    ...
    WHERE (t.ActivityDate = @TargetDate
      ...
      AND ISNULL(l.IsDeleted, 0) = 0
      AND (o_Rl.RefersionLogID IS NULL OR o_Rl.IsReprocessFlag = 1)
            and t.id = @taskID and (l.id= @LeadID or l.externalID = @LeadID)
...
-- Get Clients that been serviced and a commission record has been created within time period
    ...
    FROM SQL06.HC_Commission.dbo.FactCommissionHeader fch
             ...
             INNER JOIN SQL06.HC_BI_SFDC.dbo.Lead l
                        ON l.Id = clt.SFDC_Leadid or clt.SFDC_Leadid = l.externalid
             INNER JOIN HC_Marketing.dbo.lkpRefersionProcess rp
    ...
    WHERE fch.CommissionTypeID IN (29, 30, 3, 46, 47, 77, 53, 54, 4)
      ...
      AND ISNULL(l.IsDeleted, 0) = 0
      AND (o_Rl.RefersionLogID IS NULL OR o_Rl.IsReprocessFlag = 1)
            and (l.id= @LeadID or l.externalid = @LeadID)
