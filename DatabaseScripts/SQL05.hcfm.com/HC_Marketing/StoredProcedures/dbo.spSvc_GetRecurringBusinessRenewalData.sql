/* CreateDate: 10/24/2017 18:33:31.270 , ModifyDate: 10/22/2018 10:34:29.657 */
GO
/***********************************************************************
PROCEDURE:				spSvc_GetRecurringBusinessRenewalData
...
EXEC spSvc_GetRecurringBusinessRenewalData
***********************************************************************/
CREATE PROCEDURE [dbo].[spSvc_GetRecurringBusinessRenewalData]
AS
BEGIN
...
,	CRMFirstName NVARCHAR(50)
,	CRMLastName NVARCHAR(50)
)
...
INSERT	INTO #ClientMembership
		...
		WHERE	rg.RevenueGroupDescription = 'Recurring Business'
				AND cms.ClientMembershipStatusDescription = 'Active'
				AND DATEDIFF(DAY, @CurrentDate, dcm.EndDate) = 30
...
INSERT	INTO #Client
		...
		WHERE	clt.ClientGUID IN ( SELECT DISTINCT cm.ClientGUID FROM #ClientMembership cm ) -- Client Cancelled within time period
...
INSERT	INTO #Conversion
		SELECT  ROW_NUMBER() OVER ( PARTITION BY clt.ClientGUID ORDER BY so.OrderDate ASC ) AS 'RowID'
		,       clt.ClientGUID
		,       CAST(so.OrderDate AS DATE) AS 'ConversionDate'
		,       con.FirstName AS 'CRMFirstName'
		,       con.LastName AS 'CRMLastName'
		FROM    HairClubCMS.dbo.datSalesOrderDetail sod WITH ( NOLOCK )
				...
				INNER JOIN HairClubCMS.dbo.datClientMembership dcm WITH ( NOLOCK )
				...
				LEFT OUTER JOIN HairClubCMS.dbo.datEmployee con
					ON con.EmployeeGUID = sod.Employee1GUID
		WHERE   sc.SalesCodeDepartmentID = 1075 -- Conversions
				AND m.MembershipDescriptionShort NOT IN ( 'SHOWNOSALE', 'SNSSURGOFF' ) -- New Client (ShowNoSale), New Client (Surgery Offered)
