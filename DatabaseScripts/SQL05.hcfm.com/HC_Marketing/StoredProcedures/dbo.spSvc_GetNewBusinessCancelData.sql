/* CreateDate: 10/24/2017 18:25:40.657 , ModifyDate: 10/22/2018 10:34:42.173 */
GO
/***********************************************************************
PROCEDURE:				spSvc_GetNewBusinessCancelData
...
AND CancelDate is exactly 1 day ago
AND RevenueGroup = New Business
AND MembershipStatus is not equal to one of the following (Active | Converted)
AND CancelReasonDescription is not equal to one of the following (DATA ENTRY ERROR | UNKNOWN)
------------------------------------------------------------------------
SAMPLE EXECUTION:

EXEC spSvc_GetNewBusinessCancelData NULL, NULL
***********************************************************************/
CREATE PROCEDURE [dbo].[spSvc_GetNewBusinessCancelData]
(
...
WHERE   clt.EmailAddress <> '' -- Email Address cannot be empty
		...
		AND ( ( cm_xtrp.RevenueGroup = 'New Business' AND cm_xtrp.MembershipStatus = 'Canceled' AND cm_xtrp.Membership <> 'New Client (ShowNoSale)' ) -- Canceled Xtrands Plus Membership
				OR ( cm_ext.RevenueGroup = 'New Business' AND cm_ext.MembershipStatus = 'Canceled' ) -- Canceled EXT Membership
				OR ( cm_xtr.RevenueGroup = 'New Business' AND cm_xtr.MembershipStatus = 'Canceled' ) -- Canceled Xtrands Membership
				OR ( cm_sur.RevenueGroup = 'New Business' AND cm_sur.MembershipStatus = 'Canceled' AND cm_sur.Membership <> 'New Client (Surgery Offered)' ) ) -- Canceled Surgery Membership
