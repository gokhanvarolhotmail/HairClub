/* CreateDate: 04/28/2021 10:12:29.297 , ModifyDate: 04/28/2021 10:12:29.297 */
GO
create procedure [bi_cms_dds].[spSVC_GenerateLifeTimeValue]
as
-------------------------------------------------------------------------
...
--
--
--   exec [bi_cms_dds].[spSVC_GenerateLifeTimeValue]
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
*/
    select row_number() over (partition by dc.ClientIdentifier
                              order by clm.ClientMembershipBeginDate asc
                             )                                  as 'RowID'
...
*/
    select row_number() over (partition by dc.ClientIdentifier
         ...
         , case
               when eft.EFTAccountTypeID = 1 then
                   'CC'
               when eft.EFTAccountTypeID = 2 then
                   'Checking'
               when eft.EFTAccountTypeID = 3 then
                   'Savings'
               when eft.EFTAccountTypeID = 4 then
                   'A/R'
               else
                   null
...
*/
    ...
    select dc.ClientIdentifier as 'ClientIdentifier'
         ...
         , sum(   case
                      when dscd.SalesCodeDepartmentSSID in ( 3010, 3020, 3030, 3040, 3050, 3060, 3070, 3080, 7052 ) then
                          fst.ExtendedPrice
                      else
                          0
                  end
    ...
    where isnull(fst.IsVoided, 0) = 0
          and dscd.SalesCodeDepartmentSSID not like '1%'
          and fst.SalesOrderTypeKey not in ( 3, 7 )
...
*/
    ...
    select ct.CenterTypeDescription                                as 'CenterType'
         ...
         , convert(varchar, cast(mr.NBMembershipRevenue as money)) as 'InitialMbrRevenue'
         , mc.MembershipStatus                                     as 'CurrMbrStatus'
         , mc.Membership                                           as 'CurrentMembership'
         , mc.PaymentMethod                                        as 'CurrentMbrPmtMethod'
         ...
         , datediff(day, fldd.FullDate, mc.RRMbrEndDate)           as 'daysfromLeadCreatetoEndMbr'
         , datediff(day, mf.InitMbrBegDate, mc.RRMbrEndDate)       as 'DaysfromInitMbrtoEnd'
         , mr.[2008RRev]
         , mr.[2009RRev]
         , mr.[2010RRev]
         , mr.[2011RRev]
