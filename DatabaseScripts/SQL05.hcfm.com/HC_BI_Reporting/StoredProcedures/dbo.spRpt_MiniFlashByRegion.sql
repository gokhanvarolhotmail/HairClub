/* CreateDate: 05/15/2013 12:12:12.440 , ModifyDate: 02/25/2019 12:58:23.797 */
GO
/*===============================================================================================
...
-- Notes:
	...
	02/25/2019 - RH - (per Kevin) Changed some joins to ReportingCenterSSID so that Colorado Springs will not be excluded
-- ----------------------------------------------------------------------------------------------
--
...
BEGIN
	...
	*/
	IF @RegionID=1
		BEGIN
			...
			SELECT ReportingCenterSSID AS 'Center'
	...
	*/
	SELECT DC.ReportingCenterSSID AS 'Center'
	,	DD.FullDate AS 'Date'
	...
	WHERE DD.FullDate BETWEEN @MinDate AND @MaxDate
		AND DC.Active='Y'
		AND DC.ReportingCenterSSID IN (SELECT Center FROM #CentersByRegion)
	...
	FROM HC_BI_CMS_DDS.bi_cms_dds.DimClient clt
		INNER JOIN HC_BI_ENT_DDS.bi_ent_dds.DimCenter C
			ON clt.CenterSSID = C.CenterSSID				--Colorado Springs is 1002 as CenterSSID in DimClient
		INNER JOIN HC_BI_CMS_DDS.bi_cms_dds.DimClientMembership cm
