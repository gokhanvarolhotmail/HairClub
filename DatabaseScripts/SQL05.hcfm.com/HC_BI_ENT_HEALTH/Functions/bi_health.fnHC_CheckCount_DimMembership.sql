/* CreateDate: 05/08/2010 15:55:29.473 , ModifyDate: 05/13/2010 16:11:40.883 */
GO
CREATE    FUNCTION [bi_health].[fnHC_CheckCount_DimMembership] ()
-----------------------------------------------------------------------
-- [fnHC_CheckCount_DimMembership]
--
-- SELECT * FROM [bi_health].[fnHC_CheckCount_DimMembership]()
-----------------------------------------------------------------------
-- Change History
-----------------------------------------------------------------------
...
BEGIN
	...
	FROM [bi_health].[synHC_OLTP_SRC_TBL_CMS_cfgMembership] WITH (NOLOCK)

	SELECT @NumRecordsInSourceRepl = COUNT(*)
	FROM [bi_health].[synHC_SRC_TBL_CMS_cfgMembership] WITH (NOLOCK)

	SELECT @NumRecordsInWarehouse = COUNT(*)
	FROM [bi_health].[synHC_DDS_DimMembership] DW WITH (NOLOCK)
	WHERE DW.[MembershipKey] <> -1
