/* CreateDate: 05/03/2010 20:03:50.777 , ModifyDate: 05/03/2010 20:03:50.777 */
GO
CREATE PROCEDURE [bi_health].[spHC_ListCDC_DimContactAddress]
		  @TimeIncrement		int = 1
...
-----------------------------------------------------------------------
-- [bi_health].[spHC_NumCDC_DimContactAddress]
--
-- EXEC [bi_health].[spHC_ListCDC_DimContactAddress] 5, 'DAY'
-----------------------------------------------------------------------
-- Change History
-----------------------------------------------------------------------
...
BEGIN
		...
		DECLARE	  @Start_Time			datetime = null
				...
				, @CDCTableName			varchar(150) = N'dbo_oncd_contact_address'	-- Name of CDC table
				, @From_LSN				binary(10)
				, @To_LSN				binary(10)
		...
		SELECT @Start_Time AS ChangesFromTime
				...
				, *
		FROM [HCM].[cdc].[fn_cdc_get_net_changes_dbo_oncd_contact_address](@From_LSN, @To_LSN, @row_filter_option) chg
		ORDER BY ChangeTime
