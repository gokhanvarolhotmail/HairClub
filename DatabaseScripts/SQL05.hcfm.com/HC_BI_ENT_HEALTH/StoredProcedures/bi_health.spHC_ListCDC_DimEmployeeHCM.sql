/* CreateDate: 05/03/2010 19:56:27.650 , ModifyDate: 05/03/2010 19:56:27.650 */
GO
CREATE PROCEDURE [bi_health].[spHC_ListCDC_DimEmployeeHCM]
		  @TimeIncrement		int = 1
...
-----------------------------------------------------------------------
-- [bi_health].[spHC_NumCDC_DimEmployee]
--
-- EXEC [bi_health].[spHC_ListCDC_DimEmployee] 5, 'DAY'
-----------------------------------------------------------------------
-- Change History
-----------------------------------------------------------------------
...
BEGIN
		...
		DECLARE	  @Start_Time			datetime = null
				...
				, @CDCTableName			varchar(150) = N'dbo_onca_user'	-- Name of CDC table
				, @From_LSN				binary(10)
				, @To_LSN				binary(10)
		...
		SELECT @Start_Time AS ChangesFromTime
				...
				, *
		FROM [HCM].[cdc].[fn_cdc_get_net_changes_dbo_onca_user](@From_LSN, @To_LSN, @row_filter_option) chg
		ORDER BY ChangeTime
