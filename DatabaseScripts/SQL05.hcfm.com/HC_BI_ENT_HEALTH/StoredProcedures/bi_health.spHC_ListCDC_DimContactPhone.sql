/* CreateDate: 05/03/2010 20:01:45.470 , ModifyDate: 05/03/2010 20:01:45.470 */
GO
CREATE PROCEDURE [bi_health].[spHC_ListCDC_DimContactPhone]
		  @TimeIncrement		int = 1
...
-----------------------------------------------------------------------
-- [bi_health].[spHC_NumCDC_DimContactPhone]
--
-- EXEC [bi_health].[spHC_ListCDC_DimContactPhone] 5, 'DAY'
-----------------------------------------------------------------------
-- Change History
-----------------------------------------------------------------------
...
BEGIN
		...
		DECLARE	  @Start_Time			datetime = null
				...
				, @CDCTableName			varchar(150) = N'dbo_oncd_contact_phone'	-- Name of CDC table
				, @From_LSN				binary(10)
				, @To_LSN				binary(10)
		...
		SELECT @Start_Time AS ChangesFromTime
				...
				, *
		FROM [HCM].[cdc].[fn_cdc_get_net_changes_dbo_oncd_contact_phone](@From_LSN, @To_LSN, @row_filter_option) chg
		ORDER BY ChangeTime
