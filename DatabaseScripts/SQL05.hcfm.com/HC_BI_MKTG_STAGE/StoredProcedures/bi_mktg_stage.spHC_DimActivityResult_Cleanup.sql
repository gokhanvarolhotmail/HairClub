/* CreateDate: 05/03/2010 12:26:27.793 , ModifyDate: 05/03/2010 12:26:27.793 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DimActivityResult_Cleanup]
       (
			  @DataPkgKey				int
       )
...
AS
-------------------------------------------------------------------------
-- [spHC_DimActivityResult_Cleanup] is used to Cleanup records
--
--
-- EXEC [spHC_DimActivityResult_Cleanup] 422
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
  ...
  BEGIN TRY
		...
		WHILE ( 0 = 0 )
			BEGIN
				...
				FROM [bi_mktg_stage].[DimActivityResult]
				WHERE DataPkgKey = @DataPkgKey
				AND IsLoaded = 1
