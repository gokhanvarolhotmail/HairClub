/* CreateDate: 05/03/2010 12:26:49.793 , ModifyDate: 05/03/2010 12:26:49.793 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DQA_DimResultCode]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimResultCode] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_mktg_stage].[spHC_DQA_DimResultCode] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_mktg_stage].[spHC_DQA_DimResultCode_0000] @DataPkgKey
			END
