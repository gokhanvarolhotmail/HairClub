/* CreateDate: 05/03/2010 12:26:46.773 , ModifyDate: 05/03/2010 12:26:46.773 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DimSalesType_Cleanup]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DimSalesType_Cleanup] is used to Cleanup records
--
--
--   exec [bi_mktg_stage].[spHC_DimSalesType_Cleanup] 422
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		WHILE ( 0 = 0 )
			BEGIN
				...
				FROM [bi_mktg_stage].[DimSalesType]
				WHERE DataPkgKey = @DataPkgKey
				AND IsLoaded = 1
