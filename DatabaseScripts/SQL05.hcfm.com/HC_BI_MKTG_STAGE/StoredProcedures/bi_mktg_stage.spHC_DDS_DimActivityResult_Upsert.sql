/* CreateDate: 08/05/2019 11:14:38.580 , ModifyDate: 09/17/2020 14:45:42.247 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DDS_DimActivityResult_Upsert]
			   @DataPkgKey					int
...
--			08/06/2019  KMurdoch    Made SFDC Lead/Task ID primary
--			07/10/2020  KMurdoch    Added Accomodation
--			09/10/2020  KMurdoch    Added SFDC_PersonAccountID
--			09/17/2020  KMurdoch    Added an ISNULL to centerSSID to accomodate bad data from Salesforce
-------------------------------------------------------------------------
BEGIN
	...
	BEGIN TRY
		...
		WHERE SFDC_TaskID IN ( SELECT STG.SFDC_TaskID
										FROM [bi_mktg_stage].[DimActivityResult] STG
		...
		UPDATE DW SET
				  DW.[CenterSSID] = ISNULL(STG.[CenterSSID],-1)
		...
		FROM [bi_mktg_stage].[DimActivityResult] STG
		JOIN [bi_mktg_stage].[synHC_DDS_DimActivityResult] DW
			ON DW.SFDC_TaskID = STG.SFDC_TaskID
				--ISNULL(DW.SFDC_TaskID,DW.ActivitySSID) = ISNULL(STG.SFDC_TaskID,STG.ActivitySSID)
				--AND ISNULL(DW.SFDC_LeadID,DW.ContactSSID) = ISNULL(STG.SFDC_LeadID,STG.ContactSSID)
