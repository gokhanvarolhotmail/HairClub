/* CreateDate: 05/03/2010 12:26:25.697 , ModifyDate: 05/03/2010 12:26:25.697 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DimActionCode_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimActionCode_Extract_CDC] is used to retrieve a
-- list ActionCode
--
--   exec [bi_mktg_stage].[spHC_DimActionCode_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_onca_action'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HCM].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						INSERT INTO [bi_mktg_stage].[DimActionCode]
								   ( [DataPkgKey]
								   , [ActionCodeKey]
								   , [ActionCodeSSID]
						...
						SELECT @DataPkgKey
								, NULL AS [ActionCodeKey]
								, chg.[action_code] AS [ActionCodeSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[description])),'') AS nvarchar(10)) AS [ActionCodeDescription]
								, '' AS [ActionCodeDescriptionShort]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[action_code])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HCM].[cdc].[fn_cdc_get_net_changes_dbo_onca_action](@From_LSN, @To_LSN, @row_filter_option) chg
