/* CreateDate: 05/03/2010 12:26:54.893 , ModifyDate: 10/26/2011 10:31:42.760 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivityResults_Transform_Lkup_ActivityDueDateKey]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [spHC_FactActivityResults_Transform_Lkup_ActivityDueDateKey] is used to populate
-- the ActivityDueDateKey in the FactActivityResults rows
--
--
--   exec [bi_mktg_stage].[spHC_FactActivityResults_Transform_Lkup_ActivityDueDateKey] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		------------------------
		UPDATE STG SET
		     [ActivityDueDateKey] = COALESCE(DW.[DateKey], 0)
		...
		-----------------------
		-- Exception if [ActivityDueDate] IS NULL
		-----------------------
		UPDATE STG SET
		     IsException = 1
		     ,	ExceptionMessage = 'ActivityDueDateSSID IS NULL - FAR_Trans_LKup'
		FROM [bi_mktg_stage].[FactActivityResults] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND STG.[ActivityDueDateSSID] IS NULL
		...
		UPDATE STG SET
		     IsException = 1
		     ,	ExceptionMessage = 'ActivityDueDateKey IS NULL - FAR_Trans_LKup'
		FROM [bi_mktg_stage].[FactActivityResults] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
