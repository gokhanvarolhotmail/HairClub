/* CreateDate: 08/10/2011 17:53:22.740 , ModifyDate: 08/11/2011 08:57:13.820 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactLead_LoadInferred_DimEmployee_Assigned]
			   @DataPkgKey					int


AS
-------------------------------------------------------------------------
-- [spHC_FactLead_LoadInferred_DimEmployee_Assigned] is used to load inferred
-- members to the DimEmployee table.
--
--
--   exec [bi_mktg_stage].[spHC_FactLead_LoadInferred_DimEmployee_Assigned] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
-- -------  ----------  -----------  ------------------------------------
--  v1.0    08/10/2011  EKnapp       Initial Creation
-------------------------------------------------------------------------
BEGIN
	...
	DECLARE		@DataPkgDetailKey	int
 	...
 	SET @TableName = N'[bi_mktg_dds].[DimEmployee] (Assigned)'
	...
	BEGIN TRY
		...
		INSERT INTO [bi_mktg_stage].[synHC_DDS_DimEmployee] (
			...
			SELECT
				  DISTINCT(STG.[AssignedEmployeeSSID])
				, ''
				, ''
				, ''
				, ''
