/* CreateDate: 08/05/2019 11:23:01.607 , ModifyDate: 08/06/2019 15:32:07.737 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivity_Transform_Lkup_ActivityKey]
			   @DataPkgKey					int
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage= 'SFDC_TaskID is null - FA_Trans_Lkup'
		FROM [bi_mktg_stage].[FactActivity] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			--AND STG.[ActivitySSID] IS NULL
			AND STG.[SFDC_TaskID] IS NULL
