/* CreateDate: 05/03/2010 12:26:50.287 , ModifyDate: 05/03/2010 12:26:50.287 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DQA_FactLead]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_FactLead] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_mktg_stage].[spHC_DQA_FactLead] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_mktg_stage].[spHC_DQA_FactLead_0000] @DataPkgKey
			END

		-----------------------
		-- Validate Rule 4 - DateKey Foreign Key not defined
		-----------------------
		EXEC	@return_value = [bi_mktg_stage].[spHC_DQA_FactLead_0004] @DataPkgKey

		-----------------------
		-- Validate Rule 6 - TimeOfDayKey Foreign Key not defined
		-----------------------
		EXEC	@return_value = [bi_mktg_stage].[spHC_DQA_FactLead_0006] @DataPkgKey

		-----------------------
		-- Validate Rule 22 - CenterKey cannot be NULL
		-----------------------
		EXEC	@return_value = [bi_mktg_stage].[spHC_DQA_FactLead_0022] @DataPkgKey

		-----------------------
		-- Validate Rule 1016 - SourceKey Foreign Key not defined
		-----------------------
		EXEC	@return_value = [bi_mktg_stage].[spHC_DQA_FactLead_1016] @DataPkgKey
