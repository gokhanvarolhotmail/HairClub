/* CreateDate: 05/03/2010 12:26:59.750 , ModifyDate: 10/26/2011 10:42:23.507 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivity_Transform_Lkup_CompletedByEmployeeKey]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [sspHC_FactActivity_Transform_Lkup_CompletedByEmployeeKey] is used to determine
-- the CompletedByEmployeeKey foreign key values in the FactActivity table using DimEmployee.
--
--
--   exec [bi_mktg_stage].[spHC_FactActivity_Transform_Lkup_CompletedByEmployeeKey] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------
		EXEC	@return_value = [bi_mktg_stage].[spHC_FactActivity_LoadInferred_DimEmployee_CompletedByEmployee] @DataPkgKey
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage= 'CompletedbyEmployeeSSID is null - FA_Trans_Lkup'
		FROM [bi_mktg_stage].[FactActivity] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND STG.[CompletedByEmployeeSSID] IS NULL
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage= 'CompletedByEmployeeKey is null - FA_Trans_Lkup'
		FROM [bi_mktg_stage].[FactActivity] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND (STG.[CompletedByEmployeeKey] IS NULL
