/* CreateDate: 05/03/2010 12:26:23.710 , ModifyDate: 05/03/2010 12:26:23.710 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DimActionCode_Cleanup]
       (
			  @DataPkgKey				int
       )
...
AS
-------------------------------------------------------------------------
-- [spHC_DimActionCode_Cleanup] is used to Cleanup records
--
--
-- EXEC [spHC_DimActionCode_Cleanup] 422
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
  ...
  BEGIN TRY
		...
		WHILE ( 0 = 0 )
			BEGIN
				...
				FROM [bi_mktg_stage].[DimActionCode]
				WHERE DataPkgKey = @DataPkgKey
				AND IsLoaded = 1
