/* CreateDate: 05/03/2010 12:26:59.970 , ModifyDate: 08/31/2021 17:07:47.307 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactLead_Extract]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_FactLead_Extract] is used to retrieve a
-- list Leads
--
--   exec [bi_mktg_stage].[spHC_FactLead_Extract]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
--          02/03/2011               Add promotion code and sales type
		...
		IF (@CET IS NOT NULL) AND (@LSET IS NOT NULL)
			BEGIN
				...
				SET @ExtractRowCnt1 = @@ROWCOUNT
				 ...
				 --Now check activities which changed and get list of contacts
					INSERT INTO [bi_mktg_stage].[FactLead]
					(
						[ContactSSID],
						[SFDC_LeadID],
...
/****** Object:  StoredProcedure [bi_mktg_stage].[spHC_FactLead_Extract_CDC]    Script Date: 11/30/2018 12:43:16 PM ******/
SET ANSI_NULLS ON
GO
