/* CreateDate: 05/03/2010 12:26:23.570 , ModifyDate: 05/06/2010 15:41:18.170 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DDS_DimResultCode_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimResultCode_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_mktg_stage].[spHC_DDS_DimResultCode_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		------------------------
		DELETE
		FROM [bi_mktg_stage].[synHC_DDS_DimResultCode]
		WHERE [ResultCodeSSID] IN ( SELECT STG.[ResultCodeSSID]
										FROM [bi_mktg_stage].[DimResultCode] STG
		...
		-- New Records
		------------------------
		INSERT INTO [bi_mktg_stage].[synHC_DDS_DimResultCode] (
					  [ResultCodeSSID]
			...
			SELECT
				...
				, STG.[ResultCodeDescriptionShort]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				...
				, DW.[ResultCodeDescriptionShort] = STG.[ResultCodeDescriptionShort]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_mktg_stage].[DimResultCode] STG
		JOIN [bi_mktg_stage].[synHC_DDS_DimResultCode] DW ON
