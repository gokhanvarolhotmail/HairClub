/* CreateDate: 05/03/2010 12:26:23.613 , ModifyDate: 12/08/2020 13:11:44.913 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DDS_DimSource_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimSource_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_mktg_stage].[spHC_DDS_DimSource_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
--			09/27/2019  KMurdoch     Added Owner Type
--			07/27/2020  KMurdoch     Added Origin
--			09/05/2020	KMurdoch	 Added a check for null ssid
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_mktg_stage].[synHC_DDS_DimSource]
		...
		------------------------
		DELETE
		FROM [bi_mktg_stage].[synHC_DDS_DimSource]
		WHERE [SourceSSID] IN ( SELECT STG.[SourceSSID]
										FROM [bi_mktg_stage].[DimSource] STG
		...
		-- New Records
		------------------------
		INSERT INTO [bi_mktg_stage].[synHC_DDS_DimSource] (
					  [SourceSSID]
					, [SourceName]
					, [Media]
					, [Level02Location]
			...
			WHERE	STG.[IsNew] = 1
				...
				AND STG.[DataPkgKey] = @DataPkgKey
				AND stg.SourceSSID <> NULL   ---I added this on 9/5/2020
		...
		UPDATE DW SET
				  DW.[SourceName] = STG.[SourceName]
				, DW.[Media] = STG.[Media]
				, DW.[Level02Location] = STG.[Level02Location]
				, DW.[Level03Language] = STG.[Level03Language]
				, DW.[Level04Format] = STG.[Level04Format]
				, DW.[Level05Creative] = STG.[Level05Creative]
				, DW.[Number] = STG.[Number]
				, DW.[NumberType] = STG.[NumberType]
				, DW.[Channel] = STG.[Channel]
				, DW.[CampaignName] = STG.[CampaignName]
				, DW.[Gender] = STG.[Gender]
				, DW.[PromoCode] = STG.[PromoCode]
				, DW.[OwnerType] = STG.[OwnerType]
				, DW.[Origin] = STG.[Origin]
				, DW.[Content] = STG.[Content]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_mktg_stage].[DimSource] STG
		JOIN [bi_mktg_stage].[synHC_DDS_DimSource] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_mktg_stage].[synHC_DDS_DimSource]
