/* CreateDate: 05/03/2010 12:26:23.593 , ModifyDate: 05/06/2010 15:42:07.960 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DDS_DimSalesType_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimSalesType_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_mktg_stage].[spHC_DDS_DimSalesType_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_mktg_stage].[synHC_DDS_DimSalesType]
		...
		------------------------
		DELETE
		FROM [bi_mktg_stage].[synHC_DDS_DimSalesType]
		WHERE [SalesTypeSSID] IN ( SELECT STG.[SalesTypeSSID]
										FROM [bi_mktg_stage].[DimSalesType] STG
		...
		-- New Records
		------------------------
		INSERT INTO [bi_mktg_stage].[synHC_DDS_DimSalesType] (
					  [SalesTypeSSID]
		...
		UPDATE DW SET
				...
				, DW.[SalesTypeDescriptionShort] = STG.[SalesTypeDescriptionShort]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_mktg_stage].[DimSalesType] STG
		JOIN [bi_mktg_stage].[synHC_DDS_DimSalesType] DW ON
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_mktg_stage].[synHC_DDS_DimSalesType]
