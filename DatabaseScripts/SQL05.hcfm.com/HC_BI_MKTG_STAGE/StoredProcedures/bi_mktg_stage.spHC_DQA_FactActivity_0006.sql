/* CreateDate: 05/03/2010 12:26:50.207 , ModifyDate: 10/26/2011 10:19:04.390 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DQA_FactActivity_0006]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_FactActivity_0006] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_mktg_stage].[spHC_DQA_FactActivity_0006]
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	SET @RuleKey = 6	--TimeOfDayKey Foreign Key not defined
	SET @TableName = N'[bi_mktg_dqa].[FactActivity]'
	...
	BEGIN TRY
		...
		-----------------------
		EXEC	@return_value = [bi_mktg_stage].[spHC_DQA_FactActivity_Upsert] @DataPkgKey, @RuleKey, @RuleActionKey,
