/* CreateDate: 08/05/2019 11:18:54.617 , ModifyDate: 09/10/2020 20:48:50.190 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DimActivity_Transform_SetIsNewSCD]
       (
			  @DataPkgKey				int
       )
...
--			09/10/2020   KMurdoch    Added SFDC_PersonAccountID check
-------------------------------------------------------------------------
BEGIN
  ...
  -- interfering with SELECT statements.
   ...
   BEGIN TRY
		...
		LEFT OUTER JOIN [bi_mktg_stage].[synHC_DDS_DimActivity] DW
			ON STG.SFDC_TaskID = DW.SFDC_TaskID
		...
		UPDATE STG SET
			IsInferredMember = CASE WHEN DW.[ActivityKey] IS NOT NULL
					THEN 1 ELSE 0 END
		FROM [bi_mktg_stage].[DimActivity] STG
		INNER JOIN [bi_mktg_stage].[synHC_DDS_DimActivity] DW
			ON STG.SFDC_TaskID = DW.SFDC_TaskID
			AND DW.[RowIsCurrent] = 1
		...
		-----------------------
		WITH Duplicates  AS
			(SELECT  STG.SFDC_TaskID AS 'ActivitySSID'
						, IsDuplicate
