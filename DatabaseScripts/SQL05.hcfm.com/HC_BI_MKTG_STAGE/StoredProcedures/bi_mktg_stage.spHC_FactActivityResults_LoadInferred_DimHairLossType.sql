/* CreateDate: 05/03/2010 12:26:54.680 , ModifyDate: 05/03/2010 12:26:54.680 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivityResults_LoadInferred_DimHairLossType]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [spHC_FactActivityResults_LoadInferred_DimHairLossType] is used to load inferred
-- members to the DimHairLossType table.
--
--
--   exec [bi_mktg_stage].[spHC_FactActivityResults_LoadInferred_DimHairLossType] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_mktg_stage].[synHC_DDS_DimHairLossType]

		------------------------
		-- Add inferred members
		------------------------
		INSERT INTO [bi_mktg_stage].[synHC_DDS_DimHairLossType] (
				 [HairLossTypeSSID]
