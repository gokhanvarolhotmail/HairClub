/* CreateDate: 05/03/2010 12:26:49.343 , ModifyDate: 05/03/2010 12:26:49.343 */
GO
-- Batch submitted through debugger: SQLQuery43.sql|7|0|C:\Users\jwatermolen\AppData\Local\Temp\~vs2439.sql





CREATE PROCEDURE [bi_mktg_stage].[spHC_DQA_DimActivityResult]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimActivityResult] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_mktg_stage].[spHC_DQA_DimActivityResult] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_mktg_stage].[spHC_DQA_DimActivityResult_0000] @DataPkgKey
			END
