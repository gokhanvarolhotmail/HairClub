/* CreateDate: 05/03/2010 12:26:23.550 , ModifyDate: 11/27/2018 15:13:53.937 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DDS_DimEmployee_Upsert]
			   @DataPkgKey					int
...
--
--
--   exec [bi_mktg_stage].[spHC_DDS_DimEmployee_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		------------------------
		DELETE
		FROM [bi_mktg_stage].[synHC_DDS_DimEmployee]
		WHERE [EmployeeSSID] IN ( SELECT STG.[EmployeeSSID]
										FROM [bi_mktg_stage].[DimEmployee] STG
		...
		INSERT INTO [bi_mktg_stage].[synHC_DDS_DimEmployee] (
			...
			SELECT
				...
				, STG.[EmployeeDescription]
				, STG.[EmployeeTitle]
				, ISNULL(STG.[ActionSetCode], '')
				, ISNULL(STG.[EmployeeDepartmentSSID], '')
				, ISNULL(STG.[EmployeeDepartmentDescription], '')
				, ISNULL(STG.[EmployeeJobFunctionSSID], '')
				, ISNULL(STG.[EmployeeJobFunctionDescription], '')
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				...
				, DW.[EmployeeDescription] = STG.[EmployeeDescription]
				, DW.[EmployeeTitle] = STG.[EmployeeTitle]
				, DW.[ActionSetCode] = ISNULL(STG.[ActionSetCode], '')
				, DW.[EmployeeDepartmentSSID] = ISNULL(STG.[EmployeeDepartmentSSID], '')
				, DW.[EmployeeDepartmentDescription] = ISNULL(STG.[EmployeeDepartmentDescription], '')
				, DW.[EmployeeJobFunctionSSID] = ISNULL(STG.[EmployeeJobFunctionSSID], '')
				, DW.[EmployeeJobFunctionDescription] = ISNULL(STG.[EmployeeJobFunctionDescription], '')
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_mktg_stage].[DimEmployee] STG
		JOIN [bi_mktg_stage].[synHC_DDS_DimEmployee] DW ON
