/* CreateDate: 05/03/2010 12:26:29.943 , ModifyDate: 05/03/2010 12:26:29.943 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DimActivity_Cleanup]
       (
			  @DataPkgKey				int
       )
...
AS
-------------------------------------------------------------------------
-- [spHC_DimActivity_Cleanup] is used to Cleanup records
--
--
-- EXEC [spHC_DimActivity_Cleanup] 422
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
  ...
  BEGIN TRY
		...
		WHILE ( 0 = 0 )
			BEGIN
				...
				FROM [bi_mktg_stage].[DimActivity]
				WHERE DataPkgKey = @DataPkgKey
				AND IsLoaded = 1
