/* CreateDate: 05/03/2010 12:26:49.290 , ModifyDate: 05/03/2010 12:26:49.290 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DQA_DimActivityDemographic]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_DQA_DimActivityDemographic] is used to perform the Data Quality
-- check on the data to be loaded into the data warehouse
--
--
--   exec [bi_mktg_stage].[spHC_DQA_DimActivityDemographic] 431
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		IF (@IsDQAInformationLoggingEnabled = 1)
			BEGIN
				...
				-----------------------
				EXEC	@return_value = [bi_mktg_stage].[spHC_DQA_DimActivityDemographic_0000] @DataPkgKey
			END
