/* CreateDate: 08/05/2019 11:23:18.777 , ModifyDate: 11/02/2020 15:31:39.573 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivity_Transform_Lkup_ContactKey]
			   @DataPkgKey					int
...
--			11/02/2020  KMurdoch     Set ContactKey to -1 (Unknown) if SFDC_LeadID is NULL
-------------------------------------------------------------------------
BEGIN
	...
	BEGIN TRY
		...
		LEFT OUTER JOIN [bi_mktg_stage].[synHC_DDS_DimContact] DW
		...
		------------------------
		-- There might be some other load that just added them
		-- Update [ContactKey] Keys in STG - Based on PersonAccountID
		------------------------
		UPDATE STG SET
		     [ContactKey] = COALESCE(DW.[ContactKey], 0)
		FROM [bi_mktg_stage].[FactActivity] STG
		LEFT OUTER JOIN [bi_mktg_stage].[synHC_DDS_DimContact] DW
			ON DW.SFDC_PersonAccountID = STG.SFDC_PersonAccountID
			AND DW.[RowIsCurrent] = 1
		WHERE COALESCE(STG.[ContactKey], 0) = 0
		...
		-----------------------
		-- Fix [ContactSSID] to -1 Unknown if SFDC_LeadID is NULL
		-----------------------
		UPDATE STG SET
		       [ContactKey]  = -1
		     , [ContactSSID] = -2
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage= 'SFDC_LeadID is null - FA_Trans_Lkup'
		FROM [bi_mktg_stage].[FactActivity] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND STG.[SFDC_LeadID] IS NULL
			AND STG.[ContactKey] IS NULL
