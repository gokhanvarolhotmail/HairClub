/* CreateDate: 05/03/2010 12:26:59.580 , ModifyDate: 10/26/2011 09:51:56.197 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivity_Transform_Lkup_ActivityCompletedDateKey]
			   @DataPkgKey					int

AS
-------------------------------------------------------------------------
-- [spHC_FactActivity_Transform_Lkup_ActivityCompletedDateKey] is used to populate
-- the ActivityCompletedDateKey in the FactActivity rows
--
--
--   exec [bi_mktg_stage].[spHC_FactActivity_Transform_Lkup_ActivityCompletedDateKey] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage = 'ActivityCompletedDateSSID is null - FA_Trans_Lkup'
		FROM [bi_mktg_stage].[FactActivity] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND STG.[ActivityCompletedDateSSID] IS NULL
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage = 'ActivityCompletedDateKey is null - FA_Trans_Lkup'
		FROM [bi_mktg_stage].[FactActivity] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
