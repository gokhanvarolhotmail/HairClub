/* CreateDate: 05/03/2010 12:26:23.373 , ModifyDate: 08/05/2019 10:27:10.317 */
GO
CREATE PROCEDURE [bi_mktg_stage].[zzzspHC_DDS_DimActivity_Upsert]
			   @DataPkgKey					int
...
BEGIN
	...
	BEGIN TRY
		...
		------------------------
		--DELETE
		--FROM [bi_mktg_stage].[synHC_DDS_DimActivity]
		--WHERE [ActivitySSID] IN ( SELECT STG.[ActivitySSID]
		--								FROM [bi_mktg_stage].[DimActivity] STG
		...
		UPDATE DW SET
				...
				, DW.[GreenwichOffset] = STG.[GreenwichOffset]
