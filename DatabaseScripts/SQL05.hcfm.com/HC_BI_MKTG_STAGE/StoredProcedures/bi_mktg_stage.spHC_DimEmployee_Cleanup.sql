/* CreateDate: 05/03/2010 12:26:42.730 , ModifyDate: 05/03/2010 12:26:42.730 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DimEmployee_Cleanup]
       (
			  @DataPkgKey				int
       )
...
AS
-------------------------------------------------------------------------
-- [spHC_DimEmployee_Cleanup] is used to Cleanup records
--
--
-- EXEC [spHC_DimEmployee_Cleanup] 422
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
  ...
  BEGIN TRY
		...
		WHILE ( 0 = 0 )
			BEGIN
				...
				FROM [bi_mktg_stage].[DimEmployee]
				WHERE DataPkgKey = @DataPkgKey
				AND IsLoaded = 1
