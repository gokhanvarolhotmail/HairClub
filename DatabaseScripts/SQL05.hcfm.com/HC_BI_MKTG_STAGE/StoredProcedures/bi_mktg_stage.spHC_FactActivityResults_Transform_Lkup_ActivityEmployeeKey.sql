/* CreateDate: 08/09/2011 13:38:50.240 , ModifyDate: 10/26/2011 10:32:44.520 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivityResults_Transform_Lkup_ActivityEmployeeKey]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [sspHC_FactActivityResults_Transform_Lkup_ActivityEmployeeKey] is used to determine
-- the ActivityEmployeeKey foreign key values in the FactActivityResults table using DimEmployee.
--
--
--   exec [bi_mktg_stage].[spHC_FactActivityResults_Transform_Lkup_ActivityEmployeeKey] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------
		EXEC	@return_value = [bi_mktg_stage].[spHC_FactActivityResults_LoadInferred_DimEmployee_ActivityEmployee] @DataPkgKey
		...
		UPDATE STG SET
		     IsException = 1
		     ,	ExceptionMessage = 'ActivityEmployeeSSID IS NULL - FAR_Trans_LKup'
		FROM [bi_mktg_stage].[FactActivityResults] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND STG.[ActivityEmployeeSSID] IS NULL
		...
		UPDATE STG SET
		     IsException = 1
		     ,	ExceptionMessage = 'ActivityEmployeeKey IS NULL - FAR_Trans_LKup'
		FROM [bi_mktg_stage].[FactActivityResults] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND (STG.[ActivityEmployeeKey] IS NULL
