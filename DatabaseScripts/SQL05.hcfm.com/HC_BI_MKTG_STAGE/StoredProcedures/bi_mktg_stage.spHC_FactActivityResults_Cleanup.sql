/* CreateDate: 05/03/2010 12:26:50.420 , ModifyDate: 09/09/2020 20:37:37.723 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivityResults_Cleanup]
			  @DataPkgKey				int


AS
-------------------------------------------------------------------------
-- [spHC_FactActivityResults_Cleanup] is used to Cleanup records
--
--
--   exec [bi_mktg_stage].[spHC_FactActivityResults_Cleanup] 422
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
	...
	BEGIN TRY
		...
		WHILE ( 0 = 0 )
			BEGIN
				...
				FROM [bi_mktg_stage].[FactActivityResults]
				WHERE DataPkgKey = @DataPkgKey
				AND IsLoaded = 1
				AND IsException = 0
