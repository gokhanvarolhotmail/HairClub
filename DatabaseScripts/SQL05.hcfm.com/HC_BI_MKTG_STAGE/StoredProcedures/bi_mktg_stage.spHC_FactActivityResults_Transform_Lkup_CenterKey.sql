/* CreateDate: 05/03/2010 12:26:55.117 , ModifyDate: 01/06/2021 13:00:54.823 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivityResults_Transform_Lkup_CenterKey]
			   @DataPkgKey					int
AS
-------------------------------------------------------------------------
-- [sspHC_FactActivityResults_Transform_Lkup_CenterKey] is used to determine
-- the CenterKey foreign key values in the FactActivityResults table using DimCenter.
--
--
--   exec [bi_mktg_stage].[spHC_FactActivityResults_Transform_Lkup_CenterKey] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
--			09/22/2020  KMurdoch     Removed logic to exclude surgery center and reinstated logic to exclude surgery centers
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------
		EXEC	@return_value = [bi_mktg_stage].[spHC_FactActivityResults_LoadInferred_DimCenter] @DataPkgKey
		...
		UPDATE STG SET
		     IsException = 1
		     ,	ExceptionMessage = 'CenterSSID IS NULL - FAR_Trans_LKup'
		FROM [bi_mktg_stage].[FactActivityResults] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND STG.[CenterSSID] IS NULL
		...
		UPDATE STG SET
		     IsException = 1
		     ,	ExceptionMessage = 'CenterKey IS NULL - FAR_Trans_LKup'
		FROM [bi_mktg_stage].[FactActivityResults] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND (STG.[CenterKey] IS NULL
				OR STG.[CenterKey] = 0)
