/* CreateDate: 02/02/2011 17:41:31.987 , ModifyDate: 02/02/2011 17:41:31.987 */
GO
create PROCEDURE [bi_mktg_stage].[spHC_DDS_DimPromotionCode_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimPromotionCode_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_mktg_stage].[spHC_DDS_DimPromotionCode_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		------------------------
		DELETE
		FROM [bi_mktg_stage].[synHC_DDS_DimPromotionCode]
		WHERE [PromotionCodeSSID] IN ( SELECT STG.[PromotionCodeSSID]
										FROM [bi_mktg_stage].[DimPromotionCode] STG
		...
		FROM [bi_mktg_stage].[DimPromotionCode] STG
		JOIN [bi_mktg_stage].[synHC_DDS_DimPromotionCode] DW ON
