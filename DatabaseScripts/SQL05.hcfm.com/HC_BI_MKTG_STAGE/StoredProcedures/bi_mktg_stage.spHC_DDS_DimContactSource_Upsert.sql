/* CreateDate: 05/03/2010 12:26:23.487 , ModifyDate: 05/18/2010 09:55:12.980 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DDS_DimContactSource_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimContactSource_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_mktg_stage].[spHC_DDS_DimContactSource_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_mktg_stage].[synHC_DDS_DimContactSource]
		...
		------------------------
		DELETE
		FROM [bi_mktg_stage].[synHC_DDS_DimContactSource]
		WHERE [ContactSourceSSID] IN ( SELECT STG.[ContactSourceSSID]
										FROM [bi_mktg_stage].[DimContactSource] STG
		...
		IN (
				...
				FROM [bi_mktg_stage].[synHC_SRC_TBL_MKTG_oncd_contact_source] SRC WITH (NOLOCK)
				)
		...
		-- New Records
		------------------------
		INSERT INTO [bi_mktg_stage].[synHC_DDS_DimContactSource] (
				      [ContactSourceSSID]
				    , [ContactSSID]
			  	    , [SourceCode]
				    , [MediaCode]
		...
		UPDATE DW SET
				...
				, DW.[SourceCode] = STG.[SourceCode]
				, DW.[MediaCode] = STG.[MediaCode]
				, DW.[AssignmentDate] = STG.[AssignmentDate]
				, DW.[AssignmentTime] = STG.[AssignmentTime]
				, DW.[PrimaryFlag] = STG.[PrimaryFlag]
				, DW.[DNIS_Number] = STG.[DNIS_Number]
				, DW.[SubSourceCode] = STG.[SubSourceCode]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_mktg_stage].[DimContactSource] STG
		JOIN [bi_mktg_stage].[synHC_DDS_DimContactSource] DW ON
		...
		FROM [bi_mktg_stage].[DimContactSource] STG
		JOIN [bi_mktg_stage].[synHC_DDS_DimContactSource] DW ON
				DW.[ContactSourceKey] = STG.[ContactSourceKey]
			AND DW.[RowIsCurrent] = 1
		WHERE	STG.[IsType2] = 1
			AND	STG.[IsException] = 0
			AND STG.[DataPkgKey] = @DataPkgKey
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_mktg_stage].[synHC_DDS_DimContactSource]
