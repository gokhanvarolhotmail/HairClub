/* CreateDate: 05/03/2010 12:26:48.793 , ModifyDate: 05/03/2010 12:26:48.793 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DimSalesType_Extract_CDC]
		@DataPkgKey				int
...
AS
-------------------------------------------------------------------------
-- [spHC_DimSalesType_Extract_CDC] is used to retrieve a
-- list SalesType
--
--   exec [bi_mktg_stage].[spHC_DimSalesType_Extract_CDC]  '2009-01-01 01:00:00'
--                                       , '2009-01-02 01:00:00'
--
-------------------------------------------------------------------------
-- Change History
...
BEGIN
	...
	DECLARE		  @TableName			varchar(150)	-- Name of table
 	...
 	SET @CDCTableName = N'dbo_csta_contact_saletype'
			...
			BEGIN
				...
				IF (@From_LSN IS NOT NULL) AND (@To_LSN IS NOT NULL) AND (@From_LSN <> [HCM].[sys].[fn_cdc_increment_lsn](@To_LSN))
					BEGIN
						...
						INSERT INTO [bi_mktg_stage].[DimSalesType]
								   ( [DataPkgKey]
								   , [SalesTypeKey]
								   , [SalesTypeSSID]
						...
						SELECT @DataPkgKey
								, NULL AS [SalesTypeKey]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[saletype_code])),'') AS nvarchar(10)) AS [SalesTypeSSID]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[description])),'') AS nvarchar(50)) AS [SalesTypeDescription]
								, '' AS [SalesTypeDescriptionShort]
								...
								, 0 AS [IsDelete]
								, 0 AS [IsDuplicate]
								, CAST(ISNULL(LTRIM(RTRIM(chg.[saletype_code])),'') AS nvarchar(50)) AS [SourceSystemKey]
								, CASE [__$operation]
										WHEN 1 THEN 'D'
										WHEN 2 THEN 'I'
										WHEN 3 THEN 'UO'
						...
						FROM [HCM].[cdc].[fn_cdc_get_net_changes_dbo_csta_contact_saletype](@From_LSN, @To_LSN, @row_filter_option) chg
