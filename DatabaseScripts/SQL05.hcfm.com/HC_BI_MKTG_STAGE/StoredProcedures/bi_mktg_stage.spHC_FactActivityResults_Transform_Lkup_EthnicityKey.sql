/* CreateDate: 05/03/2010 12:26:55.160 , ModifyDate: 10/26/2011 10:38:36.487 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivityResults_Transform_Lkup_EthnicityKey]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [sspHC_FactActivityResults_Transform_Lkup_EthnicityKey] is used to determine
-- the EthnicityKey foreign key values in the FactActivityResults table using DimEthnicity.
--
--
--   exec [bi_mktg_stage].[spHC_FactActivityResults_Transform_Lkup_EthnicityKey] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------
		EXEC	@return_value = [bi_mktg_stage].[spHC_FactActivityResults_LoadInferred_DimEthnicity] @DataPkgKey
		...
		UPDATE STG SET
		     IsException = 1
		     ,	ExceptionMessage = 'EthnicitySSID IS NULL - FAR_Trans_LKup'
		FROM [bi_mktg_stage].[FactActivityResults] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND STG.[EthnicitySSID] IS NULL
		...
		UPDATE STG SET
		     IsException = 1
		     ,	ExceptionMessage = 'EthnicityKey IS NULL - FAR_Trans_LKup'
		FROM [bi_mktg_stage].[FactActivityResults] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND (STG.[EthnicityKey] IS NULL
