/* CreateDate: 05/03/2010 12:26:25.740 , ModifyDate: 05/03/2010 12:26:25.740 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DimActivityDemographic_Cleanup]
       (
			  @DataPkgKey				int
       )
...
AS
-------------------------------------------------------------------------
-- [spHC_DimActivityDemographic_Cleanup] is used to Cleanup records
--
--
-- EXEC [spHC_DimActivityDemographic_Cleanup] 422
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
  ...
  BEGIN TRY
		...
		WHILE ( 0 = 0 )
			BEGIN
				...
				FROM [bi_mktg_stage].[DimActivityDemographic]
				WHERE DataPkgKey = @DataPkgKey
				AND IsLoaded = 1
