/* CreateDate: 05/03/2010 12:26:23.287 , ModifyDate: 05/06/2010 15:31:45.120 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DDS_DimActionCode_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimActionCode_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_mktg_stage].[spHC_DDS_DimActionCode_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		------------------------
		DELETE
		FROM [bi_mktg_stage].[synHC_DDS_DimActionCode]
		WHERE [ActionCodeSSID] IN ( SELECT STG.[ActionCodeSSID]
										FROM [bi_mktg_stage].[DimActionCode] STG
		...
		-- New Records
		------------------------
		INSERT INTO [bi_mktg_stage].[synHC_DDS_DimActionCode] (
					  [ActionCodeSSID]
			...
			SELECT
				...
				, STG.[ActionCodeDescriptionShort]
				, 1 -- [RowIsCurrent]
		...
		UPDATE DW SET
				...
				, DW.[ActionCodeDescriptionShort] = STG.[ActionCodeDescriptionShort]
				, DW.[RowChangeReason] = 'Updated Inferred Member'
				, DW.[RowIsInferred] = 0
				, DW.[UpdateAuditKey] = @DataPkgKey
		FROM [bi_mktg_stage].[DimActionCode] STG
		JOIN [bi_mktg_stage].[synHC_DDS_DimActionCode] DW ON
