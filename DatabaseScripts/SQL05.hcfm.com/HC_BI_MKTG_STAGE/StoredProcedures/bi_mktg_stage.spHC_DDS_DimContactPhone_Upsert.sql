/* CreateDate: 05/03/2010 12:26:23.463 , ModifyDate: 02/09/2021 14:22:54.980 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_DDS_DimContactPhone_Upsert]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [spHC_DDS_DimContactPhone_Upsert] is used to update
-- SCD Type 1 records, update SCD Type 2 records and insert
-- New records
--
--
--   exec [bi_mktg_stage].[spHC_DDS_DimContactPhone_Upsert] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
--			11/21/2017	KMurdoch	 Added SFDC_Lead/Task/LeadPhone_ID
--			09/10/2020  KMurdoch     Added SFDC_PersonAccountID
--			09/14/2020  KMurdoch     Removed reference to ContactPhoneSSID - replaced with [SFDC_LeadPhoneID]
--			09/15/2020  KMurdoch     Fixed delete for duplicate records.
--			09/17/2020  KMurdoch     Reverted code back for delete where 'not in'
--			02/09/2021  KMurdoch     Added Check for Not Null in "Not In" query
-------------------------------------------------------------------------
BEGIN
	...
	BEGIN TRY
		...
		SELECT @InitialRowCnt = COUNT(1) FROM [bi_mktg_stage].[synHC_DDS_DimContactPhone]
		...
		------------------------
		DELETE
		FROM [bi_mktg_stage].[synHC_DDS_DimContactPhone]
		WHERE [SFDC_LeadPhoneID] IN ( SELECT STG.[SFDC_LeadPhoneID]
										FROM [bi_mktg_stage].[DimContactPhone] STG
		...
		------------------------
		--DELETE dcp
		--FROM [bi_mktg_stage].[synHC_DDS_DimContactPhone]  dcp
		--	INNER JOIN HC_BI_SFDC.dbo.Phone__c Phone__c
		--		ON dcp.SFDC_LeadPhoneID = Phone__c.id
		--WHERE dcp.[ContactPhoneKey] <> -1

		DELETE
		FROM [bi_mktg_stage].[synHC_DDS_DimContactPhone]
		WHERE SFDC_LeadPhoneID IS NOT NULL AND
					(SFDC_LeadPhoneID NOT
						IN (
								SELECT SRC.Id
								FROM HC_BI_SFDC.dbo.Phone__c SRC WITH (NOLOCK)
							)
		...
		-- New Records
		------------------------
		INSERT INTO [bi_mktg_stage].[synHC_DDS_DimContactPhone] (
				      [ContactPhoneSSID]
				    , [ContactSSID]
				    , [PhoneTypeCode]
				    , [CountryCodePrefix]
			...
			SELECT
				      STG.[ContactPhoneSSID]
				    , STG.[ContactSSID]
				    , STG.[PhoneTypeCode]
				    , STG.[CountryCodePrefix]
				    , STG.[AreaCode]
				    , STG.[PhoneNumber]
				    , STG.[Extension]
				    , STG.[Description]
		...
		UPDATE DW SET
				...
				, DW.[PhoneTypeCode] = STG.[PhoneTypeCode]
				, DW.[CountryCodePrefix] = STG.[CountryCodePrefix]
				, DW.[AreaCode] = STG.[AreaCode]
				, DW.[PhoneNumber] = STG.[PhoneNumber]
				, DW.[Extension] = STG.[Extension]
		...
		FROM [bi_mktg_stage].[DimContactPhone] STG
		JOIN [bi_mktg_stage].[synHC_DDS_DimContactPhone] DW ON
				DW.[SFDC_LeadPhoneID] = STG.[SFDC_LeadPhoneID]
		...
		FROM [bi_mktg_stage].[DimContactPhone] STG
		JOIN [bi_mktg_stage].[synHC_DDS_DimContactPhone] DW ON
				DW.[ContactPhoneKey] = STG.[ContactPhoneKey]
			AND DW.[RowIsCurrent] = 1
		WHERE	STG.[IsType2] = 1
			AND	STG.[IsException] = 0
			AND STG.[DataPkgKey] = @DataPkgKey
		...
		SELECT @FinalRowCnt = COUNT(1) FROM [bi_mktg_stage].[synHC_DDS_DimContactPhone]
