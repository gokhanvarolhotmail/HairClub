/* CreateDate: 05/03/2010 12:26:59.560 , ModifyDate: 10/26/2011 09:54:58.530 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivity_Transform_Lkup_ActionCodeKey]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [sspHC_FactActivity_Transform_Lkup_ActionCodeKey] is used to determine
-- the ActionCodeKey foreign key values in the FactActivity table using DimActionCode.
--
--
--   exec [bi_mktg_stage].[spHC_FactActivity_Transform_Lkup_ActionCodeKey] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------
		EXEC	@return_value = [bi_mktg_stage].[spHC_FactActivity_LoadInferred_DimActionCode] @DataPkgKey
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage = 'ActioncodeSSID is null - FA_Trans_Lkup'
		FROM [bi_mktg_stage].[FactActivity] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND STG.[ActionCodeSSID] IS NULL
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage = 'ActioncodeKey is null - FA_Trans_Lkup'
		FROM [bi_mktg_stage].[FactActivity] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND (STG.[ActionCodeKey] IS NULL
