/* CreateDate: 05/03/2010 12:26:59.883 , ModifyDate: 10/26/2011 10:54:04.377 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivity_Transform_Lkup_SourceKey]
			   @DataPkgKey					int
...
AS
-------------------------------------------------------------------------
-- [sspHC_FactActivity_Transform_Lkup_SourceKey] is used to determine
-- the SourceKey foreign key values in the FactActivity table using DimSource.
--
--
--   exec [bi_mktg_stage].[spHC_FactActivity_Transform_Lkup_SourceKey] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------
		EXEC	@return_value = [bi_mktg_stage].[spHC_FactActivity_LoadInferred_DimSource] @DataPkgKey
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1,ExceptionMessage = 'SourceSSID is null - FA_Trans_Lkup'
		FROM [bi_mktg_stage].[FactActivity] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND STG.[SourceSSID] IS NULL
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage = 'SourceKey is null - FA_Trans_Lkup'
		FROM [bi_mktg_stage].[FactActivity] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND (STG.[SourceKey] IS NULL
				OR STG.[SourceKey] = 0)
