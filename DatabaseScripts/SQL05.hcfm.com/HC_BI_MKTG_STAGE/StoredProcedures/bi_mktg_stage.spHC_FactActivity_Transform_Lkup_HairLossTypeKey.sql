/* CreateDate: 05/03/2010 12:26:59.817 , ModifyDate: 10/27/2011 17:29:56.260 */
GO
CREATE PROCEDURE [bi_mktg_stage].[spHC_FactActivity_Transform_Lkup_HairLossTypeKey]
			   @DataPkgKey					int


AS
-------------------------------------------------------------------------
-- [sspHC_FactActivity_Transform_Lkup_HairLossTypeKey] is used to determine
-- the HairLossTypeKey foreign key values in the FactActivity table using DimHairLossType.
--
--
--   exec [bi_mktg_stage].[spHC_FactActivity_Transform_Lkup_HairLossTypeKey] -1
--
-------------------------------------------------------------------------
-- Change History
-------------------------------------------------------------------------
...
--			02/08/2011				 Fix bug...wrong table name in update (FactLead)
--			10/26/2011  KMurdoch	 Added Exception message
--			10/26/2011  KMurdoch	 Fixed zero records. - last rec 885
-------------------------------------------------------------------------
BEGIN
...
---Added this code to prevent addition of zero records krm ek 10/26/2011
		...
		EXEC	@return_value = [bi_mktg_stage].[spHC_FactActivity_LoadInferred_DimHairLossType] @DataPkgKey
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage= 'HairLossTypeSSID is null - FA_Trans_Lkup'
		FROM [bi_mktg_stage].[FactActivity] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND STG.[HairLossTypeSSID] IS NULL
		...
		-----------------------
		UPDATE STG SET
		     IsException = 1, ExceptionMessage= 'HairLossTyepKey is null - FA_Trans_Lkup'
		FROM [bi_mktg_stage].[FactActivity] STG
		WHERE STG.[DataPkgKey] = @DataPkgKey
			AND (STG.[HairLossTypeKey] IS NULL
		...
		--SELECT @FinalRowCnt = COUNT(1) FROM [bi_mktg_stage].[synHC_DDS_DimHairLossType]
