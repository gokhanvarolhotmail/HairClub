/* CreateDate: 02/23/2022 11:31:47.630 , ModifyDate: 02/23/2022 11:31:52.413 */
GO
/***********************************************************************
...
***********************************************************************/
CREATE PROCEDURE [dbo].[spRpt_ClosingByConsultantDetailsNetSales_V2]
    @CenterType INT
  , @Center     INT
  , @EmployeeID INT
  , @StartDate  DATETIME
...
CREATE TABLE [#CenterEmployees] ( [CenterNumber] INT, [EmployeeKey] INT, [EmployeeFullName] VARCHAR(102)) ;
...
IF( @CenterType = 1 AND LEN(@Center) > 2 )
    BEGIN
        ...
        WHERE [DCT].[CenterTypeDescriptionShort] = 'C' AND [DC].[Active] = 'Y' AND ( [DC].[CenterNumber] = @Center OR @Center = 0 )
        OPTION( RECOMPILE ) ;
    END ;
ELSE IF( @CenterType = 2 AND LEN(@Center) > 2 )
    BEGIN
        INSERT INTO [#Centers]
        SELECT
            [DR].[RegionSSID]
        ...
        WHERE [DCT].[CenterTypeDescriptionShort] IN ('F', 'JV') AND [DC].[Active] = 'Y' AND ( [DC].[CenterNumber] = @Center OR @Center = 0 )
        OPTION( RECOMPILE ) ;
    END ;
ELSE IF( @CenterType = 1 AND @Center BETWEEN ( -2 ) AND 25 ) -- An Area has been selected.
    BEGIN
        ...
        OPTION( RECOMPILE ) ;
    END ;
ELSE IF( @CenterType = 2 AND @Center BETWEEN ( -2 ) AND 15 ) -- A Region has been selected.
    BEGIN
        INSERT INTO [#Centers]
        SELECT
            [DR].[RegionSSID]
...
ELSE IF( LEN(@CenterType) > 2 ) --A center has been selected
    BEGIN
        ...
        FROM [HC_BI_ENT_DDS].[bi_ent_dds].[DimCenter] AS [DC]
        INNER JOIN [HC_BI_ENT_DDS].[bi_ent_dds].[DimCenterType] AS [DCT] ON [DC].[CenterTypeKey] = [DCT].[CenterTypeKey]
        WHERE [DC].[CenterNumber] = @CenterType AND [DCT].[CenterTypeDescriptionShort] IN ('C', 'F', 'JV') AND [DC].[Active] = 'Y'
        OPTION( RECOMPILE ) ;
    END ;
...
UNION
SELECT
    [DEC].[CenterID] AS [CenterNumber]
  , [E].[EmployeeKey]
  , [E].[EmployeeFullName]
FROM [HC_BI_CMS_DDS].[bi_cms_dds].[DimEmployee] AS [E]
INNER JOIN [SQL05].[HairClubCMS].[dbo].[datEmployee] AS [DE] ON [DE].[EmployeeGUID] = [E].[EmployeeSSID]
INNER JOIN [SQL05].[HairClubCMS].[dbo].[datEmployeeCenter] AS [DEC] --Colorado Springs is 1020 as the CenterID in this table
    ON [DEC].[EmployeeGUID] = [DE].[EmployeeGUID]
INNER JOIN [#Centers] AS [C] ON [C].[CenterSSID] = [DEC].[CenterID]
OPTION( RECOMPILE ) ;

IF @EmployeeID <> 0
    BEGIN
        DELETE [CE]
        FROM [#CenterEmployees] AS [CE]
...
SELECT DISTINCT
     ...
     , CASE WHEN [SC].[SalesCodeDescriptionShort] IN ('MEMPMTTRD', 'MEMPMTGRD', 'MEMPMTEXT', 'MEMPMTXTRD') THEN
         LEFT([SC].[SalesCodeDescription], CHARINDEX('(', [SC].[SalesCodeDescription]) - 1)ELSE [SC].[SalesCodeDescription] END AS [SalesCodeDescription]
     , CASE WHEN [FST].[S_SurCnt] <> 0 THEN [FST].[S_SurCnt] WHEN [FST].[S_PRPCnt] <> 0 THEN [FST].[S_PRPCnt] ELSE [FST].[Quantity] END AS [Quantity]
     , [CM].[ClientMembershipContractPrice]
     , ISNULL([FST].[NB_TradCnt], 0) + ISNULL([FST].[NB_ExtCnt], 0) + ISNULL([FST].[NB_XTRCnt], 0) + ISNULL([FST].[NB_GradCnt], 0) + ISNULL([FST].[S_SurCnt], 0)
       + ISNULL([FST].[S_PostExtCnt], 0) + ISNULL([FST].[NB_MDPCnt], 0) + ISNULL([FST].[S_PRPCnt], 0) AS [NetNB1Count]
     , ISNULL([FST].[NB_TradAmt], 0) + ISNULL([FST].[NB_ExtAmt], 0) + ISNULL([FST].[NB_XTRAmt], 0) + ISNULL([FST].[NB_GradAmt], 0)
       + ISNULL([FST].[S_PostExtAmt], 0) + ISNULL([FST].[NB_MDPAmt], 0) + ISNULL([FST].[NB_LaserAmt], 0) + ISNULL([FST].[S_SurAmt], 0)
       + ISNULL([FST].[S_PRPAmt], 0) AS [NetNB1Sales]
...
INTO [#NetSales]
FROM [HC_BI_CMS_DDS].[bi_cms_dds].[FactSalesTransaction] AS [FST] WITH( NOLOCK )
INNER JOIN [HC_BI_ENT_DDS].[bief_dds].[DimDate] AS [DD] WITH( NOLOCK )ON [FST].[OrderDateKey] = [DD].[DateKey]
INNER JOIN [HC_BI_CMS_DDS].[bi_cms_dds].[DimSalesCode] AS [SC] WITH( NOLOCK )ON [FST].[SalesCodeKey] = [SC].[SalesCodeKey]
INNER JOIN [HC_BI_CMS_DDS].[bi_cms_dds].[DimSalesOrder] AS [SO] WITH( NOLOCK )ON [FST].[SalesOrderKey] = [SO].[SalesOrderKey]
INNER JOIN [HC_BI_CMS_DDS].[bi_cms_dds].[DimSalesOrderDetail] AS [SOD] WITH( NOLOCK )ON [FST].[SalesOrderDetailKey] = [SOD].[SalesOrderDetailKey]
INNER JOIN [HC_BI_CMS_DDS].[bi_cms_dds].[DimClientMembership] AS [CM] WITH( NOLOCK )ON [SO].[ClientMembershipKey] = [CM].[ClientMembershipKey]
INNER JOIN [HC_BI_CMS_DDS].[bi_cms_dds].[DimMembership] AS [m] WITH( NOLOCK )ON [CM].[MembershipKey] = [m].[MembershipKey]
INNER JOIN [HC_BI_ENT_DDS].[bi_ent_dds].[DimCenter] AS [C] WITH( NOLOCK )ON [CM].[CenterKey] = [C].[CenterKey]
INNER JOIN [#Centers] AS [CTR] WITH( NOLOCK )
    --ON C.CenterNumber = #Centers.CenterNumber
    ON [C].[CenterSSID] = [CTR].[CenterSSID]
INNER JOIN [HC_BI_CMS_DDS].[bi_cms_dds].[DimClient] AS [CLT] WITH( NOLOCK )ON [FST].[ClientKey] = [CLT].[ClientKey]
LEFT JOIN [#CenterEmployees] AS [DE] WITH( NOLOCK )ON [DE].[EmployeeKey] = [FST].[Employee1Key]
WHERE [DD].[FullDate] BETWEEN @StartDate AND @EndDate
  AND [SC].[SalesCodeKey] NOT IN (665, 654, 393, 668, 660, 661, 1703, 1704, 1705, 1706, 1707, 1708, 1709, 1710, 1711)
  AND [SOD].[IsVoidedFlag] = 0
  AND ( [DE].[EmployeeKey] = @EmployeeID OR @EmployeeID = 0 OR ISNULL(@EmployeeID, '') = '' OR @EmployeeID = -1 )
OPTION( RECOMPILE ) ;

--accomodate unknown employee
...
SELECT DISTINCT
     ...
     , CASE WHEN [NS].[Quantity] = 1 THEN 1 WHEN [NS].[Quantity] = -1 THEN -1 ELSE 1 END AS [Quantity1] --NS.Quantity
     , [NS].[Quantity]
