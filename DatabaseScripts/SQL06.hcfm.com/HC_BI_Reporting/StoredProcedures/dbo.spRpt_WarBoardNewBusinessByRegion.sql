/* CreateDate: 09/13/2012 11:43:04.910 , ModifyDate: 03/19/2020 10:20:05.913 */
GO
SET ANSI_NULLS ON
GO
...
PROCEDURE:				spRpt_WarBoardNewBusinessByRegion
...
04/14/2014 - RH - Changed Budget to CAST(Budget AS DECIMAL(10,0))
...
10/28/2014 - RH - Added SUM(ISNULL(CASE WHEN FA.AccountID IN (10206) THEN Flash ELSE 0 END, 0)) AS 'NetXtrCount';
				Added NetXtrCount to dbo.DIVIDE_DECIMAL((NetTradCount + NetGradCount), (NetTradCount + NetXtrCount + NetGradCount + NetEXTCount + NetSurCount)) AS 'HairSalesMixA';
...
01/28/2020 - RH - (TrackIT 5467) Added SelfGen weighting, cap, quotas - sales count made from referral consultations as defined in the NB Flash; changed @Closing_Weighting to .20; removed Corpus Christie (396)
...
==============================================================================
SAMPLE EXECUTION:
EXEC [spRpt_WarBoardNewBusinessByRegion] 2, 2020
==============================================================================
*/
CREATE PROCEDURE [dbo].[spRpt_WarBoardNewBusinessByRegion] (
	@Month			TINYINT
,	@Year			SMALLINT)
AS
BEGIN
...
--Find @StartDate and @EndDate to use to find MDP
...
SELECT  #Centers.CenterManagementAreaSSID
,	 #Centers.CenterManagementAreaDescription
,	SUM(ISNULL(#Centers.SelfGenQuota,0)) AS SelfGenQuota
,	SUM(ISNULL(NB1.NetNBCount,0)) AS NetNBCount
,	SUM(ISNULL(Consultations,0)) AS Consultations
...
FROM #Accounting ACCT
	...
	INNER JOIN #Centers
		ON #Centers.CenterNumber = NB1.CenterNumber
GROUP BY #Centers.CenterManagementAreaSSID
,	 #Centers.CenterManagementAreaDescription

/********************************** Get referrals and SelfGenCnt ****************************************************/
SELECT CenterManagementAreaDescription
,	CenterManagementAreaSSID
,	SUM(q.Referral) AS Referral
,	SUM(q.SelfGenCnt) AS SelfGenCnt
INTO #Referrals
FROM
		(SELECT #Centers.CenterManagementAreaSSID
		,	#Centers.CenterManagementAreaDescription
...
GROUP BY q.CenterManagementAreaDescription
,         q.CenterManagementAreaSSID

/********** Find HairSalesMixPercent, CashPercent and other percentages **************************************************************************************************************/

SELECT #Base.CenterManagementAreaDescription
,	#Base.CenterManagementAreaSSID
,	NetNBCount
,	Consultations
,	NetNBCount_Budget
...
LEFT JOIN #Referrals REF
	ON #Base.CenterManagementAreaSSID = REF.CenterManagementAreaSSID


/**************** Find weightings *******************************************************************************************************************************/

SELECT  f.CenterManagementAreaDescription
,	f.CenterManagementAreaSSID
,	f.NetNBCount
,	f.Consultations
,	f.NetNBCount_Budget
,	f.NetNBCountPercent
