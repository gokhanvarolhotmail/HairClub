/* CreateDate: 10/23/2015 13:46:43.230 , ModifyDate: 10/08/2020 10:42:58.447 */
GO
SET ANSI_NULLS ON
GO
...
10/08/2020 - KM  -(TrackIT 14050) - Changed Varchar(3) to Varchar in SQL; allowed for 4 character centerIDs to work in select Statement.
------------------------------------------------------------------------
SAMPLE EXECUTION:

EXEC spRpt_NewBusinessPipeline '1080', '1'
...
SELECT @SQL = 'SELECT * FROM OPENQUERY(SQL05, ''SELECT A.ClientGUID, A.AppointmentGUID, AE.EmployeeGUID,
												...
												AND (SalesCodeDescriptionShort LIKE ''''CK%''''
														...
														AND CenterID =  ''''' +  CAST(@CenterID AS VARCHAR)+ ''''''') '
