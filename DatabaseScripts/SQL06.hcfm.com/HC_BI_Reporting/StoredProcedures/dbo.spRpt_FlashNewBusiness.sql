...
BEGIN
    ...
    SET @PartitionDate = (SELECT CAST((CAST(MONTH(@begdt) AS VARCHAR(2)) + CAST('/1/' AS VARCHAR(3)) +
                                       CAST(YEAR(@begdt) AS VARCHAR(4))) AS DATETIME))
    ...
    Select t.CenterNumber,
           ...
           CASE
               WHEN ISNULL(t.Result__c, '') IN ('Completed', 'Show Sale', 'Show No Sale') AND
                    ...
                    ISNULL(t.Action__c, '') in ('Appointment', 'In House', 'Recovery'
                    ,'be back'
                 )
                   THEN 1
...
--10233 - NB - Net Sales (Incl PEXT) $
    ...
    SELECT DISTINCT B.CenterNumber
                  , B.PartitionDate
...
/********************************** Display By Main Group/Center *************************************/
    ...
    SELECT CASE
               WHEN CT.CenterTypeDescriptionShort = 'C' THEN 'Corporate'
               WHEN CT.CenterTypeDescriptionShort = 'HW' THEN 'Total Hair Solutions'
               ELSE 'Franchise' END                                                                    AS [TYPE]
         , CASE
               WHEN CT.CenterTypeDescriptionShort = 'C' THEN 1
               WHEN CT.CenterTypeDescriptionShort = 'HW' THEN 3
               ELSE 2 END                                                                              AS TypeID
         , C.MainGroupID
         , C.MainGroup
         ...
         , dbo.DIVIDE_DECIMAL(ISNULL(S.NetNB1Sales, 0),
                              ISNULL(S.NetNB1Count, 0))                                                AS [per_nb1_revenue]
         , dbo.DIVIDE_DECIMAL(ISNULL(s.NetNB1Count, 0),
                              (ISNULL(o_C.Consultations, 0) - ISNULL(o_B.BeBacks, 0)))        AS close_pct
         , dbo.DIVIDE((ISNULL(S.NetTradCount, 0) + ISNULL(S.NetGradCount, 0)),
                      (ISNULL(S.NetTradCount, 0) + ISNULL(S.NetGradCount, 0) + ISNULL(S.NetEXTCount, 0) +
                       ISNULL(S.PostEXTCount, 0) + ISNULL(S.NetXtrCount, 0) + ISNULL(S.NB_MDPCnt, 0))) AS per_Bio
         , dbo.DIVIDE((ISNULL(S.NetEXTCount, 0) + ISNULL(S.PostEXTCount, 0)),
                      (ISNULL(S.NetTradCount, 0) + ISNULL(S.NetGradCount, 0) + ISNULL(S.NetEXTCount, 0) +
                       ISNULL(S.PostEXTCount, 0) + ISNULL(S.NetXtrCount, 0) + ISNULL(S.NB_MDPCnt, 0))) AS per_EXT
         , dbo.DIVIDE((ISNULL(S.SurgeryCount, 0)),
                      (ISNULL(S.NetTradCount, 0) + ISNULL(S.NetGradCount, 0) + ISNULL(S.NetEXTCount, 0) +
                       ISNULL(S.PostEXTCount, 0) + ISNULL(S.NetXtrCount, 0) + ISNULL(S.SurgeryCount, 0) +
                       ISNULL(S.NB_MDPCnt, 0)))                                                        AS per_Sur
         , dbo.DIVIDE(ISNULL(S.NetXtrCount, 0),
                      (ISNULL(S.NetTradCount, 0) + ISNULL(S.NetGradCount, 0) + ISNULL(S.NetEXTCount, 0) +
                       ISNULL(S.PostEXTCount, 0) + ISNULL(S.SurgeryCount, 0) + ISNULL(S.NetXtrCount, 0) +
                       ISNULL(S.NB_MDPCnt, 0)))                                                        AS per_Xtr
         , dbo.DIVIDE(ISNULL(S.NB_MDPCnt, 0),
                      (ISNULL(S.NetTradCount, 0) + ISNULL(S.NetGradCount, 0) + ISNULL(S.NetEXTCount, 0) +
                       ISNULL(S.PostEXTCount, 0) + ISNULL(S.SurgeryCount, 0) + ISNULL(S.NetXtrCount, 0) +
                       ISNULL(S.NB_MDPCnt, 0)))                                                        AS per_MDP
    ...
    FROM #Centers C
        ...
        WHERE cons.CenterNumber = c.CenterNumber
             ...
             OUTER APPLY (
        SELECT SUM(CASE WHEN bb.BeBacks = 1 THEN 1 ELSE 0 END) AS 'BeBacks'
        FROM #Bebacks bb
