/* CreateDate: 05/07/2018 14:52:16.010 , ModifyDate: 07/06/2020 13:47:07.933 */
GO
SET ANSI_NULLS ON
GO
...
05/18/2020 - KM - Fixed Budget number for NB - RestorINK # from 1500 to 10901
------------------------------------------------------------------------
SAMPLE EXECUTION:
...
INSERT	INTO #Center
		...
		WHERE   ctr.CenterSSID = @CenterNumber
				AND ctr.Active = 'Y'
...
INSERT	INTO #Budget
		...
		,		SUM(ISNULL(CASE WHEN fa.AccountID IN ( 10901 ) THEN fa.Budget ELSE 0 END, 0)) AS 'Budget_MDPSales'
		...
		WHERE	fa.PartitionDate BETWEEN @StartDate AND @EndDate
				AND fa.AccountID IN ( 10205, 10305, 10210, 10310, 10215, 10315, 10206, 10306, 10220, 10320, 10225, 10325, 10901, 10240, 10235, 10552, 10891 )
		GROUP BY c.CenterNumber
		,		c.CenterDescriptionNumber
		,		c.CenterTypeDescription
		,		fa.PartitionDate
