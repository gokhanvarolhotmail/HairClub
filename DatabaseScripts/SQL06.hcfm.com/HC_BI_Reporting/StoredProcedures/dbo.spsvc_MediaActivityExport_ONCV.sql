/* CreateDate: 09/26/2012 09:30:26.350 , ModifyDate: 02/18/2019 14:42:14.347 */
GO
/***********************************************************************
...
REVISIONS:	WJB 03/11/08 Added Sale Type to export
			...
			KRM 02/18/19 Changed the number of days to go back to -1 instead of -3
------------------------------------------------------------------------
SAMPLE EXECUTION:
EXEC [spsvc_MediaActivityExport_ONCV]
EXEC spsvc_MediaActivityExport_ONCV '9/4/2017', '10/29/2017'
***********************************************************************/
...
--SELECT DATEADD(dd, -2024,CAST(CONVERT(VARCHAR, GETDATE(), 10) AS DATETIME))
        ...
        --SET @EndDt = '10/29/2017'
			...
			WHERE dd.FullDate BETWEEN @BegDt AND @EndDt
					...
					AND ISNULL(dc.ContactStatusSSID, 'LEAD') IN ( 'LEAD', 'CLIENT' )
