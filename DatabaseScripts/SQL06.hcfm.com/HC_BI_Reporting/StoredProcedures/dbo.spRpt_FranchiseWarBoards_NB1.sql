...
-- ----------------------------------------------------------------------------------------------
Sample Execution:
EXEC [spRpt_FranchiseWarBoards_NB1] 7, 2018
***********************************************************************/
...
BEGIN
	...
	--INNER JOIN HC_BI_ENT_DDS.bi_ent_dds.DimCenter ce ON ce.CenterNumber = a.CenterID
	--INNER JOIN HC_BI_ENT_DDS.bi_ent_dds.DimCenter de ON de.centerNumber = ce.CenterNumber
	INNER JOIN HC_BI_ENT_DDS.bi_ent_dds.DimCenter de ON de.centerNumber = a.CenterID
	INNER JOIN HC_BI_ENT_DDS.bi_ent_dds.DimRegion re ON re.RegionKey = de.RegionKey
	INNER JOIN HC_BI_ENT_DDS.[bi_ent_dds].[DimCenterType] CT on ct.CenterTypeSSID = de.CenterTypeSSID

	WHERE d.MonthNumber = @Month
		AND d.YearNumber = @Year
		--AND ce.CenterTypeSSID IN (2,3)
		--AND ce.Active = 'Y'
		--AND de.CenterTypeSSID IN (2,3)
		AND CT.CenterTypeDescriptionShort in ('F', 'JV')
		AND de.Active = 'Y'
	GROUP BY re.RegionDescription,de.CenterNumber,de.CenterDescriptionNumber, CT.CenterTypeDescriptionShort


	-- Get Dollar per Sale numbers.
	SELECT
		--ce.ReportingCenterSSID  Center_Num
		ce.CenterNumber Center_Num
	,	ABS(dbo.Divide(
	...
	INNER JOIN HC_Accounting.dbo.FactAccounting a ON a.CenterID = ce.CenterNumber
		INNER JOIN HC_BI_ENT_DDS.bief_dds.DimDate d ON d.DateKey = a.DateKey
	WHERE d.MonthNumber = @Month
		AND d.YearNumber = @Year
		AND ce.CenterTypeSSID IN (2,3)
	...
	WHERE d.MonthNumber = @Month
		...
		AND ce.CenterTypeSSID IN (2,3)
		AND ce.Active = 'Y'
	GROUP BY ce.CenterNumber --ce.ReportingCenterSSID
