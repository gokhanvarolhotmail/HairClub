/* CreateDate: 03/19/2014 14:58:28.067 , ModifyDate: 06/28/2021 16:19:42.930 */
GO
SET ANSI_NULLS ON
GO
...
-- Get Centers
    ...
    SELECT CASE
               ...
               ELSE ISNULL(cma.CenterManagementAreaSSID, 0) END                  AS 'AreaID'
         , CASE
               WHEN ct.CenterTypeDescriptionShort IN ('JV', 'F') THEN r.RegionDescription
               ELSE ISNULL(cma.CenterManagementAreaDescription, 'Corporate') END AS 'Area'
         , ctr.CenterSSID
         , ctr.CenterNumber
...
/********************************** Get Lead Data *************************************/
    ...
    WHERE d.FullDate BETWEEN @MinDate AND @MaxDate
      ...
      AND /*case
              when (isvalid is null)
                  then ISNULL(fil.IsInvalidLead, 0) = 0
              else (isvalid = 1)
        end*/
       (IsValid = 1 or ISNULL(fil.IsInvalidLead, 0) = 0)
    GROUP BY c.Center
           , c.CenterNumber
           , c.CenterDescription
           , c.CenterType
