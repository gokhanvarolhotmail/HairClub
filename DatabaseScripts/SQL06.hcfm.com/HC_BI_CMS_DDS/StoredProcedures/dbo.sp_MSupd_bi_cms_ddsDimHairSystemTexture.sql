/* CreateDate: 03/17/2022 11:57:06.257 , ModifyDate: 03/17/2022 11:57:06.257 */
GO
...
update [bi_cms_dds].[DimHairSystemTexture] set
		[HairSystemTextureSSID] = case substring(@bitmap,1,1) & 2 when 2 then @c2 else [HairSystemTextureSSID] end,
		[HairSystemTextureDescription] = case substring(@bitmap,1,1) & 4 when 4 then @c3 else [HairSystemTextureDescription] end,
		[HairSystemTextureDescriptionShort] = case substring(@bitmap,1,1) & 8 when 8 then @c4 else [HairSystemTextureDescriptionShort] end,
		[HairSystemTextureSortOrder] = case substring(@bitmap,1,1) & 16 when 16 then @c5 else [HairSystemTextureSortOrder] end,
	...
	where [HairSystemTextureKey] = @pkc1
if @@rowcount = 0
    if @@microsoftversion>0x07320000
		Begin
			...
			Begin
				...
				set @primarykey_text = @primarykey_text + '[HairSystemTextureKey] = ' + convert(nvarchar(100),@pkc1,1)
				exec sp_MSreplraiserror @errorid=20598, @param1=N'[bi_cms_dds].[DimHairSystemTexture]', @param2=@primarykey_text, @param3=13233
			End
			Else
