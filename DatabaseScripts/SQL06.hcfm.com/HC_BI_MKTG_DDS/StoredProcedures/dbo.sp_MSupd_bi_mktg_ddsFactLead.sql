...
begin
update [bi_mktg_dds].[FactLead] set
		...
		[NewBusinessRevenue] = case substring(@bitmap,4,1) & 32 when 32 then @c30 else [NewBusinessRevenue] end,
		[PCPMemberRevenue] = case substring(@bitmap,4,1) & 64 when 64 then @c31 else [PCPMemberRevenue] end,
		[PCPLaserRevenue] = case substring(@bitmap,4,1) & 128 when 128 then @c32 else [PCPLaserRevenue] end,
		[RetailRevenue] = case substring(@bitmap,5,1) & 1 when 1 then @c33 else [RetailRevenue] end,
		[ServiceRevenue] = case substring(@bitmap,5,1) & 2 when 2 then @c34 else [ServiceRevenue] end,
		[NBLaserRevenue] = case substring(@bitmap,5,1) & 4 when 4 then @c35 else [NBLaserRevenue] end
    ...
    if @@microsoftversion>0x07320000
		Begin
			...
			Begin
				...
				exec sp_MSreplraiserror @errorid=20598, @param1=N'[bi_mktg_dds].[FactLead]', @param2=@primarykey_text, @param3=13233
			End
			Else
