/* CreateDate: 04/10/2014 12:17:02.000 , ModifyDate: 03/01/2017 12:09:47.787 */
GO
SET ANSI_NULLS ON
GO
...
FROM    [HC_BI_CMS_DDS].[bi_cms_dds].[FactSalesTransaction] t
		...
		--LEFT OUTER JOIN [HCSQL2\SQL2005].INFOSTORE.dbo.Transactions Tr
		--	ON Tr.Center = ce.CenterSSID
		--		AND Tr.client_no = cl.ClientNumber_Temp
		--		AND Tr.transact_no = sod.TransactionNumber_Temp
WHERE   d.FullDate >= @StartDate
		AND ( ce.CenterOwnershipSSID = 2
				OR clhce.CenterOwnershipSSID = 2 )
		AND so.IsVoidedFlag <> 1
ORDER BY [ClientCenterSSID]
,		[OrderDate]
,       [TicketNumber]
