...
INSERT	INTO #ClientCount
		SELECT	dd.FullDate
		,		FST.ClientKey
		,		CASE WHEN (SUM(ISNULL(FST.NB_TradCnt, 0)) + SUM(ISNULL(FST.NB_GradCnt, 0)) + SUM(ISNULL(FST.NB_ExtCnt, 0)) + SUM(ISNULL(FST.S_PostExtCnt, 0))
						+ SUM(ISNULL(FST.NB_XTRCnt, 0)) + SUM(ISNULL(FST.S_SurCnt, 0)) + SUM(ISNULL(FST.NB_MDPCnt, 0)) + SUM(ISNULL(FST.S_PRPCnt, 0))
						) < 0 THEN (-1)
					WHEN (SUM(ISNULL(FST.NB_TradCnt, 0)) + SUM(ISNULL(FST.NB_GradCnt, 0)) + SUM(ISNULL(FST.NB_ExtCnt, 0)) + SUM(ISNULL(FST.S_PostExtCnt, 0))
						+ SUM(ISNULL(FST.NB_XTRCnt, 0)) + SUM(ISNULL(FST.S_SurCnt, 0)) + SUM(ISNULL(FST.NB_MDPCnt, 0)) + SUM(ISNULL(FST.S_PRPCnt, 0))
						) = 0 THEN 0
					ELSE 1
				END AS ClientCount
...
INSERT	INTO #Final
		...
		,		(SELECT ISNULL(SUM(NetGrad6Count), 0) FROM #Sales s WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate) AS 'NetGrad6Count'
		,		(SELECT ISNULL(SUM(NetGrad6Sales), 0) FROM #Sales s WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate) AS 'NetGrad6Sales'
		,		(SELECT ISNULL(SUM(NetGrad12Count), 0) FROM #Sales s WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate) AS 'NetGrad12Count'
		,		(SELECT ISNULL(SUM(NetGrad12Sales), 0) FROM #Sales s WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate) AS 'NetGrad12Sales'
		,		(SELECT ISNULL(SUM(NetGrad6EZPAYCount), 0) FROM #Sales s WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate) AS 'NetGrad6EZPAYCount'
		,		(SELECT ISNULL(SUM(NetGrad6EZPAYSales), 0) FROM #Sales s WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate) AS 'NetGrad6EZPAYSales'
		...
		,		(SELECT ISNULL(SUM(s.NetEXTInitialCount), 0) FROM #Sales s WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate) AS 'NetEXTInitalCount'
		,		(SELECT ISNULL(SUM(NetEXTInitialSales), 0) FROM #Sales s WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate) AS 'NetEXTInitalSales'
		...
		,		(SELECT ISNULL(SUM(EXTExpirations), 0) FROM #EXTExpirations e WHERE e.FullDate BETWEEN d.StartDate AND d.EndDate) AS 'EXTExpirations'
		...
		,		(SELECT ISNULL(SUM(ClientCount), 0) FROM #ClientCount cc WHERE cc.FullDate BETWEEN d.StartDate AND d.EndDate) AS 'ClientCount'
		FROM    #Date d
