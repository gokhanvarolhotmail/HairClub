...
-- Get Centers
INSERT  INTO #Centers
		SELECT	CASE WHEN ct.CenterTypeDescriptionShort IN ( 'JV', 'F' ) THEN r.RegionSSID ELSE ISNULL(cma.CenterManagementAreaSSID, 0) END AS 'MainGroupID'
		,		CASE WHEN ct.CenterTypeDescriptionShort IN ( 'JV', 'F' ) THEN r.RegionDescription ELSE ISNULL(cma.CenterManagementAreaDescription, 'Corporate') END AS 'MainGroup'
		,		CASE WHEN ct.CenterTypeDescriptionShort IN ( 'JV', 'F' ) THEN r.RegionSortOrder ELSE cma.CenterManagementAreaSortOrder END
		,		ctr.CenterSSID
		,		ctr.CenterNumber
...
,		F.Leads
,		F.Consultations
,		CASE WHEN SUBSTRING(F.CenterDescription, 1, 2) = 'ZZ' THEN REPLACE(F.CenterDescription, 'ZZ', '')
