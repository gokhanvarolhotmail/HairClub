/* CreateDate: 09/03/2015 16:35:24.220 , ModifyDate: 12/30/2015 09:55:51.703 */
GO
SET ANSI_NULLS ON
GO
...
GO
/***********************************************************************
PROCEDURE:				spRpt_PresidentsClubNBDetails
...
EXEC spRpt_PresidentsClubNBDetails 17, '8/1/2015', '8/31/2015', 230, NULL

EXEC spRpt_PresidentsClubNBDetails 17, '8/1/2015', '8/31/2015', 230, 'Heller, Dina'

***********************************************************************/
CREATE PROCEDURE [dbo].[xxx_spRpt_PresidentsClubNBDetails]
(
	@type INT
,	@StartDate DATETIME
,	@EndDate DATETIME
...
BEGIN
	SELECT  C.CenterSSID AS 'Center'
		...
		,		ISNULL(E.EmployeeInitials, E2.EmployeeInitials) AS 'performer'
	...
	WHERE   DD.FullDate BETWEEN @StartDate AND @EndDate
				...
				AND ( @Performer IS NULL OR E.EmployeeSSID = @EmployeeSSID )
END
...
BEGIN
	...
	FROM    HC_BI_CMS_DDS.bi_cms_dds.FactSalesTransaction FST
				...
				LEFT OUTER JOIN HC_BI_CMS_DDS.bi_cms_dds.DimMembership PREVM
					ON PREVCM.MembershipSSID = PREVM.MembershipSSID
	WHERE   DD.FullDate BETWEEN @StartDate AND @EndDate
				AND ( FST.NB_TradCnt <> 0 OR FST.NB_GradCnt <> 0 OR FST.NB_ExtCnt <> 0 OR FST.NB_XtrCnt <> 0)
				AND SC.SalesCodeKey NOT IN ( 665, 654, 393, 668 )
				AND SOD.IsVoidedFlag = 0
				AND ( @Performer IS NULL OR E.EmployeeSSID = @EmployeeSSID )
END
...
BEGIN
	...
	FROM    HC_BI_CMS_DDS.bi_cms_dds.FactSalesTransaction FST
				...
				LEFT OUTER JOIN HC_BI_CMS_DDS.bi_cms_dds.DimMembership PREVM
					ON PREVCM.MembershipSSID = PREVM.MembershipSSID
	WHERE   DD.FullDate BETWEEN @StartDate AND @EndDate
				AND ( FST.NB_TradAmt <> 0 OR FST.NB_GradAmt <> 0 OR FST.NB_ExtAmt <> 0 OR FST.NB_XtrAmt <> 0)
				AND SC.SalesCodeKey NOT IN ( 665, 654, 393, 668 )
				AND SOD.IsVoidedFlag = 0
				AND ( @Performer IS NULL OR E.EmployeeSSID = @EmployeeSSID )
END
