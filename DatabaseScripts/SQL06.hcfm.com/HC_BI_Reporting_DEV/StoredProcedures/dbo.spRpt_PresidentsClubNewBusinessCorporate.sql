/* CreateDate: 03/16/2017 16:10:11.337 , ModifyDate: 12/19/2018 09:23:03.467 */
GO
/***********************************************************************
...
EXEC [spRpt_PresidentsClubNewBusinessCorporate] 2, '11/1/2017', '11/30/2017'
...
SELECT q.CenterKey
,	q.EmployeeKey
,	SUM(ISNULL(q.Quantity,0)) AS 'Quantity'
FROM (SELECT DD.FullDate
		...
		LEFT JOIN HC_BI_CMS_DDS.bi_cms_dds.DimEmployee E
			ON (FST.Employee1Key = E.EmployeeKey AND E.CenterSSID = #Centers.CenterSSID)
		WHERE DD.FullDate BETWEEN @BeginningOfThePreviousYear AND @EndDate
...
,		R.EmployeeKey
,		ISNULL(R.NB_AppsCnt,0) AS 'NB_AppsCnt'
...
WHERE (ISNULL(R.HairSalesCount,0) <> 0 OR ISNULL(R.Consultations,0) <> 0 OR ISNULL(NetNB1Count,0)  <> 0 OR ISNULL(NetNB1Sales,0) <> 0 OR ISNULL(SWA.Quantity,0) <> 0)
