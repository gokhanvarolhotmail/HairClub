/* CreateDate: 05/22/2018 15:44:37.893 , ModifyDate: 05/30/2018 15:57:07.570 */
GO
/******************************************************************************************************************************
PROCEDURE:				[spRpt_SalesKPIReview_RB_YTD]
...
EXEC [spRpt_SalesKPIReview_RB_YTD] 201,5,2018

--********************************************************************************************************************************/
CREATE PROCEDURE [dbo].[spRpt_SalesKPIReview_RB_YTD]
(
	@CenterNumber INT
,	@Month INT
,	@Year INT
...
IF (MONTH(GETDATE()) = @Month AND YEAR(GETDATE()) = @Year)   --There will be no Closing PCP for the current month, so go back to December for Opening PCP
BEGIN
...
FROM
	...
	INNER JOIN #JanPreviousPCP
		ON #JanPreviousPCP.CenterSSID = #RB.CenterSSID
	INNER JOIN #PCPRevenue
		ON #JanPreviousPCP.CenterSSID = #PCPRevenue.CenterSSID
	INNER JOIN #PCPRevenue_Budget
		ON #JanPreviousPCP.CenterSSID = #PCPRevenue_Budget.CenterID
	LEFT JOIN #Match
		ON #RB.CenterSSID = #Match.CenterID
	LEFT OUTER JOIN #RetailSales RS
		ON #RB.CenterSSID = RS.CenterSSID
	LEFT OUTER JOIN #ClosedPCP
		ON #ClosedPCP.CenterSSID = #RB.CenterSSID
	)q
