/* CreateDate: 02/11/2022 11:41:56.147 , ModifyDate: 02/15/2022 16:23:13.460 */
GO
/*
==============================================================================

PROCEDURE:				[spRpt_KPISummary_test]
...
==============================================================================
*/
CREATE PROCEDURE [dbo].[spRpt_KPISummary_test] (
	@Filter	INT
,	@Year	INT
,	@Timeframe INT
,	@Time	INT
...
INTO	#Retail
		...
		WHERE   d.FullDate BETWEEN @StartDate AND @EndDate
				AND (SCD.SalesCodeDivisionSSID IN ( 30, 50 )
				AND SC.SalesCodeDepartmentSSID <> 3065)
...
,	CASE WHEN SUM(ISNULL(CASE WHEN FA.AccountID IN (10305,10306,10310,10315,10320,10321,10325,10552,10891) THEN ABS(Budget) ELSE 0 END, 0)) = 0
		THEN 1 ELSE SUM(ISNULL(CASE WHEN FA.AccountID IN (10305,10306,10310,10315,10320,10321,10325,10552,10891) THEN ABS(Budget) ELSE 0 END, 0)) END AS 'NetNb1RevenueBudget'
...
,	CASE WHEN SUM(ISNULL(CASE WHEN FA.AccountID IN (3015,10305,10306,10310,10315,10320,10321,10325,10536,10540,10551,10552,10555,10575,10891) THEN Flash ELSE 0 END, 0)) = 0
		THEN 0 ELSE SUM(ISNULL(CASE WHEN FA.AccountID IN (3015,10305,10306,10310,10315,10320,10321,10325,10536,10540,10551,10552,10555,10575,10891) THEN Flash ELSE 0 END, 0))
		END AS 'SubTotalRevenueActual'  --Retail (10555) is not included here

,	NULL AS 'TotalRevenueActual'
,	CASE WHEN SUM(ISNULL(CASE WHEN FA.AccountID IN (3015,10321,10305,10306,10310,10315,10320,10325,10536,10540,10551,10552,10555,10575,10891) THEN ABS(Budget) ELSE 0 END, 0)) = 0
		THEN 1 ELSE SUM(ISNULL(CASE WHEN FA.AccountID IN (3015,10321,10305,10306,10310,10315,10320,10325,10536,10540,10551,10552,10555,10575,10891) THEN ABS(Budget) ELSE 0 END, 0)) END AS 'TotalRevenueBudget' --Retail (10555) is here
