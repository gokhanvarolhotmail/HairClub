/* CreateDate: 10/04/2018 16:59:21.823 , ModifyDate: 01/10/2020 16:41:32.343 */
GO
/*
==============================================================================
...
06/11/2018 - RH - (#147216) Added Retail, @RetailToBudget_Weighting; @NBTradGradToBudget_Cap = 1.50;@NBTradGradToBudget_Weighting = .20; Changed CenterSSID to CenterNumber; Removed Retention
07/05/2018 - RH - (#151550) Corrected dates
...
EXEC spRpt_WarBoardManagerChart  12, 2019

==============================================================================
*/
CREATE PROCEDURE [dbo].[spRpt_WarBoardManagerChart] (
	@Month INT
,	@Year INT
)
AS
...
,	SUM(ISNULL(CASE WHEN FA.AccountID IN (10433) THEN ROUND(Flash,0) ELSE 0 END, 0)) AS 'XtrConversions'
...
,	SUM(ISNULL(CASE WHEN FA.AccountID IN (10435) THEN ROUND(Flash,0) ELSE 0 END, 0)) AS 'EXTConversions'
...
,	CASE WHEN SUM(ISNULL(CASE WHEN FA.AccountID IN(3090,3096)  THEN ROUND(ABS(Budget),0) ELSE 0 END, 0))= 0 THEN 1
		ELSE SUM(ISNULL(CASE WHEN FA.AccountID IN(3090,3096) THEN ROUND(ABS(Budget),0) ELSE 0 END, 0))END AS 'Retail_Budget' --Retail amount is found separately below
...
IF OBJECT_ID('tempdb..#FirstFive') IS NOT NULL
BEGIN
	DROP TABLE #FirstFive
END
SELECT CenterNumber
...
,	PartitionDate
,	Total
,	ROW_NUMBER() OVER(PARTITION BY RecurringBusinessSize ORDER BY Total DESC) AS 'TopFive'
INTO #FirstFive
FROM #Rank
...
SELECT CenterNumber,
       ...
       PartitionDate,
       Total,
       TopFive FROM #FirstFive
WHERE TopFive IN(1,2,3,4,5)
ORDER BY RecurringBusinessSize
