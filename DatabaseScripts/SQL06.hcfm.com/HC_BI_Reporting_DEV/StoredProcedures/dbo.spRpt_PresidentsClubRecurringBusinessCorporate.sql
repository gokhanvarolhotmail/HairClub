/* CreateDate: 03/15/2017 17:04:52.703 , ModifyDate: 04/04/2017 15:52:31.640 */
GO
SET ANSI_NULLS ON
GO
...
EXEC spRpt_PresidentsClubRecurringBusinessCorporate 2, '1/1/2017', '4/1/2017'
...
SELECT @JanuaryStartDate = CAST('2/1/' + CAST(YEAR(@StartDate) AS NVARCHAR(4)) AS DATETIME)
...
,	@LastYrPCPEndDate = DATEADD(YEAR,-1,@DecemberEndDate)


--PRINT '@JanuaryStartDate = ' + CAST(@JanuaryStartDate AS VARCHAR(12))
--PRINT '@JanuaryEndDate = ' + CAST(@JanuaryEndDate AS VARCHAR(12))
--PRINT '@DecemberEndDate = ' + CAST(@DecemberEndDate AS VARCHAR(12))
--PRINT '@LastYrPCPStartDate = ' + CAST(@LastYrPCPStartDate AS VARCHAR(12))
--PRINT '@LastYrPCPEndDate = ' + CAST(@LastYrPCPEndDate AS VARCHAR(12))
...
SELECT  b.CenterID AS 'CenterSSID'
...
FROM    #Centers C
	...
	LEFT OUTER JOIN #JanuaryOpenPCP janPCP
		ON C.CenterSSID = janPCP.CenterSSID
	LEFT OUTER JOIN #JanuaryClosePCP janPCPclose
		ON C.CenterSSID = janPCPclose.CenterSSID
