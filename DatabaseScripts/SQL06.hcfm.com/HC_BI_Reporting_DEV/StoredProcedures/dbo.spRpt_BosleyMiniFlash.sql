...
--CREATE TABLE #BoSConsultationAtHCSourceCode (
...
INSERT  INTO #SourceCode
		...
		WHERE	( ds.SourceSSID LIKE 'BOS%REF%'
					AND ds.SourceSSID <> 'BOSOHREF' )
...
--				AND irl.ProcessName = 'ProcedureDone'
--				AND irl.ProcedureStatus = 'Done'
--				--AND l.Source_Code_Legacy__c  NOT IN (SELECT DISTINCT SourceSSID FROM #SourceCode)
--				AND sc.SourceSSID IS NULL
--		GROUP BY CAST(irl.ProcedureDate AS DATE)
...
INSERT  INTO #NonSurgery
		...
		--,       SUM(ISNULL(fst.NB_TradCnt, 0) + ISNULL(fst.NB_GradCnt, 0) + ISNULL(fst.NB_ExtCnt, 0) + ISNULL(fst.NB_XTRCnt, 0) ) AS 'NetCount'
		--,       SUM(ISNULL(fst.NB_TradAmt, 0) + ISNULL(fst.NB_GradAmt, 0) + ISNULL(fst.NB_ExtAmt, 0) + ISNULL(fst.NB_XTRAmt, 0) ) AS 'NetSales'
