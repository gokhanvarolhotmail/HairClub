...
------------------------------------------------------------------------
SAMPLE EXECUTION:
EXEC [spRpt_PresidentsClubStylist] 8, 2019
...
/************** Declare and set variables for dates ********************************************************/
...
/************** Declare and set variables for weightings ********************************************************/

DECLARE @GrowthWeight FLOAT
DECLARE @PMCWeight FLOAT
DECLARE @RetailPerClientWeight FLOAT

SET @PMCWeight = .333						--one third
SET @RetailPerClientWeight = .333			--one third
SET @GrowthWeight = .333					--one third
...
FROM #Centers C
	...
	LEFT OUTER JOIN #PMC PMC
		ON C.CenterNumber = PMC.CenterNumber AND EmpID.Employee2Key = PMC.Employee2Key
...
,       BIOopenPCP
,       BIOClosePCP
,		(BIOClosePCP - BIOopenPCP) AS Growth
,       RetailSales
,       ClientServicedCnt
,       RetailPerClient
,       StartDate
,       EndDate
,		(@GrowthWeight * (BIOClosePCP - BIOopenPCP)) + (@PMCWeight * Upgrades) + (@RetailPerClientWeight * RetailPerClient) AS Total
FROM #Stylist
GROUP BY (BIOClosePCP - BIOopenPCP)
,         (@GrowthWeight * (BIOClosePCP - BIOopenPCP)) + (@PMCWeight * Upgrades) + (@RetailPerClientWeight * RetailPerClient)
,         CenterNumber
,         CenterManagementAreaSSID
,         CenterManagementAreaDescription
,         CenterDescriptionNumber
