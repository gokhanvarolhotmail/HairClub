...
EXEC spSVC_CommissionTypeID_11_Step03_ValidateDetails
***********************************************************************/
CREATE PROCEDURE [dbo].[spSVC_CommissionTypeID_11_Step03_ValidateDetails]
AS
BEGIN
            ...
            WHILE @CurrentDetailsCount <= @TotalDetailsCount
                  BEGIN
                        ...
                        IF @CurrentDetailsSalesCodeKey IN ( 469, 503, 1697, 1701 )
                           ...
                           AND @CurrentDetailsOrderDateDate > DATEADD(YEAR, 1, ISNULL(@CurrentSurgeryPerformedDate, GETDATE()))
                           BEGIN
                                 UPDATE FactCommissionDetail
                                 SET    IsValidTransaction = 0
                        ...
                        WHERE   CommissionDetailKey = @CurrentCommissionDetailKey
                                ...
                                AND @CurrentDetailsSalesCodeKey IN ( 469, 503, 1697, 1701 )
                                AND @CurrentIsRefund = 1
