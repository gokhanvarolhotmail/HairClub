/****** Object:  View [dbo].[LeadWithoutCampaignPhone]    Script Date: 3/15/2022 2:11:56 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE VIEW [dbo].[LeadWithoutCampaignPhone]
AS WITH LeadWCampMem AS (
	...
	FROM [ODS].[SF_Lead] a --100
	INNER JOIN [ODS].[BP_CallDetail] b
		ON TRIM(REPLACE(REPLACE(REPLACE(REPLACE(A.Phone, '(','') ,')',''), ' ', ''), '-','')) = substring([from_phone],2,len([from_phone]))
