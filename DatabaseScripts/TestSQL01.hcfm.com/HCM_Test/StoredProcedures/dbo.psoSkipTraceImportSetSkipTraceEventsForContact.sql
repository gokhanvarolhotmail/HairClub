/* CreateDate: 12/04/2015 15:43:38.013 , ModifyDate: 12/04/2015 15:43:38.157 */
GO
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[psoSkipTraceImportSetSkipTraceEventsForContact]
...
BEGIN
	...
	IF @process_email_flag = 'Y' AND NOT EXISTS (SELECT 1 FROM oncd_contact_email WHERE contact_id = @contact_id AND cst_valid_flag = 'Y')
	UPDATE cstd_contact_skip_trace_events SET
		next_address_export_date = CASE WHEN @email_next_vendor_delay_days IS NULL OR @email_next_vendor_code IS NULL THEN NULL ELSE DATEADD(d, @email_next_vendor_delay_days, @begin_today) END,
		next_address_export_vendor_code = @email_next_vendor_code,
		next_phone_export_date = CASE WHEN @email_next_vendor_delay_days IS NULL OR @email_next_vendor_code IS NULL THEN NULL ELSE DATEADD(d, @email_next_vendor_delay_days, @begin_today) END,
		next_phone_export_vendor_code = @email_next_vendor_code,
	...
	IF @process_phone_flag = 'Y' AND NOT EXISTS (SELECT 1 FROM oncd_contact_phone WHERE contact_id = @contact_id AND cst_valid_flag = 'Y')
	UPDATE cstd_contact_skip_trace_events SET
		next_address_export_date = CASE WHEN @phone_next_vendor_delay_days IS NULL OR @phone_next_vendor_code IS NULL THEN NULL ELSE DATEADD(d, @phone_next_vendor_delay_days, @begin_today) END,
		next_address_export_vendor_code = @phone_next_vendor_code,
		...
		next_email_export_date = CASE WHEN @phone_next_vendor_delay_days IS NULL OR @phone_next_vendor_code IS NULL THEN NULL ELSE DATEADD(d, @phone_next_vendor_delay_days, @begin_today) END,
		next_email_export_vendor_code = @phone_next_vendor_code
		WHERE contact_id = @contact_id

	IF @process_address_flag = 'Y' AND NOT EXISTS (SELECT 1 FROM oncd_contact_address WHERE contact_id = @contact_id AND cst_valid_flag = 'Y')
	UPDATE cstd_contact_skip_trace_events SET
		...
		next_phone_export_date = CASE WHEN @address_next_vendor_delay_days IS NULL OR @address_next_vendor_code IS NULL THEN NULL ELSE DATEADD(d, @address_next_vendor_delay_days, @begin_today) END,
		next_phone_export_vendor_code = @address_next_vendor_code,
		next_email_export_date = CASE WHEN @address_next_vendor_delay_days IS NULL OR @address_next_vendor_code IS NULL THEN NULL ELSE DATEADD(d, @address_next_vendor_delay_days, @begin_today) END,
		next_email_export_vendor_code = @address_next_vendor_code
		WHERE contact_id = @contact_id
