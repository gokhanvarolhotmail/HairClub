/* CreateDate: 06/20/2012 11:33:12.263 , ModifyDate: 06/21/2012 12:45:10.500 */
GO
-- =============================================
-- Create date: 18 June 2012
-- Description:	Refreshes the data for a single contact.
-- =============================================
CREATE PROCEDURE [dbo].[pso_RefreshEmailDHContact]
	@ContactId CHAR(10)
AS
BEGIN
	...
	SET @Message = 'Processing Contact: ' + @ContactId
	...
	WHERE
		cstd_email_dh_flat.contact_contact_id = @ContactId AND
		oncd_contact_email.contact_id = @ContactId AND
	...
	WHERE
		cstd_email_dh_flat.contact_contact_id = @ContactId AND
		oncd_contact_address.contact_id = @ContactId AND
	...
	WHERE
		cstd_email_dh_flat.contact_contact_id = @ContactId AND
		oncd_contact_company.contact_id = @ContactId AND
	...
	WHERE
		cstd_email_dh_flat.contact_contact_id = @ContactId AND
		cstd_email_dh_appointment_activity.contact_id = @ContactId
	...
	WHERE
		cstd_email_dh_flat.contact_contact_id = @ContactId AND
		cstd_email_dh_brochure_activity.contact_id = @ContactId
	...
	WHERE
		cstd_email_dh_flat.contact_contact_id = @ContactId AND
		cstd_email_dh_activity_demographic.contact_id = @ContactId
	...
	WHERE
		cstd_email_dh_flat.contact_contact_id = @ContactId AND
		oncd_contact_source.contact_id = @ContactId AND
	...
	WHERE
		cstd_email_dh_flat.contact_contact_id = @ContactId AND
		cstd_email_dh_contact_completion.contact_id = @ContactId
