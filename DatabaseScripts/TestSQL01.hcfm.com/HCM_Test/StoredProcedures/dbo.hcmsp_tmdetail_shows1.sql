...
ca.state_code state,
ca.zip_code zip,
'(' + RTRIM(cp.area_code) + ')' + ' ' + substring(cp.phone_number,1,3) + '-' + substring(cp.phone_number,4,4) as phone,
...
WHERE a.due_date BETWEEN @begdt AND @enddt AND dbo.ISSHOW(result_code)=1
