/* CreateDate: 10/02/2017 19:58:07.030 , ModifyDate: 10/30/2017 11:54:53.423 */
GO
/*
==============================================================================
...
DESCRIPTION:	Gathers all the data for the Salesforce lead object
				that has been updated but not loaded to salesforce
==============================================================================
NOTES:
...
==============================================================================
SAMPLE EXECUTION:
EXEC dbo.oncd_contact_salesforce_leads_LeadsUpdated
==============================================================================
*/
CREATE PROCEDURE [dbo].[oncd_lead_salesforce_diffs_LeadsUpdated]
AS
BEGIN

    SET NOCOUNT ON;

    -- Get all the leads that have been updated
    CREATE TABLE #temp
    (
    ...
    SELECT @updated_date = DATEADD(MINUTE, -5, SDM.updated_date)
    ...
    WHERE C.updated_date > @updated_date
          AND C.contact_id NOT IN (
                                      SELECT OCSM.contact_id FROM dbo.oncd_contact_salesforce_mapping AS OCSM
                                  )
    ...
    WHERE OAC.creation_date > @updated_date;
