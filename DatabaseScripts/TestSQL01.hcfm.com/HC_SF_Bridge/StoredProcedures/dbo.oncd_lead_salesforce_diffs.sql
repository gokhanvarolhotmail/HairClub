/* CreateDate: 10/27/2017 09:57:13.070 , ModifyDate: 11/17/2017 11:26:52.220 */
GO
/*
==============================================================================
...
DESCRIPTION:	Gathers all the data for the Salesforce lead object
				that has not been inserted yet using the salesforce
				mapping table based on the create date for the last records inserted into salesforce
==============================================================================
NOTES:
...
==============================================================================
*/
CREATE PROCEDURE [dbo].[oncd_lead_salesforce_diffs]
AS
BEGIN
    ...
    SELECT @creation_date = DATEADD(MINUTE, -20, SDM.creation_date)
    ...
    WHERE C.creation_date > @creation_date
          AND C.contact_id NOT IN (
                                      SELECT HSL.contact_id FROM dbo.HCM_SFDC_Lead AS HSL
                                  )
    ...
    SELECT C.contact_id,
           ...
           CASE
               WHEN OU.full_name IS NOT NULL THEN
                   dbo.fn_capitalize(RTRIM(OU.full_name))
               ELSE
                   dbo.fn_capitalize(RTRIM(OU.first_name)) + ' ' + dbo.fn_capitalize(RTRIM(OU.last_name))
           END AS UserName
    FROM #ContactAvtivity AS CA
        ...
        LEFT JOIN HCM.dbo.onca_user AS OU
