/* CreateDate: 04/07/2021 13:57:01.447 , ModifyDate: 04/07/2021 14:19:17.863 */
GO
/***********************************************************************
...
NOTES:
	...
	04/06/2021  KMurdoch    Added Underscore and Dash in first second of email address
------------------------------------------------------------------------
...
SELECT * FROM dbo.fnIsInvalidLead('00Q1L00001dojDdUAI')
SELECT * FROM dbo.fnIsInvalidLead('00Q1L00001dojDdUAI')
***********************************************************************/
CREATE FUNCTION [dbo].[fnIsInvalidLeadOLD]
(
...
BEGIN
    ...
    WHERE l.Id = @SFDC_LeadID
          ...
          (
              ...
              (
                  (
                      ISNULL(ec.Name, l.Email)NOT LIKE '%[-A-Z0-9_][@][A-Z0-9]%[.][A-Z0-9]%'
                      OR ISNULL(ec.Name, l.Email) LIKE '%@hcfm.com'
                      OR ISNULL(ec.Name, l.Email) LIKE '%hairclub.com'
                      OR ISNULL(ec.Name, l.Email) LIKE '%@test.com'
