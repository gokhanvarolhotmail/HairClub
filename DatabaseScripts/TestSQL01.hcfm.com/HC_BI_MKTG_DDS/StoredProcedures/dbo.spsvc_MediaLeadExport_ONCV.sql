/* CreateDate: 08/03/2012 12:14:18.167 , ModifyDate: 08/08/2012 14:15:12.390 */
GO
/***********************************************************************
...
SAMPLE EXECUTION: EXEC spsvc_IMSLeadExport_ONCV
							EXEC spsvc_IMSLeadExport_ONCV '8/2/2012', '8/2/2012'
...
--set @EndDt = null
        IF ( @BegDt IS NULL
            ...
            BEGIN
                SET @BegDt = DATEADD(dd, -1,
                                     CAST(CONVERT(VARCHAR, GETDATE(), 10) AS DATETIME))
                SET @EndDt = DATEADD(dd, -1,
                                     CAST(CONVERT(VARCHAR, GETDATE(), 10) AS DATETIME)) + ' 23:59:59'
            END
		...
		WHERE
			c.creation_date BETWEEN @BegDt AND @EndDt
