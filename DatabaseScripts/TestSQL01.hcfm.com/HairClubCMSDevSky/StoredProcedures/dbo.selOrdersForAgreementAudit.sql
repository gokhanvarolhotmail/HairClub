...
--------------------------------------------------------------------------------------------------------
NOTES: 	Return Agreement Audit detail

		*** When Updating this proc you have to update rptFeeExceptions too.

		02/17/2014 - MVT: Created Stored Proc
		01/19/2015 - MVT: Modified to include additional fields per TFS #3979

--------------------------------------------------------------------------------------------------------

SAMPLE EXECUTION:

selOrdersForAgreementAudit 201, '2/1/2014', '2/1/2014', 0, 1

***********************************************************************/
CREATE PROCEDURE [dbo].[selOrdersForAgreementAudit]
	@CenterId as int,
...
BEGIN
	...
	WHERE ((@IncludeAllCorpCentersOnly = 1 AND cbt.CenterBusinessTypeDescriptionShort = 'cONEctCorp')
		OR (@IncludeAllCorpCentersOnly = 0 AND  so.CenterId = @CenterId))
