/* CreateDate: 10/14/2011 12:05:55.807 , ModifyDate: 05/04/2020 10:41:01.620 */
GO
/***********************************************************************
PROCEDURE:				selFactoryXMLExportForPO
DESTINATION SERVER:		SQL01
DESTINATION DATABASE: 	HairClubCMS
RELATED APPLICATION:  	CMS
AUTHOR: 				Mike Maass
IMPLEMENTOR: 			Mike Maass
DATE IMPLEMENTED: 		 07/17/2011
LAST REVISION DATE: 	 05/24/2011
--------------------------------------------------------------------------------------------------------
NOTES:  This script is used during the allocation process (SSIS package) to retrieve Factory Orders to
		create XML Files to be 'picked up' by the Factory Web Service.
		* 07/17/2011 MLM - Created stored proc
		* 10/05/2011 MVT - Updated to include Hair System Order MEA data
		* 10/31/2011 MVT - Added Lace Length and Bleach knots to the export
		* 03/06/2012 HDu - Added PoType for Factory App
		* 03/07/2012 HDu - Only join MEA data on Orders with MEA Design Template *this was a change I wrote
							01/20/2012 but failed to be included in the last major release*
		* 06/04/2012 HDu - Added IsFashionHairlineHighlightsFlag to the export
		* 03/27/2019 EJP - Removed BleachKnot from export.  Added IsSignatureHairlineAddOn,
							IsExtendedLaceAddOn, IsOmbreAddOn, and IsLongHairAddOn to the export (TFS #12179)
		* 04/08/2020 SAL - Added IsCuticleIntactHairAddOn, IsRootShadowingAddOn, RootShadowingRootColorLength,
							and RootShadowingRootColor to the export (TFS #14082 and #14036)
--------------------------------------------------------------------------------------------------------
SAMPLE EXECUTION:
EXEC selFactoryXMLExportForPO '50703020-2530-4868-9262-E59B78EFCC2B'
***********************************************************************/

CREATE   PROCEDURE [dbo].[selFactoryXMLExportForPO] (
...
BEGIN
	...
	SELECT CONVERT(varchar(50),hso.HairSystemOrderNumber) as HairSystemOrderNumber
	...
	,	CONVERT(text, dbo.fnGetNotesForHairSystemOrder(hso.HairSystemOrderGUID)) as factnotes  -- Factory Notes
	,	CONVERT(varchar(10),laceLen.HairSystemFrontalLaceLengthDescriptionShort) as laceLengthCode  -- Lace Length Description Short

		-- Include the MEA in the XML export
	,	CASE WHEN mea.HairSystemOrderMeasurementGUID IS NOT NULL THEN 'TRUE' ELSE 'FALSE' END AS [DoesMeasurementExist]
	...
	,	hso.IsExtendedLaceAddOn
	,	hso.IsOmbreAddOn
	,	hso.IsLongHairAddOn
	,	hso.IsCuticleIntactHairAddOn
	,	hso.IsRootShadowingAddOn
	,	CONVERT(varchar(10),rsrcl.RootShadowingRootColorLengthDescriptionShort) as RootShadowingRootColorLength
	,	CONVERT(varchar(100),rsrcl.RootShadowingRootColorLengthDescription) as RootShadowingRootColorLengthDesc
	,	CONVERT(varchar(10),rsrchc.HairSystemHairColorDescriptionShort) as RootShadowingRootColor
	...
	FROM datPurchaseOrderDetail pd
		...
		-- Root Shadowing
		LEFT JOIN lkpRootShadowingRootColorLength rsrcl ON hso.RootShadowingRootColorLengthID = rsrcl.RootShadowingRootColorLengthID
		LEFT JOIN lkpHairSystemHairColor rsrchc ON hso.RootShadowingRootColorID = rsrchc.HairSystemHairColorID
