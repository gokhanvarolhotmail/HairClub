/* CreateDate: 05/05/2014 17:18:47.180 , ModifyDate: 01/25/2020 11:21:18.343 */
GO
SET ANSI_NULLS ON
GO
...
GO
/***********************************************************************
PROCEDURE:				spSVC_CommissionTypeID_30_Step06_RetractCommission
...
EXEC spSVC_CommissionTypeID_30_Step06_RetractCommission
***********************************************************************/
CREATE PROCEDURE [dbo].[spSVC_CommissionTypeID_30_Step06_RetractCommission]
AS
BEGIN
...
-- Refunds must be within 90 days of the original sale
INSERT	INTO #Refunds
		...
		,		DATEDIFF(DAY, oc.SalesOrderDate, so.OrderDate)
		...
		WHERE	ctr.CenterNumber LIKE '[2]%'
				...
				AND ( fst.NB_XTRCnt < 0 OR fst.NB_XTRAmt < 0 )
...
WHERE	r.RefundSalesOrderDetailKey <> -1
...
UPDATE	r
SET		r.CommissionHeaderKey = x_R.CommissionHeaderKey
FROM	#Refunds r
		CROSS APPLY (
			SELECT	r2.CommissionHeaderKey
			FROM	#Refunds r2
			WHERE	r2.ClientKey = r.ClientKey
					AND r2.CommissionHeaderKey IS NOT NULL
		) x_R
WHERE	r.CommissionHeaderKey IS NULL
...
WHERE	fcd.CommissionDetailKey IS NULL
		AND r.CommissionHeaderKey IS NOT NULL
