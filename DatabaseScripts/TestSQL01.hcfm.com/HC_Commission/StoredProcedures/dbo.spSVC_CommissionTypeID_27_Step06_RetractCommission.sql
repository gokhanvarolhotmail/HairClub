/* CreateDate: 02/26/2013 13:20:00.357 , ModifyDate: 05/22/2019 14:41:07.960 */
GO
/***********************************************************************
PROCEDURE:				spSVC_CommissionTypeID_27_Step06_RetractCommission
...
EXEC spSVC_CommissionTypeID_27_Step06_RetractCommission
***********************************************************************/
CREATE PROCEDURE [dbo].[spSVC_CommissionTypeID_27_Step06_RetractCommission]
AS
BEGIN
            ...
            ELSE
				BEGIN
					--If @CurrentTotalPaidCommission <> 0, the commission was paid.  Update all values accordingly
						...
						BEGIN
							UPDATE FactCommissionHeader
							SET    CalculatedCommission = CASE WHEN @CurrentTotalPaidCommission <= 0 THEN 0 ELSE ISNULL(@CurrentTotalPaidCommission, 0) * -1 END
							,      AdvancedCommission = CASE WHEN @CurrentTotalPaidCommission <= 0 THEN 0 ELSE ISNULL(@CurrentTotalPaidCommission, 0) * -1 END
							,      AdvancedCommissionDate = @CurrentRetractionTransactionDate
							,      AdvancedPayPeriodKey = @CurrentPayPeriodKey
							,      UpdateDate = GETDATE()
							,      UpdateUser = OBJECT_NAME(@@PROCID)
							WHERE  CommissionHeaderKey = @CurrentCommissionHeaderKey
						END
					ELSE IF @CurrentRefundTransactionDate IS NULL AND @CurrentCancelTransactionDate IS NOT NULL --Client was cancelled, update the cancellation commission to negative.
						BEGIN
							UPDATE FactCommissionHeader
							SET    CalculatedCommission = CASE WHEN @CurrentTotalPaidCommission <= 0 THEN 0 ELSE ISNULL(@CurrentTotalPaidCommission, 0) * -1 END
							,      AdvancedCommission = CASE WHEN @CurrentTotalPaidCommission <= 0 THEN 0 ELSE ISNULL(@CurrentTotalPaidCommission, 0) * -1 END
							...
							WHERE  CommissionHeaderKey = @CurrentCommissionHeaderKey
						END
					ELSE IF @CurrentRefundTransactionDate IS NOT NULL AND @CurrentCancelTransactionDate IS NOT NULL AND @CurrentSalesCodeKey IN ( 471, 632 ) --Client was refunded AND cancelled, update the cancellation commission to null.
						BEGIN
							UPDATE FactCommissionHeader
