/* CreateDate: 12/31/2010 13:21:04.670 , ModifyDate: 02/27/2017 09:49:15.790 */
GO
SET ANSI_NULLS ON
GO
...
==============================================================================
*/
CREATE PROCEDURE [dbo].[dbaClientMembershipHistoricalImport-SAVE]
      @CenterID int = NULL
AS
BEGIN
      ...
      Select NEWID()
            ,*
      FROM (
      SELECT Distinct trans.Client_no as Client_no
            ,trans.Center as Center
            ...
            ,ISNULL(g.GenderID, 3) AS GenderID
            ,cp.[Bday]
            ,cp.[DoNotCall]
      ...
      FROM [HCSQL2\SQL2005].Infostore.dbo.Transactions  trans
            ...
            INNER JOIN bosproduction.dbo.FOClients0707 on trans.Center = FOClients0707.center and
                        trans.client_no = FOClients0707.client_no
            LEFT OUTER JOIN dbo.lkpState s on cp.[State] = s.StateDescriptionShort
            LEFT OUTER JOIN dbo.cfgCenter ctr ON trans.[Center] = ctr.CenterID
            INNER JOIN dbo.lkpGender g ON cp.Gender = g.GenderDescriptionShort
      WHERE trans.[Date] >= @StartDate
            AND (@CenterID IS NULL OR trans.Center = @CenterID)) results
      ...
      DECLARE @InsertMembership bit,
            ...
            SET @Client_Cursor = CURSOR FAST_FORWARD FOR
                  ...
                  WHERE (trans.ticket_no = trans.Member1_ID) AND (@CenterID IS NULL OR trans.Center = @CenterID)
                  ORDER BY trans.center,trans.client_no, trans.transact_no

                  OPEN @Client_Cursor
                  FETCH NEXT FROM @Client_Cursor INTO @ClientRowNumber,@TransNumber,@Client_Number,@ClientGUID,@CenterID2,@MembershipID,@ClientMembershipStatusID,@ContractPrice,@ContractPaidAmount,@MonthlyFee,@BeginDate,@EndDate,@MembershipCancelReasonID,@CancelDate,@IsActiveFlag,@UsedAccumQuantity,@TotalAccumQuantity, @BusinessSegmentDescriptionShort
