/* CreateDate: 03/12/2013 15:05:09.607 , ModifyDate: 02/06/2018 13:12:19.217 */
GO
/***********************************************************************
...
from various Fact tables
	...
	12/03/2014 - RH - Removed Xtrands from 10430 - BIO Conversions and 10431 - TRAD Conversions; Added 10433 Xtrands Conversions;
						Changed the statements for Conversions (WO#109328)
...
***********************************************************************/
CREATE PROCEDURE [dbo].[spsvc_InsertAccountingTableFlashValuesAccountsFeb2018] AS
BEGIN
	...
	*/
	...
	SET @RefreshInterval = -16
	--select @RefreshDate = DATEADD(dd, -1575, GETDATE())
	SET @RefreshDate = DATEADD(mm, @RefreshInterval, GETDATE())  --Goes back 16 months (@RefreshInterval)
	...
	*/
	...
	WHERE [DimDate].[FullDate] between @TransactionRefreshDate and @TransactionEndDate
		...
		AND [DimCenter].[CenterSSID] LIKE '[278]%'
	...
	*/
	...
	WHERE DimCenter.CenterSSID LIKE '[278]%'
	...
	*/
	...
	,	SUM(FactSalesConect.[PCP_NB2$] - FactSalesConect.[PCP_PCP$]) AS '10540' --Non-Program $
	,	SUM(FactSalesConect.[PCP_PCP$]) AS '10530'
	,	SUM(FactSalesConect.[PCP_Xtr$]) AS '10531'			--Added RH 12/23/2014
	,	SUM(FactSalesConect.[PCP_BIO$]) AS '10532'			--Added RH 06/30/2015
	,	SUM(FactSalesConect.[PCP_ExtMem$]) AS '10535'		--Moved from below RH 06/30/2015
	,	SUM(FactSalesConect.[PCP_Xtr$])+SUM(FactSalesConect.[PCP_BIO$])+SUM(FactSalesConect.[PCP_ExtMem$]) AS '10536'		--Moved from below RH 06/30/2015
	...
	,	SUM(FactSalesConect.NB_Trad# + FactSalesConect.NB_Grad# + FactSalesConect.NB_Ext# + FactSalesConect.NB_Xtr#  --Added Xtrands RH 11/6/2014
	...
	*/
	...
	WHERE FactPCP.CenterID LIKE '[278]%'
	...
	*/
	...
	--		AND Mem.[MembershipDescriptionShort] NOT IN ('EXTENH6','EXT6','EXTENH9','EXT12','EXTENH12','POSTEXT','POSTEL','Xtrand','XTRANDMEM','XTRAND6','XTRMEMSOL')
	--		THEN [FactSalesConect].[SF-Quantity] ELSE 0 END) AS '10430'  --PCP - BIO Conversion #
	,	SUM(NB_BIOConv#) AS '10430'	--PCP - BIO Conversion #  --Removed Xtrands RH 12/3/2014
	--,	SUM(CASE WHEN SC.[SalesCodeDepartmentSSID] IN (1075)
	--		AND Mem.[MembershipDescriptionShort] NOT IN ('EXTENH6','EXT6','EXTENH9','EXT12','EXTENH12','POSTEXT','POSTEL','Xtrand','XTRANDMEM','XTRAND6','XTRMEMSOL')
	--		AND Mem.[MembershipDescription] NOT LIKE ('GRAD%')
	--		THEN [FactSalesConect].[SF-Quantity] ELSE 0 END) AS '10431' --PCP - BIO (Trad) Conversion #  Removed Xtrands RH 12/3/2014
	,		SUM(CASE WHEN SC.[SalesCodeDepartmentSSID] IN (1075)
			AND NB_BIOConv# = 1
			AND Mem.[MembershipDescription] NOT LIKE 'Grad%'
			THEN [FactSalesConect].[SF-Quantity] ELSE 0 END) AS '10431'  --PCP - BIO (Trad) Conversion #
	,	SUM(CASE WHEN SC.[SalesCodeDepartmentSSID] IN (1075)
	...
	,	SUM(NB_XTRConv#) AS '10433'   --Added RH 12/3/2014
	--,	SUM(CASE WHEN SC.[SalesCodeDepartmentSSID] IN (1075)
	...
	,	SUM(NB_ExtConv#) AS '10435'  --EXT Conv #
	,	SUM(CASE WHEN SC.[SalesCodeDepartmentSSID] IN (1075)
			THEN [FactSalesConect].[SF-Quantity] ELSE 0 END) AS '10440'  --Xtrands will be included automatically
	...
	,	SUM(CASE WHEN SC.[SalesCodeDepartmentSSID] IN (5010, 5030, 5020, 5040, 5037) --Added 5037 Xtrands RH 11/6/2014
			AND Mem.[MembershipDescriptionShort] IN ('TRADITION', 'GRDSV','GRAD','GRDSV12','GRDSVSOL','GRADSOL6','GRDSVSOL','GRADSOL12','GRDSVSOL12', 'XTRAND','XTRAND6') --Added Xtrands NB RH 11/6/2014
	...
	,	SUM(CASE WHEN SC.[SalesCodeDepartmentSSID] IN (5010, 5020, 5030, 5035, 5037, 5038, 5040) --Added Xtrands Services - 5037,5038 RH 03/16/2015
			THEN [FactSalesConect].[SF-Quantity] ELSE 0 END) AS '10570'
	,	SUM(CASE WHEN SC.[SalesCodeDepartmentSSID] IN (5010, 5020, 5030, 5035, 5037, 5038, 5040) --Added Xtrands Services RH 03/16/2015
			THEN [FactSalesConect].[SF-ExtendedPrice] ELSE 0 END) AS '10575'
	,	SUM(CASE WHEN SC.[SalesCodeDepartmentSSID] IN (5010, 5020, 5030, 5035, 5036, 5037, 5038, 5040, 5050, 5060, 5061) --Added Xtrands Services RH 03/16/2015
	...
	*/
	...
	,	dbo.DivideNumeric((#Sales.[10305] + #Sales.[10306] + #Sales.[10315] + #Sales.[10310]), (#Sales.[10205] + #Sales.[10206] + #Sales.[10210] + #Sales.[10215])) AS '10300' --Added Xtrands 10206 - RH - 03/16/2015
	...
	,	dbo.DivideNumeric(#Sales.[10305], #Sales.[10205]) AS '10275' --NB - Traditional Average Sale $
	,	dbo.DivideNumeric(#Sales.[10306], #Sales.[10206]) AS '10281' --Added RH 11/6/2014 --[NB - Xtrands Average Sale $]
	...
	,	dbo.DivideNumeric(#Sales.[10206], #FactActivity.[10110]) AS '10172' --Added RH 11/6/2014  --[Closing Rate - Xtrands %]
	...
	,	dbo.[DivideNumeric]((#Sales.[10215] + #Sales.[10210] + #Sales.[10225] + #Sales.[10220] + #Sales.[10205] + #Sales.[10206]), #FactActivity.[10110]) AS '10190'  --Added Xtrands 10206 RH 11/6/2014
	--	,	dbo.[DivideNumeric]((#Sales.[10215] + #Sales.[10210] + #Sales.[10225] + #Sales.[10220] + #Sales.[10205]), #FactActivity.[10110]) AS '10190'

	,	dbo.[DivideNumeric]((#Sales.[10215] + #Sales.[10210] + #Sales.[10220] + #Sales.[10205] + #Sales.[10206]), #FactActivity.[10110]) AS '10191'  --Added Xtrands 10206 RH 11/6/2014
	--	,	dbo.[DivideNumeric]((#Sales.[10215] + #Sales.[10210] + #Sales.[10220] + #Sales.[10205]), #FactActivity.[10110]) AS '10191'

	,	#Sales.[10215] + #Sales.[10210] + #Sales.[10225] + #Sales.[10220] + #Sales.[10205]  + #Sales.[10206] AS '10231'  --Added Xtrands 10206 RH 11/6/2014
	--	,	#Sales.[10215] + #Sales.[10210] + #Sales.[10225] + #Sales.[10220] + #Sales.[10205] AS '10231'

	,	#Sales.[10215] + #Sales.[10210] + #Sales.[10220] + #Sales.[10205] + #Sales.[10206] AS '10232' --Added Xtrands 10206 RH 11/6/2014
	...
	,	#Sales.[10305] + #Sales.[10306] + #Sales.[10310] + #Sales.[10315] + #Sales.[10320] + #Sales.[10325] AS '10233' --Added Xtrands 10306 RH 11/6/2014
	--	,	#Sales.[10305] + #Sales.[10310] + #Sales.[10315] + #Sales.[10320] + #Sales.[10325] AS '10233'

	,	#Sales.[10305] + #Sales.[10306]+ #Sales.[10310] + #Sales.[10315] + #Sales.[10320] AS '10234' --Added Xtrands 10306 RH 11/6/2014
	--	,	#Sales.[10305] + #Sales.[10310] + #Sales.[10315] + #Sales.[10320] AS '10234'

	,	dbo.DivideNumeric((#Milestones.[10440] + #Milestones.[10435]), (#Sales.[10205] + #Sales.[10206] + #Sales.[10210] + #Sales.[10215])) AS '10485' --Added Xtrands 10206 RH 11/6/2014
	...
	FROM #Centers
		...
		INNER JOIN HC_BI_ENT_DDS.bi_ent_dds.DimCenter c
			ON #Centers.center = c.CenterSSID
