/* CreateDate: 03/04/2022 11:54:26.457 , ModifyDate: 03/04/2022 11:58:20.497 */
GO
/***********************************************************************
...
-492 C
-108 F
EXEC spRpt_DailyFlash 'F'

***********************************************************************/
CREATE PROCEDURE [dbo].[spRpt_DailyFlash_Test](
    @CenterType CHAR(1)
)
AS
BEGIN
    ...
    SET @CurrentDate = dateadd("DD",-10,getdate())
    ...
    SET @MTDDays = (SELECT COUNT(d.FullDate)
                    FROM HC_BI_ENT_DDS.bief_dds.DimDate d
                    WHERE d.FullDate BETWEEN @CurrentMonthStart AND @YesterdayEnd)
    SET @MTDWorkingDays = (SELECT COUNT(d.FullDate)
                           FROM HC_BI_ENT_DDS.bief_dds.DimDate d
                           WHERE d.FullDate BETWEEN @CurrentMonthStart AND @YesterdayEnd
                             AND d.DayOfWeekName NOT IN ('Sunday', 'Monday'))
    SET @WorkingDaysInMonth = (SELECT COUNT(d.FullDate)
                               FROM HC_BI_ENT_DDS.bief_dds.DimDate d
...
/********************************** Create temp table objects *************************************/
	...
	CREATE NONCLUSTERED INDEX ix_tempDate ON #Date ([StartDate],[EndDate]);
	...
	CREATE NONCLUSTERED INDEX ix_tempCenter ON #Center ([CenterKey],[CenterSSID])
	...
	CREATE NONCLUSTERED INDEX ix_tempBudget ON #Budget ([CenterKey],[CenterSSID])
	...
	CREATE NONCLUSTERED INDEX ix_tempSales on #Sales([FullDate])
	...
	CREATE NONCLUSTERED INDEX ix_tempRetailSales on #RetailSales([FullDate])
	...
	CREATE NONCLUSTERED INDEX ix_tempReceivables on #Receivables([FullDate])
	...
	CREATE NONCLUSTERED INDEX ix_tempLead on #Lead([FullDate])
	...
	CREATE NONCLUSTERED INDEX ix_tempTask on #Task([FullDate],[CenterNumber])
	...
	CREATE NONCLUSTERED INDEX ix_tempActivity on #Activity([FullDate])
	...
	CREATE NONCLUSTERED INDEX ix_tempFinal on #Final([ReportDate])
...
/********************************** Get Budget Data *************************************/
    ...
    WHERE d.FirstDateOfMonth BETWEEN @CurrentFiscalYearStart AND @YesterdayEnd
      AND a.AccountID IN
          (10125, 10230, 10305, 10310, 10430, 10206, 10306, 10433, 10215, 10225, 10315, 10325, 10435, 10220, 10221,
           10320, 10321, 10901, 10891, 10240, 10536, 10551, 10552, 10540, 10575, 10555, 10155, 10100, 10110, 10440
              )
...
-----This information is coming from Conect
    INSERT INTO #Sales
    SELECT dd.FullDate
         ...
         , SUM(CASE
                   WHEN sc.SalesCodeDepartmentSSID = 1099 AND m.RevenueGroupDescriptionShort = 'NB' THEN 1
                   ELSE 0 END)                                             AS 'NBCancels'
         , SUM(CASE
                   WHEN sc.SalesCodeDepartmentSSID = 1099 AND m.RevenueGroupDescriptionShort = 'PCP' THEN 1
                   ELSE 0 END)                                             AS 'PCPCancels'
         , SUM(CASE
                   WHEN sc.SalesCodeDepartmentSSID = 1099 AND m.RevenueGroupDescriptionShort IN ('NB', 'PCP') THEN 1
                   ELSE 0 END)                                             AS 'TotalCancels'
    FROM [syn_FactSalesTransaction] fst --???????
             INNER JOIN HC_BI_ENT_DDS.bief_dds.DimDate dd
...
--- no idea
    ...
    FROM syn_FactReceivables fr --- ??
             ...
             INNER JOIN #Center c
                        ON c.CenterNumber = ctr.CenterNumber
                            AND ctr.Active = 'Y'
             INNER JOIN HC_BI_CMS_DDS.bi_cms_dds.DimClient clt ---Connect
                        ON clt.ClientKey = fr.ClientKey
             INNER JOIN HC_BI_CMS_DDS.bi_cms_dds.DimClientMembership cm ---connect
             ...
             INNER JOIN HC_BI_CMS_DDS.bi_cms_dds.DimMembership m ---connect
    ...
    SELECT t.FullDate
         ...
         , SUM(CASE
                   WHEN
                           ...
                           ISNULL(t.Result__c, '') IN ('Completed', 'Show No Sale', 'Show Sale') and
                           ISNULL(t.Action__c, '') in ('Appointment', 'In House', 'Recovery', 'Be Back') and
...
/********************************** Get Totals *************************************/
    ...
    SELECT CONVERT(DATE, @CurrentDate)                                             AS 'ReportDate'
         ...
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10155)                                             AS 'BudgetLeads'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10100)                                             AS 'BudgetAppointments'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10110)                                             AS 'BudgetConsultations'
         , (SELECT (ISNULL(SUM(b.Value), 0) * 1.10)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10125)                                             AS 'BudgetGrossNB1Count'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10125)                                             AS 'BudgetNB1Count'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID IN
                  (10305, 10310, 10315, 10325, 10306, 10891, 10320, 10321, 10552)) AS 'BudgetNB1Sales'
         , NULL                                                                    AS 'BudgetNB1DollarPerSale'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10230)                                             AS 'BudgetXtrandsPlusInitialCount'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID IN (10305, 10310))                                   AS 'BudgetXtrandsPlusInitialSales'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10430)                                             AS 'BudgetXtrandsPlusInitialConversionCount'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10206)                                             AS 'BudgetXtrandsCount'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10306)                                             AS 'BudgetXtrandsSales'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10433)                                             AS 'BudgetXtrandsConversionCount'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID IN (10215, 10225))                                   AS 'BudgetEXTCount'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID IN (10315, 10325))                                   AS 'BudgetEXTSales'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10435)                                             AS 'BudgetEXTConversionCount'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID IN (10220, 10221))                                   AS 'BudgetSurgeryCount'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID IN (10320, 10321))                                   AS 'BudgetSurgerySales'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10901)                                             AS 'BudgetRestorInkCount'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10891)                                             AS 'BudgetRestorInkSales'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10240)                                             AS 'BudgetApplications'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID IN (10430, 10433, 10435))                            AS 'BudgetConversions'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10536)                                             AS 'BudgetRecurringSales'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID IN (10551, 10552))                                   AS 'BudgetLaserSales'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10540)                                             AS 'BudgetNonProgramSales'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10575)                                             AS 'BudgetServiceSales'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate BETWEEN d.StartDate AND d.EndDate
              AND b.AccountID = 10555)                                             AS 'BudgetRetailSales'
         ...
         , (SELECT dbo.DIVIDE_DECIMAL(ISNULL(SUM(NetNB1Sales), 0), ISNULL(SUM(NetNB1Count), 0))
            FROM #Sales s
            WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate)                    AS 'NetNB1Sales'
         ...
         , (SELECT ISNULL(SUM(NetNBLaserCount), 0)
            FROM #Sales s
            WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate)                    AS 'NetNBLaserCount'
         , (SELECT ISNULL(SUM(NetNBLaserSales), 0)
            FROM #Sales s
            WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate)                    AS 'NetNBLaserSales'
         , (SELECT ISNULL(SUM(NetPCPLaserCount), 0)
            FROM #Sales s
            WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate)                    AS 'NetPCPLaserCount'
         , (SELECT ISNULL(SUM(NetPCPLaserSales), 0)
            FROM #Sales s
            WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate)                    AS 'NetPCPLaserSales'
         ...
         , (SELECT ISNULL(SUM(RetailSales), 0)
            FROM #RetailSales rs
            WHERE rs.FullDate BETWEEN d.StartDate AND d.EndDate)                   AS 'RetailSales'
         , NULL                                                                    AS 'Royalty'
         ...
         , (SELECT ISNULL(SUM(NBCancels), 0)
            FROM #Sales s
            WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate)                    AS 'NBCancels'
         , (SELECT ISNULL(SUM(PCPCancels), 0)
            FROM #Sales s
            WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate)                    AS 'PCPCancels'
         , (SELECT ISNULL(SUM(TotalCancels), 0)
            FROM #Sales s
            WHERE s.FullDate BETWEEN d.StartDate AND d.EndDate)                    AS 'TotalCancels'
         ...
         , (SELECT ISNULL(SUM(ClientCount), 0)
            FROM #Receivables r
            WHERE r.FullDate BETWEEN d.StartDate AND d.EndDate)                    AS 'ReceivablesClientCount'
         ...
         , (SELECT ISNULL(SUM(a.InPersonConsultations), 0)
            FROM #Activity a
            WHERE a.FullDate BETWEEN d.StartDate AND d.EndDate)                    AS 'InPersonConsultations'
         , (SELECT ISNULL(SUM(a.VirtualConsultations), 0)
            FROM #Activity a
            WHERE a.FullDate BETWEEN d.StartDate AND d.EndDate)                    AS 'VirtualConsultations'
...
/********************************** Get NB1 $/Sale Budget Value *************************************/
    SET @BudgetNBDollarsPerSale = (SELECT dbo.DIVIDE_DECIMAL(f.BudgetNetNB1Sales, f.BudgetNetNB1Count)
                                   FROM #Final f
                                   WHERE f.DateDesc = 'Full Month')
...
/********************************** Update Dataset with Franchise Royalty *************************************/
    IF @CenterType = 'F'
        BEGIN
            UPDATE f
            SET f.Royalty = ((f.NetNB1Sales + f.PCPSales + f.NetNB2Sales + f.ServiceSales + f.RetailSales) *
                             @RoyaltyPercentage)
            FROM #Final f
        END
...
/********************************** Update Dataset with Daily Budgets *************************************/
    ...
    SET f_d.BudgetLeads                             = dbo.DIVIDE_DECIMAL(x_M.BudgetLeads, @DaysInMonth)
      , f_d.BudgetAppointments                      = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetAppointments, @WorkingDaysInMonth) END
      , f_d.BudgetConsultations                     = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetConsultations, @WorkingDaysInMonth) END
      , f_d.BudgetGrossNB1Count                     = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL((x_M.BudgetNetNB1Count * 1.10),
                                                                                  @WorkingDaysInMonth) END
      , f_d.BudgetNetNB1Count                       = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetNetNB1Count, @WorkingDaysInMonth) END
      , f_d.BudgetNetNB1Sales                       = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetNetNB1Sales, @WorkingDaysInMonth) END
      , f_d.BudgetXtrandsPlusInitialCount           = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetXtrandsPlusInitialCount,
                                                                                  @WorkingDaysInMonth) END
      , f_d.BudgetXtrandsPlusInitialSales           = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetXtrandsPlusInitialSales,
                                                                                  @WorkingDaysInMonth) END
      , f_d.BudgetXtrandsPlusInitialConversionCount = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(
                                                                  x_M.BudgetXtrandsPlusInitialConversionCount,
                                                                  @WorkingDaysInMonth) END
      , f_d.BudgetXtrandsCount                      = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetXtrandsCount, @WorkingDaysInMonth) END
      , f_d.BudgetXtrandsSales                      = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetXtrandsSales, @WorkingDaysInMonth) END
      , f_d.BudgetXtrandsConversionCount            = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetXtrandsConversionCount,
                                                                                  @WorkingDaysInMonth) END
      , f_d.BudgetEXTCount                          = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetEXTCount, @WorkingDaysInMonth) END
      , f_d.BudgetEXTSales                          = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetEXTSales, @WorkingDaysInMonth) END
      , f_d.BudgetEXTConversionCount                = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetEXTConversionCount,
                                                                                  @WorkingDaysInMonth) END
      , f_d.BudgetSurgeryCount                      = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetSurgeryCount, @WorkingDaysInMonth) END
      , f_d.BudgetSurgerySales                      = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetSurgerySales, @WorkingDaysInMonth) END
      , f_d.BudgetRestorInkCount                    = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetRestorInkCount, @WorkingDaysInMonth) END
      , f_d.BudgetRestorInkSales                    = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetRestorInkSales, @WorkingDaysInMonth) END
      , f_d.BudgetApplications                      = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetApplications, @WorkingDaysInMonth) END
      , f_d.BudgetConversions                       = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetConversions, @WorkingDaysInMonth) END
--,		f_d.BudgetRecurringSales = CASE WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ( 'Sunday', 'Monday' ) THEN 0 ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetRecurringSales, @DaysInMonth) END
      , f_d.BudgetLaserSales                        = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetLaserSales, @WorkingDaysInMonth) END
      , f_d.BudgetNonProgramSales                   = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetNonProgramSales, @WorkingDaysInMonth) END
      , f_d.BudgetServiceSales                      = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetServiceSales, @WorkingDaysInMonth) END
      , f_d.BudgetRetailSales                       = CASE
                                                          WHEN DATENAME(WEEKDAY, @YesterdayEnd) IN ('Sunday', 'Monday')
                                                              THEN 0
                                                          ELSE dbo.DIVIDE_DECIMAL(x_M.BudgetRetailSales, @WorkingDaysInMonth) END
    FROM #Final f_d
             CROSS APPLY (
...
/********************************** Update Dataset with MTD Budgets *************************************/
    ...
    SET f_m.BudgetLeads                             = ((dbo.DIVIDE_DECIMAL(f_m.BudgetLeads, @DaysInMonth)) * @MTDDays)
      , f_m.BudgetAppointments                      = ((dbo.DIVIDE_DECIMAL(f_m.BudgetAppointments, @DaysInMonth)) *
                                                       @MTDDays)
      , f_m.BudgetConsultations                     = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetConsultations, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetGrossNB1Count                     = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetGrossNB1Count, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetNetNB1Count                       = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetNetNB1Count, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetNetNB1Sales                       = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetNetNB1Sales, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetXtrandsPlusInitialCount           = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetXtrandsPlusInitialCount, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetXtrandsPlusInitialSales           = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetXtrandsPlusInitialSales, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetXtrandsPlusInitialConversionCount = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetXtrandsPlusInitialConversionCount, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetXtrandsCount                      = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetXtrandsCount, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetXtrandsSales                      = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetXtrandsSales, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetXtrandsConversionCount            = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetXtrandsConversionCount, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetEXTCount                          = ((dbo.DIVIDE_DECIMAL(f_m.BudgetEXTCount, @WorkingDaysInMonth)) *
                                                       @MTDWorkingDays)
      , f_m.BudgetEXTSales                          = ((dbo.DIVIDE_DECIMAL(f_m.BudgetEXTSales, @WorkingDaysInMonth)) *
                                                       @MTDWorkingDays)
      , f_m.BudgetEXTConversionCount                = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetEXTConversionCount, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetSurgeryCount                      = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetSurgeryCount, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetSurgerySales                      = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetSurgerySales, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetRestorInkCount                    = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetRestorInkCount, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetRestorInkSales                    = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetRestorInkSales, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetApplications                      = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetApplications, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetConversions                       = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetConversions, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetRecurringSales                    = ((dbo.DIVIDE_DECIMAL(f_m.BudgetRecurringSales, @DaysInMonth)) *
                                                       @MTDDays)
      , f_m.BudgetLaserSales                        = ((dbo.DIVIDE_DECIMAL(f_m.BudgetLaserSales, @WorkingDaysInMonth)) *
                                                       @MTDWorkingDays)
      , f_m.BudgetNonProgramSales                   = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetNonProgramSales, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetServiceSales                      = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetServiceSales, @WorkingDaysInMonth)) * @MTDWorkingDays)
      , f_m.BudgetRetailSales                       = (
            (dbo.DIVIDE_DECIMAL(f_m.BudgetRetailSales, @WorkingDaysInMonth)) * @MTDWorkingDays)
    FROM #Final f_m
...
/********************************** Update Dataset with QTD Budgets *************************************/
    ...
    SET f_q.BudgetLeads                             = ((f_q.BudgetLeads - x_M.BudgetLeads) + x_MTD.BudgetLeads)
      , f_q.BudgetAppointments                      = ((f_q.BudgetAppointments - x_M.BudgetAppointments) +
                                                       x_MTD.BudgetAppointments)
      , f_q.BudgetConsultations                     = ((f_q.BudgetConsultations - x_M.BudgetConsultations) +
                                                       x_MTD.BudgetConsultations)
      , f_q.BudgetGrossNB1Count                     = ((f_q.BudgetGrossNB1Count - x_M.BudgetGrossNB1Count) +
                                                       x_MTD.BudgetGrossNB1Count)
      , f_q.BudgetNetNB1Count                       = ((f_q.BudgetNetNB1Count - x_M.BudgetNetNB1Count) +
                                                       x_MTD.BudgetNetNB1Count)
      , f_q.BudgetNetNB1Sales                       = ((f_q.BudgetNetNB1Sales - x_M.BudgetNetNB1Sales) +
                                                       x_MTD.BudgetNetNB1Sales)
      , f_q.BudgetXtrandsPlusInitialCount           = (
            (f_q.BudgetXtrandsPlusInitialCount - x_M.BudgetXtrandsPlusInitialCount) +
            x_MTD.BudgetXtrandsPlusInitialCount)
      , f_q.BudgetXtrandsPlusInitialSales           = (
            (f_q.BudgetXtrandsPlusInitialSales - x_M.BudgetXtrandsPlusInitialSales) +
            x_MTD.BudgetXtrandsPlusInitialSales)
      , f_q.BudgetXtrandsPlusInitialConversionCount = (
            (f_q.BudgetXtrandsPlusInitialConversionCount - x_M.BudgetXtrandsPlusInitialConversionCount) +
            x_MTD.BudgetXtrandsPlusInitialConversionCount)
      , f_q.BudgetXtrandsCount                      = ((f_q.BudgetXtrandsCount - x_M.BudgetXtrandsCount) +
                                                       x_MTD.BudgetXtrandsCount)
      , f_q.BudgetXtrandsSales                      = ((f_q.BudgetXtrandsSales - x_M.BudgetXtrandsSales) +
                                                       x_MTD.BudgetXtrandsSales)
      , f_q.BudgetXtrandsConversionCount            = (
            (f_q.BudgetXtrandsConversionCount - x_M.BudgetXtrandsConversionCount) + x_MTD.BudgetXtrandsConversionCount)
      ...
      , f_q.BudgetEXTConversionCount                = ((f_q.BudgetEXTConversionCount - x_M.BudgetEXTConversionCount) +
                                                       x_MTD.BudgetEXTConversionCount)
      , f_q.BudgetSurgeryCount                      = ((f_q.BudgetSurgeryCount - x_M.BudgetSurgeryCount) +
                                                       x_MTD.BudgetSurgeryCount)
      , f_q.BudgetSurgerySales                      = ((f_q.BudgetSurgerySales - x_M.BudgetSurgerySales) +
                                                       x_MTD.BudgetSurgerySales)
      , f_q.BudgetRestorInkCount                    = ((f_q.BudgetRestorInkCount - x_M.BudgetRestorInkCount) +
                                                       x_MTD.BudgetRestorInkCount)
      , f_q.BudgetRestorInkSales                    = ((f_q.BudgetRestorInkSales - x_M.BudgetRestorInkSales) +
                                                       x_MTD.BudgetRestorInkSales)
      , f_q.BudgetApplications                      = ((f_q.BudgetApplications - x_M.BudgetApplications) +
                                                       x_MTD.BudgetApplications)
      , f_q.BudgetConversions                       = ((f_q.BudgetConversions - x_M.BudgetConversions) +
                                                       x_MTD.BudgetConversions)
      , f_q.BudgetRecurringSales                    = ((f_q.BudgetRecurringSales - x_M.BudgetRecurringSales) +
                                                       x_MTD.BudgetRecurringSales)
      , f_q.BudgetLaserSales                        = ((f_q.BudgetLaserSales - x_M.BudgetLaserSales) + x_MTD.BudgetLaserSales)
      , f_q.BudgetNonProgramSales                   = ((f_q.BudgetNonProgramSales - x_M.BudgetNonProgramSales) +
                                                       x_MTD.BudgetNonProgramSales)
      , f_q.BudgetServiceSales                      = ((f_q.BudgetServiceSales - x_M.BudgetServiceSales) +
                                                       x_MTD.BudgetServiceSales)
      , f_q.BudgetRetailSales                       = ((f_q.BudgetRetailSales - x_M.BudgetRetailSales) +
                                                       x_MTD.BudgetRetailSales)
    FROM #Final f_q
             CROSS APPLY (
        SELECT f_m.StartDate
             , f_m.EndDate
...
/********************************** Save Final Data *************************************/
    ...
    SELECT f.ReportDate
         ...
         , ROUND((f.BIOConversions - f.BudgetXtrandsPlusInitialConversionCount),
                 0)                                                                        AS 'XtrandsPlusConversionsCountVariance'
