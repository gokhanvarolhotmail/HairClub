...
-- ----------------------------------------------------------------------------------------------
Sample Execution:
exec spRpt_FranchiseWarBoards_NB2 7, 2018
***********************************************************************/
CREATE PROCEDURE [dbo].[spRpt_FranchiseWarBoards_NB2] (
	...
	BEGIN
		...
		INTO #PcpNumbersData
		FROM #Accounting a
		INNER JOIN HC_BI_ENT_DDS.bi_ent_dds.DimCenter ce ON ce.CenterSSID = a.CenterNumber
		INNER JOIN HC_BI_ENT_DDS.bi_ent_dds.DimRegion re ON re.RegionKey = ce.RegionKey
		...
		INNER JOIN HC_BI_ENT_DDS.bi_ent_dds.DimRegion re ON re.RegionKey = ce.RegionKey
		--LEFT OUTER JOIN [#PcpNumbersData] cl ON ce.CenterNumber = cl.[Center]
		LEFT OUTER JOIN [#PcpNumbersData] cl ON ce.CenterNumber = cl.CenterNumber
		WHERE ce.CenterTypeSSID IN (2,3)
		AND ce.Active = 'Y'
