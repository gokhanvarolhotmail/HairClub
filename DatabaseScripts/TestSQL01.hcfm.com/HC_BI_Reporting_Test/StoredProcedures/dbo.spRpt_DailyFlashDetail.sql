/* CreateDate: 06/19/2021 08:19:06.107 , ModifyDate: 01/20/2022 12:05:19.227 */
GO
/***********************************************************************
...
EXEC spRpt_DailyFlashDetail 'C'
EXEC spRpt_DailyFlashDetail 'F'
***********************************************************************/
CREATE PROCEDURE [dbo].[spRpt_DailyFlashDetail](
    @CenterType CHAR(1)
)
AS
BEGIN
...
/********************************** Create temp table objects *************************************/
	...
	CREATE NONCLUSTERED INDEX ix_tempDate ON #Date ([FULLDate]);
	...
	CREATE NONCLUSTERED INDEX ix_tempFinal on #Final([FullDate])
    ...
    (
		...
		CREATE NONCLUSTERED INDEX ix_tempDailyBudget on #DailyBudget([FullDate])
...
/********************************** Get Budget Data *************************************/
    ...
    SELECT c.CenterKey
         ...
         , fa.Budget AS 'Value'
    FROM [dbo].[syn_FactAccounting] fa
...
/********************************** Get Lead data *************************************/
    INSERT INTO #Lead
    SELECT CAST(l.CreatedDateest AS DATE) AS 'FullDate'
         , COUNT(l.Id)                    AS 'Leads'

    FROM syn_vwLeadMS l --SF
             INNER JOIN #Center c
    ...
    WHERE l.Status IN
          ('Lead', 'Client', 'Prospect', 'Event', 'HWClient', 'HWLead', 'NEW', 'PURSUING', 'CONVERTED', 'SCHEDULED',
           ...
           'Establishing Value',
           'Converted')
      AND CAST(l.CreatedDateEst AS DATE) BETWEEN @MinDate AND @YesterdayEnd
      AND ISNULL(l.IsDeleted, 0) = 0
      and isvalid = 1
    GROUP BY CAST(l.CreatedDateEst AS DATE)
...
/********************************** Get Task data *************************************/
    ...
    FROM syn_vwTaskMS t
             ...
             LEFT OUTER JOIN syn_vwLeadMS l
                             ON l.Id = t.WhoId
             LEFT OUTER JOIN syn_Account a
...
/********************************** Get Activity data *************************************/
    ...
    SELECT t.FullDate
         , SUM(CASE
                   WHEN t.Action__c IN ('Appointment', 'In House', 'Recovery') AND ISNULL(t.Result__c, '') NOT IN
                                                                                   (--'Void',
                                                                                    --'Cancel',
                                                                                    --'Reschedule',
                                                                                    --'Center Exception',
         ...
         , SUM(CASE
                   WHEN ISNULL(t.Result__c, '') IN ('Completed', 'Show Sale', 'Show No Sale') AND
                        ...
                        ISNULL(t.Action__c, '') in ('Appointment', 'Be back')
                       THEN 1
                   ELSE 0 END)                                                                       AS 'Consultations'

         , SUM(CASE
                   WHEN
                               ISNULL(t.Accomodation, 'In Person Consult') in
                               ('In Person Consult', 'Center', 'company', 'Center Consultation') AND
                               ISNULL(t.Result__c, '') IN ('Completed') and
         ...
         , SUM(CASE
                   WHEN ISNULL(t.Accomodation, 'In Person Consult') in ('Virtual', 'Virtual Consultation', 'Video') AND
                        ISNULL(t.Result__c, '') IN ('Completed') and
                       -- ISNULL(t.Result__c, '') IN ('Show No Sale', 'Show Sale') AND
                        ISNULL(t.ExcludeFromConsults, 0) = 0
                       THEN 1
...
/********************************** Get Totals *************************************/
    ...
    SELECT d.FullDate
         ...
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate = d.FullDate AND b.AccountID = 10155)                                               AS 'BudgetLeads'
         , (SELECT ISNULL(SUM(b.Value), 0)
            FROM #Budget b
            WHERE b.FullDate = d.FullDate
         ...
         , (SELECT ISNULL(SUM(NB1Applications), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NB1Applications'
         , (SELECT ISNULL(SUM(GrossNB1Count), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'GrossNB1Count'
         , (SELECT ISNULL(SUM(NetNB1Count), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetNB1Count'
         , (SELECT ISNULL(SUM(NetNB1Sales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetNB1Sales'
         , (SELECT dbo.DIVIDE_DECIMAL(ISNULL(SUM(NetNB1Sales), 0), ISNULL(SUM(NetNB1Count), 0))
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetNB1Sales'
         , (SELECT ISNULL(SUM(NetTradCount), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetTradCount'
         , (SELECT ISNULL(SUM(NetTradSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetTradSales'
         , (SELECT ISNULL(SUM(NetGradCount), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetGradCount'
         , (SELECT ISNULL(SUM(NetGradSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetGradSales'
         , (SELECT ISNULL(SUM(NetEXTCount), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetEXTCount'
         , (SELECT ISNULL(SUM(NetEXTSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetEXTSales'
         , (SELECT ISNULL(SUM(NetXtrCount), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetXtrCount'
         , (SELECT ISNULL(SUM(NetXtrSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetXtrSales'
         , (SELECT ISNULL(SUM(SurgeryCount), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'SurgeryCount'
         , (SELECT ISNULL(SUM(SurgerySales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'SurgerySales'
         , (SELECT ISNULL(SUM(PostEXTCount), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'PostEXTCount'
         , (SELECT ISNULL(SUM(PostEXTSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'PostEXTSales'
         , (SELECT ISNULL(SUM(NetMDPCount), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetMDPCount'
         , (SELECT ISNULL(SUM(NetMDPSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetMDPSales'
         , (SELECT ISNULL(SUM(NetLaserCount), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetLaserCount'
         , (SELECT ISNULL(SUM(NetLaserSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetLaserSales'
         , (SELECT ISNULL(SUM(NetNBLaserCount), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetNBLaserCount'
         , (SELECT ISNULL(SUM(NetNBLaserSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetNBLaserSales'
         , (SELECT ISNULL(SUM(NetPCPLaserCount), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetPCPLaserCount'
         , (SELECT ISNULL(SUM(NetPCPLaserSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetPCPLaserSales'
         , (SELECT ISNULL(SUM(PCPSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'PCPSales'
         , (SELECT ISNULL(SUM(NetPCPSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetPCPSales'
         , (SELECT ISNULL(SUM(NetNB2Sales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NetNB2Sales'
         , (SELECT ISNULL(SUM(ServiceSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'ServiceSales'
         , (SELECT ISNULL(SUM(RetailSales), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'RetailSales'
         , (SELECT ISNULL(SUM(BIOConversions), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'BIOConversions'
         , (SELECT ISNULL(SUM(EXTConversions), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'EXTConversions'
         , (SELECT ISNULL(SUM(XTRConversions), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'XTRConversions'
         , (SELECT ISNULL(SUM(TotalConversions), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'TotalConversions'
         , (SELECT ISNULL(SUM(NBCancels), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'NBCancels'
         , (SELECT ISNULL(SUM(PCPCancels), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'PCPCancels'
         , (SELECT ISNULL(SUM(TotalCancels), 0)
            FROM #Sales s
            WHERE s.FullDate = d.FullDate)                                                                       AS 'TotalCancels'
         , (SELECT ISNULL(SUM(Leads), 0)
            FROM #Lead l
            WHERE l.FullDate = d.FullDate)                                                                       AS 'Leads'
         , (SELECT ISNULL(SUM(Appointments), 0)
            FROM #Activity a
            WHERE a.FullDate = d.FullDate)                                                                       AS 'Appointments'
         , (SELECT ISNULL(SUM(Consultations), 0)
            FROM #Activity a
            WHERE a.FullDate = d.FullDate)                                                                       AS 'Consultations'
         , (SELECT ISNULL(SUM(a.InPersonConsultations), 0)
            FROM #Activity a
            WHERE a.FullDate = d.FullDate)                                                                       AS 'InPersonConsultations'
         , (SELECT ISNULL(SUM(a.VirtualConsultations), 0)
            FROM #Activity a
            WHERE a.FullDate = d.FullDate)                                                                       AS 'VirtualConsultations'
         , (SELECT ISNULL(SUM(BeBacks), 0)
            FROM #Activity a
            WHERE a.FullDate = d.FullDate)                                                                       AS 'BeBacks'
         , (SELECT ISNULL(SUM(a.BeBacksToExclude), 0)
            FROM #Activity a
            WHERE a.FullDate = d.FullDate)                                                                       AS 'BeBacksToExclude'
         , (SELECT ISNULL(SUM(Shows), 0)
            FROM #Activity a
            WHERE a.FullDate = d.FullDate)                                                                       AS 'Shows'
         , (SELECT ISNULL(SUM(Sales), 0)
            FROM #Activity a
            WHERE a.FullDate = d.FullDate)                                                                       AS 'Sales'
    FROM #Date d
...
/********************************** Get NB1 $/Sale Budget Value *************************************/
    SET @BudgetNBDollarsPerSale =
            (SELECT dbo.DIVIDE_DECIMAL(SUM(f.BudgetNetNB1Sales), SUM(f.BudgetNetNB1Count)) FROM #Final f)
...
/********************************** Calculate Daily Budgets *************************************/
    ...
    SELECT d.FullDate
         , d.DayOfWeekName
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetLeads, @DaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetLeads'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetAppointments, @DaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetAppointments'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetConsultations, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetConsultations'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetGrossNB1Count, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetGrossNB1Count'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetNetNB1Count, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetNetNB1Count'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetNetNB1Sales, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetNetNB1Sales'
         , @BudgetNBDollarsPerSale                 AS 'BudgetNBDollarsPerSale'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetXtrandsPlusInitialCount, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetXtrandsPlusInitialCount'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetXtrandsPlusInitialSales, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetXtrandsPlusInitialSales'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetXtrandsPlusInitialConversionCount, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetXtrandsPlusInitialConversionCount'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetXtrandsCount, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetXtrandsCount'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetXtrandsSales, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetXtrandsSales'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetXtrandsConversionCount, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetXtrandsConversionCount'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetEXTCount, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetEXTCount'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetEXTSales, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetEXTSales'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetEXTConversionCount, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetEXTConversionCount'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetSurgeryCount, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetSurgeryCount'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetSurgerySales, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetSurgerySales'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetRestorInkCount, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetRestorInkCount'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetRestorInkSales, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetRestorInkSales'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetApplications, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetApplications'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetConversions, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetConversions'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetRecurringSales, @DaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetRecurringSales'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetLaserSales, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetLaserSales'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetNonProgramSales, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetNonProgramSales'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetServiceSales, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetServiceSales'
         , (SELECT dbo.DIVIDE_DECIMAL(f.BudgetRetailSales, @WorkingDaysInMonth)
            FROM #Final f
            WHERE f.FullDate = @CurrentMonthStart) AS 'BudgetRetailSales'
    FROM #Date d
...
/********************************** Return Final Data *************************************/
    SELECT CONVERT(VARCHAR(11), dfd.FullDate, 101) AS 'FullDate'
         , DAY(dfd.FullDate)                       AS 'DayOfMonth'
         , dfd.DayOfWeekName
         , dfd.BudgetLeads
         , dfd.Leads
         , dfd.BudgetAppointments
         , dfd.Appointments
         , dfd.BudgetConsultations
         , dfd.Consultations
         , dfd.BudgetGrossNB1Count
         , dfd.GrossNB1Count
         , dfd.BudgetNetNB1Count
         , dfd.NetNB1Count
         , dfd.BudgetNetNB1Sales
         , dfd.NetNB1Sales
         , dfd.BudgetNB1DollarPerSale
         , dfd.NB1DollarPerSale
         , dfd.BudgetXtrandsPlusCount
         , dfd.XtrandsPlusCount
         , dfd.BudgetXtrandsPlusSales
         , dfd.XtrandsPlusSales
         , dfd.BudgetXtrandsPlusConversionsCount
         , dfd.XtrandsPlusConversionsCount
         , dfd.BudgetXtrandsCount
         , dfd.XtrandsCount
         , dfd.BudgetXtrandsSales
         , dfd.XtrandsSales
         , dfd.BudgetXtrandsConversionCount
         , dfd.XtrandsConversionsCount
         , dfd.BudgetEXTCount
         , dfd.EXTCount
         , dfd.BudgetEXTSales
         , dfd.EXTSales
         , dfd.BudgetEXTConversionCount
         , dfd.EXTConversionsCount
         , dfd.BudgetSurgeryCount
         , dfd.SurgeryCount
         , dfd.BudgetSurgerySales
         , dfd.SurgerySales
         , dfd.BudgetRestorInkCount
         , dfd.RestorInkCount
         , dfd.BudgetRestorInkSales
         , dfd.RestorInkSales
         , dfd.BudgetApplicationsCount
         , dfd.ApplicationsCount
         , dfd.BudgetConversionsCount
         , dfd.ConversionsCount
         , dfd.BudgetNetPCPSales
         , dfd.NetPCPSales
         , dfd.BudgetLaserSales
         , dfd.LaserSales
         , dfd.BudgetNonProgramSales
         , dfd.NonProgramSales
         , dfd.BudgetServiceSales
         , dfd.ServiceSales
         , dfd.BudgetRetailSales
         , dfd.RetailSales
         , dfd.NBCancels
         , dfd.PCPCancels
         , dfd.TotalCancels
    FROM datDailyFlashDetail dfd
