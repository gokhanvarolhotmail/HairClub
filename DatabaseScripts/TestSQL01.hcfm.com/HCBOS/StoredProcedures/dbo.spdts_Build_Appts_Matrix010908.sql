...
WHILE (@ApptDate <= @EndDate) BEGIN
	...
	SELECT hcm.dbo.cstd_company_schedule.company_id, cst_center_number, hcm.dbo.oncd_company_address.time_zone_code FROM hcm.dbo.cstd_company_schedule
		INNER JOIN hcm.dbo.oncd_company ON hcm.dbo.oncd_company.company_id = hcm.dbo.cstd_company_schedule.company_id
		LEFT OUTER JOIN hcm.dbo.oncd_company_address ON hcm.dbo.oncd_company.company_id = hcm.dbo.oncd_company_address.company_id AND hcm.dbo.oncd_company_address.primary_flag = 'Y'
		WHERE 1=1
			...
			AND NOT EXISTS (SELECT 1 FROM hcm.dbo.cstd_company_schedule_by_date
						WHERE company_id = hcm.dbo.cstd_company_schedule.company_id
							AND schedule_date = @ApptDate
							AND (SUBSTRING(CONVERT(varchar(16),hcm.dbo.cstd_company_schedule_by_date.first_appointment,120),12,5) <= SUBSTRING(CONVERT(varchar(16),hcm.dbo.cstd_company_schedule.first_appointment,120),12,5)
								AND
							     SUBSTRING(CONVERT(varchar(16),hcm.dbo.cstd_company_schedule_by_date.last_appointment,120),12,5) >= SUBSTRING(CONVERT(varchar(16),hcm.dbo.cstd_company_schedule.last_appointment,120),12,5)
							    )
							AND hcm.dbo.cstd_company_schedule_by_date.status_code = 'CLOSED')
	...
	BEGIN
		...
		BEGIN
			...
			SET @csbdStatus = (SELECT status_code FROM hcm.dbo.cstd_company_schedule_by_date WHERE company_id = @currentCompany AND schedule_date = @ApptDate AND @ApptTimeString BETWEEN SUBSTRING(CONVERT(varchar(16),first_appointment,120),12,5) AND SUBSTRING(CONVERT(varchar(16),last_appointment,120),12,5))
			...
			IF ISNULL(@csbdStatus,'') <> ''
				BEGIN
					...
					ELSE
						BEGIN
							IF @UseHalfHour = 0
								SET @NoAppts = (SELECT hour_appointments FROM hcm.dbo.cstd_company_schedule_by_date WHERE company_id = @currentCompany AND schedule_date = @ApptDate AND status_code = 'OPEN' AND @ApptTimeString BETWEEN SUBSTRING(CONVERT(varchar(16),first_appointment,120),12,5) AND SUBSTRING(CONVERT(varchar(16),last_appointment,120),12,5))
							ELSE
			...
			ELSE
				BEGIN
					...
					SET @currentTimeframeDailyID = (SELECT company_schedule_timeframe_daily_id FROM hcm.dbo.cstd_company_schedule_timeframe_daily std INNER JOIN hcm.dbo.cstd_company_schedule_timeframe st ON st.company_schedule_timeframe_id = std.company_schedule_timeframe_id AND @ApptDate BETWEEN st.start_date AND st.end_date AND st.active = 'Y' INNER JOIN hcm.dbo.cstd_company_schedule s ON st.company_schedule_id = s.company_schedule_id AND s.company_id = @currentCompany WHERE std.day_of_week = @currentWeekday)

					IF @currentTimeframeDailyID IS NOT NULL
						BEGIN
							IF (SELECT status_code FROM hcm.dbo.cstd_company_schedule_timeframe_daily WHERE company_schedule_timeframe_daily_id = @currentTimeframeDailyID AND @ApptTimeString BETWEEN SUBSTRING(CONVERT(varchar(16),first_appointment,120),12,5) AND SUBSTRING(CONVERT(varchar(16),last_appointment,120),12,5)) = 'CLOSED'
								SET @NoAppts = 0
							ELSE
								BEGIN
...
--END
			IF @NoAppts > 0
				BEGIN
					INSERT INTO appointments_matrix(Center, ApptDate, ApptTime, Appts) VALUES (CONVERT(int,@currentCenterCode), CONVERT(varchar(10),@ApptDate,101),@ApptTimeString, @NoAppts )
				END
			SET @slotNumber = @slotNumber + 1
		END
