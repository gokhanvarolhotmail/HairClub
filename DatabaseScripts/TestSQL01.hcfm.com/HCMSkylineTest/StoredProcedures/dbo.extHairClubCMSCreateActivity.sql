/* CreateDate: 08/10/2015 11:43:07.110 , ModifyDate: 09/29/2016 15:08:53.617 */
GO
SET ANSI_NULLS ON
GO
...
BEGIN TRANSACTION
      ...
      INSERT  intO oncd_activity (activity_id, due_date, start_time, action_code, result_code, completed_by_user_code, completion_date, completion_time, created_by_user_code, creation_date, updated_by_user_code, updated_date, [description], cst_activity_type_code, source_code, cst_time_zone_code)
            VALUES (@ActivityID, CAST(CONVERT(varchar(11),  GETDATE(),  101) AS datetime), CAST(CONVERT(TIME,GETDATE()) AS DateTime), @ActionCode, @ResultCode, NULL, NULL, NULL, @p_User, GETDATE(), @p_User, GETDATE(), @Description, @ActivityType, CASE WHEN @ActionCode = 'INHOUSE' THEN @ActivitySource ELSE NULL END, @TimeZoneCode )
